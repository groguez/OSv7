...
class Woo_attributes extends MY_Woo
{
	...
	public function save_attribute($attribute_id)
	{
		...
		try
		{
			...
			$this->data = $this->make_attribute_data($attribute,$attribute_id);
	...
	private function make_attribute_data($attribute,$attribute_id)
	{	
                $data = array(
                        'name' => $attribute['name'],
                        'slug' => $attribute_id.'-'.mb_substr_if_possible(mystr_replace(' ',  '-', $attribute['name']), 0, 23),
                );
	...
	public function batch_attributes($attribute_ids = array())
	{
		...
		foreach($attributes as $attribute_id => $attribute)
		{
			...
			{
				$data = $this->make_attribute_data($attribute,$attribute_id);
