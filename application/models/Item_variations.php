	...
	*/
	function get_variations($item_id = array(), $include_deleted = false)
	{
		...
		if (is_array($item_id))
		{
			...
			{
				$return[$result['item_id']][$result['id']]['name'] = $result['name'] ? $result['name'] : implode(', ', array_column($this->get_attributes($result['id']), 'label'));
				...
				$return[$result['item_id']][$result['id']]['limited_sales_quantity'] = $result['limited_sales_quantity'];
			}
		}
		else
		{
			foreach($this->db->get()->result_array() as $result)
			{
				$return[$result['id']]['name'] = $result['name'] ? $result['name'] :  implode(', ', array_column($this->get_attributes($result['id']), 'label'));
				...
				$return[$result['id']]['limited_sales_quantity'] = $result['limited_sales_quantity'];
			}
		}
	...
	function lookup($item_id, $attribute_value_ids, $supplier_id = false,$dont_check_count = false)
	{
		...
		if ($dont_check_count)
		{
			$this->db->where("id IN (SELECT `item_variation_id` FROM `".$this->db->dbprefix('item_variation_attribute_values')."` GROUP BY `item_variation_id`)",NULL,FALSE);
		}
		else
		{
