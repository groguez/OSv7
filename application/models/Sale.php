...
class Sale extends MY_Model
{
	...
	{				
		...
		if(($account_sid && $auth_token && $twilio_sms_from && $to_phone_number && $text_message) || ($this->config->item('sms_mobile_tel_username') && $this->config->item('sms_mobile_tel_password') && $text_message))
		{
			...
			$this->Common->send_sms($to_phone_number, $text_message);
		}
	}
	...
	function get_receipt_signature($sale_id)
	{
		...
		return hash('sha1',$sale_id."|".$db_name.'|'.$secure_key);
	}
	...
	function get_payment_sales_total_for_shift($shift_start, $shift_end,$payment_type)
  {
		...
		foreach($all_payments_for_sales as $sale_id => $payment_rows)
		{
			if (isset($sales_totals[$sale_id]))
			{
				...
				foreach($payment_rows as $payment_row)
				{
					...
					elseif($sales_totals[$sale_id] < 0 && $payment_row['payment_amount']  >= 0)
					{
						$payment_amount = $total_sale_balance;
					}
	...
	function _get_all_sale_payments($sale_ids, $order_by=false)
	{
		...
		if (count($sale_ids) > 0)
		{			
			// Retrieve payments in manageable chunks instead of one massive
			// OR chain. Each chunk is queried separately and the results are
			// merged in PHP.
			...
			foreach($sale_ids_chunk as $ids)
			{
				$this->db->select('phppos_sp.*, SUBSTRING_INDEX(phppos_sp.payment_type, ":", 1) as payment_type, phppos_l.name as location_name,phppos_s.sale_time,phppos_r.name as register');
		      	$this->db->from('sales_payments as phppos_sp');
		      	$this->db->join('sales as phppos_s', 'phppos_s.sale_id=phppos_sp.sale_id');
		      	$this->db->join('locations as phppos_l', 'phppos_l.location_id=phppos_s.location_id');
		      	$this->db->join('registers as phppos_r', 'phppos_r.register_id=phppos_s.register_id','left');
				$this->db->where_in('phppos_sp.sale_id',$ids);
				...
				{
					$prefix = 'phppos_sp';
					$paymentAmountCondition = "CASE WHEN phppos_s.total < 0 THEN payment_amount > 0 ELSE payment_amount < 0 END";
					$orderConditions = [
	...
	function get_payment_data_grouped_by_sale($payments_by_sale,$sales_totals)
	{
					...
					if (!isset($foreign_language_to_cur_language[$payment_row['payment_type']]) || !isset($payment_data[$sale_id][$foreign_language_to_cur_language[$payment_row['payment_type']]]))
					{
						...
						if (strpos($payment_row['payment_type'],':') !== FALSE && !isset($foreign_language_to_cur_language[$payment_row['payment_type']]))
						{
			   	   	list($giftcard_translation, $giftcard_number) = explode(":",$payment_row['payment_type']);
							...
							{
								$payment_data[$sale_id][$foreign_language_to_cur_language[$payment_row['payment_type']]] = array('sale_id' => $sale_id, 'paid_online' => $payment_row['paid_online'],'payment_type' => $foreign_language_to_cur_language[$payment_row['payment_type']], 'payment_amount' => 0,'payment_date' => $payment_row['payment_date'], 'sale_time' => $payment_row['sale_time'], 'location_name' => $payment_row['location_name'] );
							}
							$payment_key = $foreign_language_to_cur_language[$payment_row['payment_type']];
						...
						else
						{
							...
							{
								$payment_data[$sale_id][$payment_row['payment_type']] = array('sale_id' => $sale_id, 'paid_online' => $payment_row['paid_online'],'payment_type' => $payment_row['payment_type'], 'payment_amount' => 0,'payment_date' => $payment_row['payment_date'], 'sale_time' => $payment_row['sale_time'], 'location_name' => $payment_row['location_name'] );
							}
       ...
       /**
        * Returns an array mapping sale_id to total for the current register
        * and shift. Previously this method fetched sale_ids first and then
        * executed another query to retrieve totals. The current implementation
        * performs a single grouped query which reduces round trips to the
        * database and yields faster results when closing the register.
        */
       function get_sales_totaled_by_id($shift_start, $shift_end)
	{
		$register_id = $this->Employee->get_logged_in_employee_current_register_id();
               ...
               $sales_totals = array();
               foreach($this->db->select('sales.sale_id, sales.total')
                               ->from('sales')
                               ->join('sales_payments','sales_payments.sale_id=sales.sale_id')
                               ->where('sales_payments.payment_date >=', $shift_start)
                               ->where('sales_payments.payment_date <=', $shift_end)
                               ->where('register_id', $register_id)
                               ->where($this->db->dbprefix('sales').'.deleted', 0)
                               ->group_by('sales.sale_id')
                               ->get()->result_array() as $sale_row)
               {
                       $sales_totals[$sale_row['sale_id']] = $sale_row['total'];
               }
	...
	{	
		...
		$already_paid_store_account_amount = 0;
		if ($sale_id && $this->is_store_accounts_paid_sale_already_exist($sale_id))
		{
			$already_paid_store_account_amount = "";
			
			$this->db->select('SUM(transaction_amount) as already_paid_store_account_amount');
			$this->db->from('store_accounts_paid_sales');
			$this->db->join('store_accounts', 'store_accounts_paid_sales.store_account_payment_sale_id = store_accounts.sale_id');
			$this->db->where('store_accounts_paid_sales.sale_id',$sale_id);

			$ret = $this->db->get()->row_array();
			$already_paid_store_account_amount = $ret['already_paid_store_account_amount'];
		}		
		...
		if ($sale_id)
		{
            ...
            $sale_create_date = $before_save_sale_info->created_at;
		...
		else
		{
            ...
            $sale_create_date = date('Y-m-d H:i:s');
		}
		...
		$sales_data = array(
	        ...
	        'created_at'		               => $sale_create_date,
	        'updated_at'               		   => $sale_id ? date('Y-m-d H:i:s') : NULL,
			...
			if($customer_id > 0 && $balance){
				if($before_save_sale_info){
					...
					$store_account_transaction = array(
						...
						'balance' => 0, // update next
						'date' => isset($sales_data['sale_time']) ? $sales_data['sale_time'] : $before_save_sale_info->sale_time
					);

					$this->update_store_account($sale_id, 1, $store_account_payment, $store_account_transaction, 1);
				}else{
					//new
					$store_account_transaction = array(
						'customer_id' => $customer_id,
						...
						'transaction_amount' => $balance,
						'balance' => 0, // update next
						'date' => isset($sales_data['sale_time']) ? $sales_data['sale_time'] : date('Y-m-d H:i:s')
					);

					$this->update_store_account($sale_id, 1, $store_account_payment, $store_account_transaction);
				}
			} else if($customer_id > 0 && $store_account_payment) {
				if($before_save_sale_info){ // edit
					$store_account_transaction = array(
						'customer_id'=> $customer_id,
						'sale_id'=> $sale_id,
						'comment'=> $comment,
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			(
				...
				'paid_online' => $payment->paid_online ? '1' : '0',
			);
		...
		{			
			...
			if (property_exists($item,'item_id'))
			{
				...
				if ($item->is_recurring)
				{
					...
					array(
						...
						'location_id' => $cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id(),
						'payment_option' => $cart->selected_payment == lang('common_credit')? 1 : 2,
					);
				...
				if( $cur_item_info->non_profit_sale == 1){ // to set profit zero
					$cost_price = $item->unit_price;
				}
				...
				$line_item_commission = get_commission_for_item($cart,$item->item_id,$item->unit_price+$item->get_modifier_unit_total(),to_currency_no_money($cost_price+$item->get_modifier_cost_total(),10), $item->quantity, $item->discount, $item->sold_by_employee_id_2);
				...
				(
					...
					'flat_discount_amount'=>$item->flat_discount_amount,
					...
					'sold_by_employee_id_2' => $item->sold_by_employee_id_2,
				);

				for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
				{
					$sales_items_data["custom_field_${k}_value"] = $item->{"line_item_custom_field_${k}_value"};
				}
				...
				if (((!$cart->is_ecommerce && $suspended < 2) || ($this->config->item('import_ecommerce_orders_suspended') && $suspended < 2)))
				{
					...
					{ 
						...
						{												
							...
							$this->Giftcard->add_giftcard_balance($item->description,$item->unit_price*$item->quantity);
				...
				if (((!$cart->is_ecommerce && $suspended < 2) || ($this->config->item('import_ecommerce_orders_suspended') && $suspended < 2)) || (!$cart->is_ecommerce && $this->Sale_types->can_remove_quantity($suspended)) || ($cart->is_ecommerce && $this->config->item('import_ecommerce_orders_suspended')))
				{
					...
					{	
						...
						{			
							...
							{
								$message.="<br /><br />".lang("common_location").': '.$this->Location->get_info_for_key('name',$cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id());
							}
                                                       ...
                                                       if ($cur_item_info->main_image_id)
                                                       {
                                                               $image_url = secure_app_file_url($cur_item_info->main_image_id);
                                                               $this->email->message($message.'<br /><br /><img src="'.$image_url.'" style="max-width:200px;" />');
                                                       }
                                                       else
                                                       {
						...
						if (!$cur_item_info->is_service)
						{
							...
							$inv_data = array
							(
								'trans_date'=>$this->config->item('inventory_post_date_is_always_sale_date') && $sales_data['sale_time'] ? $sales_data['sale_time'] : date('Y-m-d H:i:s'),
					...
					else
					{
						...
						{			
							...
							if ($this->Location->count_all() > 1)
							{
								$message.="<br /><br />".lang("common_location").': '.$this->Location->get_info_for_key('name');
							}
			...
			else
			{
				...
				if( $cur_item_kit_info->non_profit_sale == 1){ // to set profit zero
					$cost_price = $item->unit_price;
				}
				...
				$line_item_commission = get_commission_for_item_kit($cart,$item->item_kit_id,$item->unit_price+$item->get_modifier_unit_total(),$cost_price === NULL ? 0.00 : to_currency_no_money($cost_price+$item->get_modifier_cost_total(),10), $item->quantity, $item->discount, $item->sold_by_employee_id_2);
				$sale_commission+=$line_item_commission;
				...
				for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
				{
					$sales_item_kits_data["custom_field_${k}_value"] = $item->{"line_item_custom_field_${k}_value"};
				}
		...
		if ($cart->create_invoice && $balance)
		{
			...
			$invoice_data = array(
				...
				'customer_id' 	=> $customer_id,
				'term_id' 		=> $term_id,
				'balance' 		=> $balance + $already_paid_store_account_amount,
				'total' 		=> $balance,
				'location_id' 	=> $cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id(),
		...
		if (!$cart->skip_webhook)
		{
			...
			elseif($this->config->item('edit_sale_web_hook') && !$is_new_sale)
			{
	...
	//action = 0: delete, 1: insert or edit, 2: undelete
	//override = 0: none (insert), override = 1 (edit)
	function update_store_account($sale_id, $action = 0, $store_account_payment = 0, $insert_record = NULL, $override = 0)
	{
		...
		if($action == 0){ // delete
			...
			$this->db->from('store_accounts');
			$this->db->where('store_accounts.state', 0);
			$this->db->where('sale_id', $sale_id);
			$store_account_records = $this->db->get()->result_array();

			if( count($store_account_records)>0 && $to_be_paid_result->num_rows()>=1 ){
				foreach($store_account_records as $store_account_record)
				{
					if($store_account_record['transaction_amount'])
					{
						//update customer balance
						$this->db->set('balance','balance-'.$store_account_record['transaction_amount'],false);
						$this->db->where('person_id', $customer_id);
						$this->db->update('customers');
					}
					...
					//update after logs
					$this->db->from('store_accounts');
					$this->db->where('store_accounts.state', 0); // normal
					$this->db->where('customer_id',$customer_id);
					$this->db->group_start();
						$this->db->group_start();
							$this->db->or_where("store_accounts.date = ", $store_account_record['date']);
							$this->db->where("store_accounts.sno > ", $store_account_record['sno']);
						$this->db->group_end();
						$this->db->or_where("store_accounts.date > ", $store_account_record['date']);
					$this->db->group_end();
					$this->db->where("store_accounts.sale_id !=", $sale_id);
					$after_logs = $this->db->get()->result_array();
					foreach($after_logs as $log){
						$log['balance'] = $log['balance'] - $store_account_record['transaction_amount'];
						$this->db->where('sno', $log['sno']);
						$this->db->update('store_accounts', $log);
					}

					$store_account_record['state'] = 1; // deleted status
					$this->db->where('sno', $store_account_record['sno']);
					$this->db->update('store_accounts', $store_account_record);
				}
			}
		} else if($action == 1){ //insert or edit
			$this->db->set('balance','balance+'.$insert_record['transaction_amount'],false);
			$this->db->where('person_id', $customer_id);
			$this->db->update('customers');

			//get the latest before log
			$this->db->reset_query();
			$this->db->from('store_accounts');
			$this->db->where('store_accounts.state', 0);
			$this->db->where('store_accounts.customer_id', $customer_id);
			$this->db->where("store_accounts.date <= ", $insert_record['date']);
			$this->db->order_by("store_accounts.date", "desc");
			$this->db->order_by("store_accounts.sno", "desc");
			$this->db->limit(1);

			$before_log = $this->db->get()->row_array();
			if($before_log){
				$insert_record['balance'] = $before_log['balance'] + $insert_record['transaction_amount'];
			}else{
				$insert_record['balance'] = $insert_record['transaction_amount'];
			}
			...
			foreach($after_logs as $log){
				$log['balance'] = $log['balance'] + $insert_record['transaction_amount'];
				$this->db->where('sno', $log['sno']);
				$this->db->update('store_accounts', $log);
			}

			if($override == 1){// edit mode
				$this->db->delete('store_accounts', array('sale_id' => $sale_id, 'state' => 1));
			}

			$this->db->insert('store_accounts', $insert_record);
		} else if($action == 2){ //undelete
			$this->db->reset_query();
			$this->db->from('store_accounts');
			$this->db->where('store_accounts.state', 1);// only get deleted state except overwritten state
			$this->db->where("store_accounts.sale_id", $sale_id);
			$this->db->order_by("store_accounts.date", "asc");
			$this->db->order_by("store_accounts.sno", "asc");

			$deleted_log_list = $this->db->get()->result_array();

			foreach($deleted_log_list as $delete_log){

				$this->db->set('balance','balance+'.$delete_log['transaction_amount'],false);
				$this->db->where('person_id', $customer_id);
				$this->db->update('customers');

				//get the latest before log
				$this->db->reset_query();
				$this->db->select('store_accounts.*');
				$this->db->from('store_accounts');
				$this->db->where('store_accounts.state', 0);
				$this->db->where('store_accounts.customer_id', $customer_id);
				...
				$this->db->group_start();
					$this->db->group_start();
						$this->db->where("store_accounts.date = ", $delete_log['date']);
						$this->db->where("store_accounts.sno < ", $delete_log['sno']);
					$this->db->group_end();
					$this->db->or_where("store_accounts.date < ", $delete_log['date']);
				$this->db->group_end();
				...
				$before_log = $this->db->get()->row_array();
				if($before_log){
					$delete_log['balance'] = $before_log['balance'] + $delete_log['transaction_amount'];
				}else{
 					$delete_log['balance'] = $delete_log['transaction_amount'];
				}
				...
				foreach($after_logs as $log){
					$log['balance'] = $log['balance'] + $delete_log['transaction_amount'];
					$this->db->where('sno', $log['sno']);
					$this->db->update('store_accounts', $log);
				}
				...
				$this->db->reset_query();
				$delete_log['state'] = 0;
				$this->db->where('sno', $delete_log['sno']);
				$this->db->update('store_accounts', $delete_log);
			}
		}
	}
	...
	function delete($sale_id, $all_data = false, $update_ecommerce=TRUE, $update_points = TRUE)
	{
		$sales_data = $this->get_info($sale_id)->row_array();
		$this->load->model('Sale_types');
		...
		{		
			...
			$this->update_store_account($sale_id, 0, $sale_info['store_account_payment']);
		...
		$this->db->where('sale_id', $sale_id);
		$ret_update = $this->db->update('sales', array('deleted' => 1,'deleted_by'=>$employee_id, 'last_modified' => date('Y-m-d H:i:s')));
		if($ret_update){
	...
	function undelete($sale_id)
	{
		...
		{		
			...
			$this->update_store_account($sale_id, 2, $sale_info['store_account_payment'], NULL);
		...
		$this->db->where('sale_id', $sale_id);
		$ret_update = $this->db->update('sales', array('deleted' => 0, 'deleted_by' => NULL,'last_modified' => date('Y-m-d H:i:s')));
		if($ret_update){
            $this->db->trans_commit();
		}else{
            $this->db->trans_rollback();
	...
	function get_recent_sales_for_customer($customer_id)
	{
       ...
       function get_sales_for_customer_by_items($item_id, $customer_id)
       {
               ...
               $this->db->select('sales.sale_time, sales.sale_id, sales_items.item_unit_price, sales_items.discount_percent, sales_items.flat_discount_amount')
                       ->from('sales')
    ...
    function get_sale_ids($start_date = false, $end_date = false, $customer_id = false, $suspended_type_ids = false, $start_payment_date = false, $end_payment_date = false, $include_created_sales_in_range = false, $start_date_created = false, $end_date_created = false, $start_date_updated = false, $end_date_updated = false, $location_id = NULL) 
	{
        ...
        if ($suspended_type_ids) {
            $this->db->where_in('sales.suspended', $suspended_type_ids);
        }

        if ($start_date) {
            $this->db->group_start();
			$this->db->where('sales.sale_time >=', $start_date);
            if ($include_created_sales_in_range) {
                $this->db->or_where('sales.created_at >=', $start_date);
            }
            $this->db->group_end();
		}
        ...
        if ($end_date) {
            ...
            if ($include_created_sales_in_range) {
                $this->db->or_where('sales.created_at <=', $end_date);
            }
            $this->db->group_end();
		}

        if ($start_payment_date) {
			$this->db->where('sales_payments.payment_date >=', $start_payment_date);
		}

		if ($end_payment_date) {
			$this->db->where('sales_payments.payment_date <=', $end_payment_date);
		}

        if ($start_date_created) {
			$this->db->where('sales.created_at >=', $start_date_created);
		}

		if ($end_date_created) {
			$this->db->where('sales.created_at <=', $end_date_created);
		}

        if ($start_date_updated) {
			$this->db->where('sales.updated_at >=', $start_date_updated);
		}

		if ($end_date_updated) {
			$this->db->where('sales.updated_at <=', $end_date_updated);
		}
	...
	function get_highest_payment_ref_no($sale_id)
	{
		$this->db->select('sale_id,ref_no');
		$this->db->from('sales_payments');
		$this->db->where('sale_id',$sale_id);
		$this->db->order_by('payment_amount', 'DESC');
		$this->db->limit(1);
	...
	function can_void_cc_sale($sale_id)
	{
		...
		elseif($this->Location->get_info_for_key('credit_card_processor') == 'valor')
		{
			$processor = 'valor';
		}
		...
		$this->db->from('sales_payments');
		$this->db->where('sale_id',$sale_id);
		$this->db->where_in('payment_type', array(lang('common_credit'),lang('sales_partial_credit'), lang('common_ebt'), lang('common_ebt_cash')));
	...
	function get_payment_options($cart)
	{
		...
		$customer_id=$cart?$cart->customer_id:NULL;
		...
		if ($cart && $cart->has_recurring_item())
		{
			$payment_options = array(
		...
		if ($this->Location->get_info_for_key('enable_credit_card_processing'))
		{
			$payment_options=array(
				...
				if($this->config->item('customers_store_accounts') && $cart && $cart->get_mode() != 'store_account_payment') 
				{
					$payment_options=array_merge($payment_options,	array(lang('common_store_account') => lang('common_store_account')		
					));
				}
				
				if ($this->config->item('show_debit_payment_option_when_using_integrated_credit_card_processing') && ($this->Location->get_info_for_key('credit_card_processor') == 'coreclear2' || $this->Location->get_info_for_key('credit_card_processor') == 'valor'))
				{
					$payment_options=array_merge($payment_options,	array(lang('common_debit') => lang('common_debit')	
					));
				}
		...
		else
		{
			$payment_options=array(
				...
				if($this->config->item('customers_store_accounts') && ($cart ? $cart->get_mode() != 'store_account_payment' : ($cart == NULL) )) 
				{
					$payment_options=array_merge($payment_options,	array(lang('common_store_account') => lang('common_store_account')		
					));
				}
	...
	function get_line_item_custom_field($number,$key="name")
	{
		static $config_data;
		
		if (!$config_data)
		{
			$config_data = $this->config->item('sale_line_item_custom_field_prefs') ? unserialize($this->config->item('sale_line_item_custom_field_prefs')) : array();
		}
		
		return isset($config_data["custom_field_${number}_${key}"]) && $config_data["custom_field_${number}_${key}"] ? $config_data["custom_field_${number}_${key}"] : FALSE;
	}

	function get_line_item_custom_field_count(){
		$count_line_item_custom_field = 0;
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) {
			$custom_field = $this->get_line_item_custom_field($k);
			if($custom_field){
				$count_line_item_custom_field ++;
			}
		}
		return $count_line_item_custom_field;
	}
	...
	function add_sale_item($sale_id,$item_id,$quantity=1,$serialnumber=null,$is_repair_item=false, $item_description = ''){
		...
		if(!$customer_id || $customer_info->taxable){
			$tax = $this->get_taxes_for_sale_item($item_id,$regular_price,$quantity);
		} else{
			$tax = 0;
		}
		...
		$sales_item_data = array(
			...
			'description'=>$item_description ? $item_description : $item_info->description,
    ...
    function get_sales_totals_for_item_range($item_id, $start_date, $end_date, $item_variation_id = false, $location_ids = NULL)
    {
                if($location_ids === NULL)
                {
                        $location_ids = array($this->Employee->get_logged_in_employee_current_location_id());
                }
		 
            $this->db->select('SUM(quantity_purchased * IFNULL(unit_quantity,1)) as qty, SUM(phppos_sales_items.total) as amount', false);
            $this->db->from('sales_items');
            $this->db->join('sales','sales_items.sale_id = sales.sale_id');
            $this->db->where('sales_items.item_id',$item_id);
            if($item_variation_id)
            {
                    $this->db->where('sales_items.item_variation_id',$item_variation_id);
            }
            $this->db->where('sales.deleted',0);
            $this->db->where('sales.suspended',0);
            ...
            $row = $this->db->get()->row_array();
            return array('qty' => $row && $row['qty'] ? $row['qty'] : 0,
                         'amount' => to_currency_no_money($row && $row['amount'] ? $row['amount'] : 0));
    }

    function get_sales_totals_by_month($item_id, $start_month, $item_variation_id = false, $location_ids = NULL)
    {
                if($location_ids === NULL)
                {
                        $location_ids = array($this->Employee->get_logged_in_employee_current_location_id());
            ...
            $this->db->select("DATE_FORMAT(phppos_sales.sale_time,'%Y-%m') as sale_month, SUM(quantity_purchased * IFNULL(unit_quantity,1)) as qty, SUM(phppos_sales_items.total) as amount", false);
            $this->db->from('sales_items');
            $this->db->join('sales','sales_items.sale_id = sales.sale_id');
            $this->db->where('sales_items.item_id',$item_id);
            if($item_variation_id)
            ...
            $this->db->where('sales.suspended',0);
            $this->db->where('sales_items.quantity_purchased >',0);
            $this->db->where('sales.sale_time >=',$start_month);
            $this->db->group_by('sale_month');
            $this->db->order_by('sale_month');
            $this->db->where_in('sales.location_id',$location_ids);
            $rows = $this->db->get()->result_array();
            $data = array();
            foreach($rows as $row)
            {
                    $data[$row['sale_month']] = array('qty' => to_quantity($row['qty']), 'amount' => to_currency_no_money($row['amount']));
            }
            return $data;
    }
	...
	function sale_item_discount_update($sale_id, $item_id, $line=0, $discount="", $is_item_kit = false, $discount_type = 'discount_percent'){
		if($is_item_kit) {
		...
		if($sale_item){
			...
			{
				$sales_item_data['flat_discount_amount'] = 0;
			}
			else
			{
				$sales_item_data['discount_percent'] = 0;
			}
			$this->db->where('sale_id', $sale_id);
	...
	*/
	...
	Gets id of item with given ecommerce_product_id
	*/
	function get_sale_deleted_for_ecommerce_order_id($ecommerce_order_id)
	{
		$this->db->from('sales');
		$this->db->where('ecommerce_order_id', $ecommerce_order_id);
		$result = $this->db->get();
	...
	function get_processor_payment_link($sale_id)
	{
		$this->load->helper('text');
		$encrypted_sale_id = do_encrypt($sale_id, $this->Appconfig->get_secure_key());

		$store_url_base = site_url();
		return $store_url_base.'/s/'.$encrypted_sale_id;
	}

        function send_layaway_payment_email_alert($sale_id, $payment_amount, $balance)
        {
                $sale_info = $this->get_info($sale_id)->row();
                $loc_info = $this->Location->get_info($sale_info->location_id);
                $customer_info = $this->Customer->get_info($sale_info->customer_id);
		$customer_name = $customer_info->first_name . ' ' . $customer_info->last_name;
		
		$subject = $customer_name . ' '.lang('sales_has_made').' ' . to_currency($payment_amount) . ' '.lang('sales_layaway_paymeent');
		$message = $customer_name . ' '.lang('sales_has_made_payment_of').' ' . to_currency($payment_amount) . ' '.lang('sales_towards_sale').' ' . $sale_id . '. '.lang('sales_balance_of_sale').' ' . to_currency($balance);
		
		$this->load->library('email');
		$this->email->from($loc_info->email, $loc_info->company ? $loc_info->company : $this->config->item('company'));
		$this->email->to($loc_info->email);
       ...
       /**
       * Get lifetime total amount spent by a customer
       *
       * @param int $customer_id
       * @return float
       */
       function get_customer_total_spent($customer_id)
       {
               ...
               return $row && $row->total_spent ? (float)$row->total_spent : 0.0;
       }
}
?>
