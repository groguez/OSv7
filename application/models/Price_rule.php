...
class Price_rule extends MY_Model
{
	...
	*/
	...
	/**
	 * Retrieves the best applicable price rule for an item or item kit.
	 *
	 * Check mix-and-match first, then item-specific rules.
	 *
	 * @param array $params {
	 *   @type array $coupons Array of coupons (optional).
	 *   @type object $item_kit Item kit object (optional).
	 *   @type object $item Item object (optional).
	 *   @type int    $quantity Quantity for evaluation.
	 * }
	 *
	 * @return array|null|false Returns the best matching price rule or null/false if no match.
	 */
	function get_price_rule_for_item(array $params = [])
	{
		if (!isset($params['coupons'])) {
			$params['coupons'] = [];
		}

		$coupon_rule_ids = array_map([$this, 'coupons_to_rule_id'], $params['coupons']);

		$best_rule         = null;
		$mix_and_match_rule = null;

		if (isset($params['item_kit'], $params['item_kit']->item_kit_id) && $params['item_kit']->item_kit_id > 0) {
		
			// Item kit–specific rule
			$rule = $this->get_rule_for_item_kit($params['item_kit']->item_kit_id, $params['quantity'], $coupon_rule_ids);
			if (!empty($rule['rule_item_kit']) && !$this->is_rule_excluded_by_tier($rule)) {
				$rule['mix_and_match'] = 0;
				$best_rule = $rule;
			}

			// Category-specific rule for item kits
			$rule = $this->get_rule_for_itemkit_category($params['item_kit']->item_kit_id, $params['quantity'], $coupon_rule_ids);
			if (!empty($rule['rule_item_kit_cat']) && !$this->is_rule_excluded_by_tier($rule)) {
				$target_field = ($rule['mix_and_match'] == 1) ? 'mix_and_match_rule' : 'best_rule';
				$rule['mix_and_match'] = 0;
				$$target_field = $rule;
			}

			// Tag-specific rule for item kits
			$rule = $this->get_rule_for_itemkit_tags($params['item_kit']->item_kit_id, $params['quantity'], $coupon_rule_ids);
			if (!empty($rule['rule_item_kit_tags']) && !$this->is_rule_excluded_by_tier($rule)) {
				$rule['mix_and_match'] = 0;
				$best_rule = $rule;
			}

			// Manufacturer-specific rule for item kits
			$rule = $this->get_rule_for_itemkit_manufacturers($params['item_kit']->item_kit_id, $params['quantity'], $coupon_rule_ids);
			if (!empty($rule['rule_item_kit_manu']) && !$this->is_rule_excluded_by_tier($rule)) {
				$rule['mix_and_match'] = 0;
				$best_rule = $rule;
			}

			// Mix-and-match rule for items
			...
			if ($rule && !$this->is_rule_excluded_by_tier($rule)) {
				$rule['mix_and_match'] = 1;
				$mix_and_match_rule = $rule;
			}
			if ($mix_and_match_rule && (!$best_rule)) {
				return $mix_and_match_rule;
			} elseif ($best_rule) {
				return $best_rule;
			}
		...
		if (!empty($rule['rule_item']) && !$this->is_rule_excluded_by_tier($rule)) {
			$rule['mix_and_match'] = 0;
			$best_rule = $rule;
		}
		...
		if (!empty($rule['rule_item_cat']) && !$this->is_rule_excluded_by_tier($rule)) {
			$target_field = ($rule['mix_and_match'] == 1) ? 'mix_and_match_rule' : 'best_rule';
			$rule['mix_and_match'] = 0;
			$$target_field = $rule;
		}
		...
		if (!empty($rule['rule_item_tags']) && !$this->is_rule_excluded_by_tier($rule)) {
			$rule['mix_and_match'] = 0;
			$best_rule = $rule;
		}
		...
		if (!empty($rule['rule_item_manu']) && !$this->is_rule_excluded_by_tier($rule)) {
			$rule['mix_and_match'] = 0;
			$best_rule = $rule;
		}
