...
class Detailed_receivings extends Report
{
	...
	function get_details_data_column_recv()
	{
		return array(
			...
			array('data'=>lang('common_unit_price'), 'align'=>'right'),  
			array('data'=>lang('common_cost_price'), 'align'=>'right'),  
		  ...
		  for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
			...
			if($export_excel == 1)
			{
				foreach($report_data['details'][$key] as $drow)
				{
					...
					array('data'=>$drow['rid'], 'align'=> 'left'),
					...
					array('data'=>to_quantity($drow['current_selling_price']), 'align'=> 'left'), 
					array('data'=>to_quantity($drow['item_unit_price']), 'align'=> 'left'), 
	...
	function get_report_details_recv($ids, $export_excel=0)
	{
		...
		foreach($res as $key=>$drow)
			{	
				...
				$details_data_row[] = array('data'=>$drow['rid'], 'align'=>'left');
				...
				$details_data_row[] = array('data'=>to_currency($drow['current_selling_price']), 'align'=> 'right');
