...
class Closeout extends Report
{
	...
	public function getData()
	{
		...
		$query1 = $this->db->query('select category_id, category,sum(subtotal) as subtotal,sum(total) as total from  ( ' .$qry3." UNION ".$qry4. ') as alias group by category_id order by category desc');
		...
		$query2 = $this->db->query('select category_id, category,sum(subtotal) as subtotal,sum(total) as total from  ( ' .$qry7." UNION ".$qry8. ') as alias group by category_id order by category desc');
		...
		$query3 = $this->db->query('select category_id, category,sum(subtotal) as subtotal,sum(total) as total from  ( ' .$qry9." UNION ".$qry10. ') as alias group by category_id order by category desc');
		...
		$this->db->select('CONCAT("$", FORMAT(flat_discount_amount, 2)) as discount, count(*) as summary', false);
		$this->db->from('sales_items');
		$this->db->join('sales', 'sales.sale_id = sales_items.sale_id');
		$this->sale_time_where();
		$this->db->where('flat_discount_amount > 0');
		$this->db->where('sales.deleted', 0);
		$this->db->group_by('sales_items.flat_discount_amount');
		...
		$this->db->where('flat_discount_amount > 0');
		$this->db->where('sales.deleted', 0);
		$this->db->group_by('sales_item_kits.flat_discount_amount');
		
		$qry4=$this->db->get_compiled_select();
		
		$query = $this->db->query($qry1." UNION ".$qry2." UNION ".$qry3." UNION ".$qry4. "order by discount desc");
		...
		$this->db->select('SUM(flat_discount_amount * quantity_purchased) as discount_total');
		$this->db->from('sales_items');
		$this->db->join('sales', 'sales.sale_id = sales_items.sale_id');
		$this->db->join('items', 'sales_items.item_id = items.item_id');
		$this->sale_time_where();
		$this->db->group_start();
		$this->db->where_in('items.name', $this->discount_langs);
		$this->db->or_where('flat_discount_amount > 0');
		$this->db->group_end();
		$this->db->where('sales.deleted', 0);
