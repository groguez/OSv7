...
class Store_account_statements extends Report
{
	...
	{		
		...
		if ($this->settings['display'] == 'tabular')
		{
			...
			$input_params[] = array('view' => 'checkbox','checkbox_label' => lang('reports_show_manual_adjustments'), 'checkbox_name' => 'show_manual_adjustments');
	...
	public function getData()
	{
		...
		foreach($customer_ids_for_report as $customer_id)
		{
			...
			$this->db->where('store_accounts.transaction_amount != 0');
			...
			if (isset($this->params['show_manual_adjustments']))
			{
				$this->db->or_where('sales.deleted IS NULL');
			}
	...
	public function getSummaryData()
	{
		...
		foreach($customer_ids_for_report as $customer_id)
		{
			...
			if (count($result) == 0)
			{
				...
				$this->db->join('sales', 'sales.sale_id = store_accounts.sale_id', 'left');
				$this->db->join('locations', 'sales.location_id = locations.location_id', 'left');
				$this->db->order_by('store_accounts.date');		
