...
class Detailed_payments extends Report
{
	...
	public function getDataColumns()
	{
		...
		$return['summary'][] = array('data'=>lang('common_paid_online'), 'align'=> 'left');
		...
		$return['details'][] = array('data'=>lang('common_paid_online'), 'align'=> 'left');
	...
	public function getOutputData()
	{
		...
		{			
			foreach($row as $payment_type => $payment_data_row)
			{
				...
				$summary_data_row[] = array('data'=>boolean_as_string($payment_data_row['paid_online']), 'align'=>'left');
		...
		foreach($report_data['details']['sale_ids'] as $sale_id => $drows)
		{
			...
			foreach(array_keys($payment_types) as $payment_type)
			{
				foreach ($drows as $drow)
				{
					...
					$details_data_row[] = array('data'=>boolean_as_string($drow['paid_online']), 'align'=>'right');
	...
	public function getData()
	{
		...
		$this->db->select('sales.sale_time, sales_payments.sale_id, sales_payments.payment_type, payment_amount, payment_date, payment_id, phppos_sales_payments.paid_online as paid_online', false);
