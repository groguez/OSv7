...
abstract class Report extends MY_Model 
{
	...
	protected function setupDefaultPagination($total_row = 0)
	{
		$config = array();
		...
		$config['total_rows'] = $total_row? $total_row : $this->getTotalRows();
	...
	public static function get_selected_payment_types()
	{
		$CI =& get_instance();
		
		if ($CI->input->get('payment_types'))
		{
			return $CI->input->get('payment_types');
		}
		else
		{
			$payment_types = array(lang('common_cash'),lang('common_check'), lang('common_giftcard'),lang('common_debit'),lang('common_credit'));
			
			if($CI->config->item('suppliers_store_accounts'))
			{
				$payment_types[] =lang('common_store_account');
			}
			
			foreach($CI->Appconfig->get_additional_payment_types() as $additional_payment_type)
			{
				$payment_types[] = $additional_payment_type;
			}
	...
	public static function get_payment_types_to_use()
	{
		$CI =& get_instance();
		$payment_types = array(lang('common_cash') => lang('common_cash'),lang('common_check') => lang('common_check'), lang('common_giftcard') => lang('common_giftcard'),lang('common_debit') => lang('common_debit'),lang('common_credit') => lang('common_credit'));
		
		if($CI->config->item('suppliers_store_accounts'))
		{
			$payment_types[lang('common_store_account')] =lang('common_store_account');
		}
		...
		'detailed_sales' => array(
			...
			'summary_sales_time' => array(
 			  ...
 			  'graphical_summary_categories' => array(
				...
				'detailed_new_customers' => array(
					'model' => 'Detailed_new_customers',
					'settings' => array(
						'permission_action' => 'view_customers',
						'display' => 'tabular'
					),
 	  ...
 	  'detailed_expenses' => array(
 		 ...
 		 'settings' => array(
			...
			'giftcard_audit' => array(
			  ...
			  'detailed_giftcards' => array(
 			  ...
 			  'inventory_aging' => array(
 				 'model' => 'Inventory_aging',
 				 'settings' => array(
 				 	'permission_action' => 'view_inventory_reports',
 				 	'display' => 'tabular'
 				 	),
		  ...
		  'summary_item_kits_variance' => array(
		   ...
		   'top_sellers' => array(
			...
			'best_selling_attribute' => array(
				'model' => 'Summary_items_best_selling_attribute',
				'settings' => array(
					'permission_action' => 'view_items',
					'display' => 'tabular'
				),
 		  ...
 		  'summary_manufacturers' => array(
  		  ...
  		  'graphical_summary_payments' => array(
   		  ...
   		  'detailed_payments' => array(
				...
				'store_account_payments' => array(
					'model' => 'Store_account_payments',
					'settings' => array(
						'permission_action' => 'view_store_account',
						'display' => 'tabular'
					),
										 	...
										 	'item_kit_price_history' => array(
												...
												'summary_journal' =>  array(
			  						  		  ...
			  						  		  'customer_invoices' => array(
							 						...
							 						'tax_by_payments_received' => array(
							 							'model' => 'Tax_by_payments_received',
							 							'settings' => array(
							 								'permission_action' => 'view_taxes',
							 								'display' => 'tabular'
							 								),
