...
class Summary_categories extends Report
{
	...
	public function getInputData()
	{
		...
		$specific_entity_data2['specific_input_name'] = 'supplier_id';
		$specific_entity_data2['specific_input_label'] = lang('reports_supplier');
		$specific_entity_data2['search_suggestion_url'] = site_url('reports/supplier_search');
		$specific_entity_data2['view'] = 'specific_entity';
	...
	function getOutputData()
	{
		...
		if (isset($this->params['supplier_id']) && $this->params['supplier_id']){
			$inventory_totals_by_category = $this->getInventoryTotalsCategories($this->params['supplier_id']);
		}else{
		...
		{				
			...
			foreach($report_data as $row)
			{
				...
				if(isset($inventory_totals_by_category[$row['category_id']])){
					...
					$data_row[] = array('data'=>to_quantity(null), 'align' => 'right');
					$data_row[] = array('data'=>to_currency(null), 'align' => 'right');
					$data_row[] = array('data'=>to_currency(null), 'align' => 'right');
				}
	...
	{	
		$this->db->select('SUM(item_unit_price * quantity_purchased*(discount_percent/100)) + flat_discount_amount * quantity_purchased as discount_total,items.category_id, categories.name as category , sum('.$this->db->dbprefix('sales_items').'.subtotal) as subtotal, sum('.$this->db->dbprefix('sales_items').'.total) as total, sum('.$this->db->dbprefix('sales_items').'.tax) as tax, sum('.$this->db->dbprefix('sales_items').'.profit) as profit, sum('.$this->db->dbprefix('sales_items').'.quantity_purchased) as item_sold', false);
	...
	function getInventoryTotalsCategories($supplier_id = '')
	{
    	...
    	$query = 'select
            ...
            from (
              ...
              FROM phppos_items
			  JOIN phppos_categories ON phppos_categories.id = phppos_items.category_id
              left join (
              ...
              left JOIN (
               ...
               where
                 ...
                 '.($supplier_id ? 'and phppos_items.supplier_id = '.$supplier_id : '').'
					       and is_service != 1 and phppos_categories.deleted=0
