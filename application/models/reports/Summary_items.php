...
class Summary_items extends Report
{
	...
	private function get_items_with_sales_query($paginate,$only_item_id = FALSE, $cal_total = TRUE)
	{
		...
		{
			if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
				$this->db->group_by('sales.location_id, sales_items.item_variation_id,items.item_id');
			}else{
		...
		{
			if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
				$this->db->group_by('sales.location_id, sales_items.item_id');
			}else{
	...
	private function get_item_kits_with_sales_query($paginate,$only_item_kit_id = FALSE, $cal_total = TRUE)
	{
		$location_ids = self::get_selected_location_ids();
		$location_ids_string = implode(',',$location_ids);
	...
	private function get_items_without_sales($paginate = TRUE, $cal = true)
	{
		if ($this->params['category_id'])
		{
			if ($this->config->item('include_child_categories_when_searching_or_reporting'))
		...
		$items_with_sales_query = $this->get_items_with_sales_query(FALSE,TRUE, FALSE)->get_compiled_select();
	...
	private function get_item_kits_without_sales($paginate = TRUE, $cal = TRUE)
	{
		if ($this->params['category_id'])
		{
			if ($this->config->item('include_child_categories_when_searching_or_reporting'))
		...
		$item_kits_with_sales_query = $this->get_item_kits_with_sales_query(FALSE,TRUE,$cal)->get_compiled_select();
	...
	{		
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_with_sales')){
			if (isset($this->params['include_item_kits']) && $this->params['include_item_kits'])
			{
				$items_sales_query = $this->get_items_with_sales_query(FALSE, FALSE, FALSE)->get_compiled_select();	
				$items_kits_sales_query = $this->get_item_kits_with_sales_query(FALSE, FALSE, FALSE)->get_compiled_select();
			...
			$total_row = $this->db->query("SELECT FOUND_ROWS() AS total_count;")->row()->total_count;
			
			if ($total_row)
			{
				$this->setupDefaultPagination($total_row);
			}
		}
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_without_sales'))
		{
			if (isset($this->params['include_item_kits']) && $this->params['include_item_kits'])
			{
				$items_sales_query = $this->get_items_without_sales(FALSE, FALSE)->get_compiled_select();	
				$items_kits_sales_query = $this->get_item_kits_without_sales(FALSE, FALSE)->get_compiled_select();
		...
		$quantities_item_variation_indexed_by_id = array();
		$this->db->from('items');
		$quantity_result_item_variation = array();
		$this->db->select('item_variations.id as item_variation_id, item_variations.item_id as item_id, SUM(quantity) as quantity, locations.location_id as location_id, locations.name as location_name', FALSE);
		$this->db->join('item_variations','item_variations.item_id = items.item_id', 'left');
		...
		$this->db->join('locations', 'locations.location_id = location_item_variations.location_id', 'left');

		if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
			if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
				$this->db->group_by('location_item_variations.location_id, item_variations.id');
			} else {
				$this->db->group_by('location_item_variations.location_id, item_variations.item_id');
			}
		}else{
			if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
				$this->db->group_by('item_variations.id');
			} else {
				$this->db->group_by('item_variations.item_id');
			}
		}

		$quantity_result_item_variation = $this->db->get()->result_array();

		foreach($quantity_result_item_variation as $quan_row)
		{
			if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
				if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
					$quantities_item_variation_indexed_by_id[$quan_row['location_id'].'_'.$quan_row['item_variation_id']] = array(
						'quantity' => $quan_row['quantity'],
						'location_id' => $quan_row['location_id'],
						'location_name' => $quan_row['location_name']
					);
				}else{
					$quantities_item_variation_indexed_by_id[$quan_row['location_id'].'_'.$quan_row['item_id']] = array(
						'quantity' => $quan_row['quantity'],
						'location_id' => $quan_row['location_id'],
						'location_name' => $quan_row['location_name']
					);
				...
				if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
					$quantities_item_variation_indexed_by_id[$quan_row['item_variation_id']] = array(
						'quantity' => $quan_row['quantity'],
						'location_id' => $quan_row['location_id'],
						'location_name' => $quan_row['location_name']
					);
				} else {
					$quantities_item_variation_indexed_by_id[$quan_row['item_id']] = array(
						'quantity' => $quan_row['quantity'],
						'location_id' => $quan_row['location_id'],
						'location_name' => $quan_row['location_name']
					);
		...
		$quantities_item_indexed_by_id = array();
		$this->db->from('items');
		$this->db->select('items.item_id as item_id, SUM(quantity) as quantity, locations.location_id as location_id, locations.name as location_name', FALSE);
		...
		if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
			$this->db->group_by('location_items.location_id, items.item_id');
		}else{
			$this->db->group_by('items.item_id');
		}
		$quantity_result_item = $this->db->get()->result_array();
		foreach($quantity_result_item as $quan_row)
		{
			if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
				$quantities_item_indexed_by_id[$quan_row['location_id'].'_'.$quan_row['item_id']] = array(
					'quantity' => $quan_row['quantity'],
					'location_id' => $quan_row['location_id'],
					'location_name' => $quan_row['location_name']
				);
			} else {
				$quantities_item_indexed_by_id[$quan_row['item_id']] = array(
					'quantity' => $quan_row['quantity'],
					'location_id' => $quan_row['location_id'],
					'location_name' => $quan_row['location_name']
				);
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_with_sales')){
			...
			{
				if(isset($items_sales_data[$k]['item_variation_id']) && $items_sales_data[$k]['item_variation_id'] > 0){
					if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
						if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
							$q_i_key = $items_sales_data[$k]['location_id'].'_'.$items_sales_data[$k]['item_variation_id'];
							$items_sales_data[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$q_i_key]['quantity'];
							$items_sales_data[$k]['location_name'] = $quantities_item_variation_indexed_by_id[$q_i_key]['location_name'];
						}else{
							$q_i_key = $items_sales_data[$k]['location_id'].'_'.$items_sales_data[$k]['item_id'];
							$items_sales_data[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$q_i_key]['quantity'];
							$items_sales_data[$k]['location_name'] = $quantities_item_variation_indexed_by_id[$q_i_key]['location_name'];
						}
						...
						if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
							$items_sales_data[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$items_sales_data[$k]['item_variation_id']]['quantity'];
						}else{
							$items_sales_data[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$items_sales_data[$k]['item_id']]['quantity'];
						}
					}
				}else{
					if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
						$items_sales_data[$k]['quantity'] = $quantities_item_indexed_by_id[$items_sales_data[$k]['location_id'].'_'.$items_sales_data[$k]['item_id']]['quantity'];
					}else{
						$items_sales_data[$k]['quantity'] = $quantities_item_indexed_by_id[$items_sales_data[$k]['item_id']]['quantity'];
					}
				}
			}
		}
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_without_sales'))
		{
			...
			$location_list = array();
			$this->db->select('locations.location_id, locations.name', FALSE);
			$this->db->from('locations');
			$this->db->where('location_id IN('.$location_ids_string.')');
			$location_info_ret = $this->db->get()->result_array();
			foreach($location_info_ret as $location_info){
				$location_list[$location_info['location_id']] = $location_info['name'];
			}

			$location_separately_report_temp = array();
			for($k=0;$k<count($items_in_report_without_sales);$k++)
			{
				if(isset($items_in_report_without_sales[$k]['item_variation_id']) && $items_in_report_without_sales[$k]['item_variation_id'] > 0){
					if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
						foreach($location_ids as $k2=>$s_location_id){
							$q_iv_key = $s_location_id.'_'.$items_in_report_without_sales[$k]['item_variation_id'];
							if(isset($quantities_item_variation_indexed_by_id[$q_iv_key])){
								$items_in_report_without_sales[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$q_iv_key]['quantity'];
								$items_in_report_without_sales[$k]['location_name'] = $quantities_item_variation_indexed_by_id[$q_iv_key]['location_name'];
							}else{
								$items_in_report_without_sales[$k]['quantity'] = null;
								$items_in_report_without_sales[$k]['location_name'] =$location_list[$s_location_id];
							}
							array_push($location_separately_report_temp, array_merge([], $items_in_report_without_sales[$k]));
						}
					} else {
						$items_in_report_without_sales[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$items_in_report_without_sales[$k]['item_variation_id']]['quantity'];
						$items_in_report_without_sales[$k]['location_name'] = $quantities_item_variation_indexed_by_id[$items_in_report_without_sales[$k]['item_variation_id']]['location_name'];
					}
				}else{
					if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
						foreach($location_ids as $s_location_id){
							$q_i_key = $s_location_id.'_'.$items_in_report_without_sales[$k]['item_id'];
							if(isset($quantities_item_variation_indexed_by_id[$q_i_key])){
								$items_in_report_without_sales[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$q_i_key]['quantity'];
								$items_in_report_without_sales[$k]['location_name'] = $quantities_item_variation_indexed_by_id[$q_i_key]['location_name'];
								array_push($location_separately_report_temp, array_merge([], $items_in_report_without_sales[$k]));
							}else if(isset($quantities_item_indexed_by_id[$q_i_key])){
								$items_in_report_without_sales[$k]['quantity'] = $quantities_item_indexed_by_id[$q_i_key]['quantity'];
								$items_in_report_without_sales[$k]['location_name'] = $quantities_item_indexed_by_id[$q_i_key]['location_name'];
								array_push($location_separately_report_temp, array_merge([], $items_in_report_without_sales[$k]));
							}
						}
					} else {
						$q_i_key = $items_in_report_without_sales[$k]['item_id'];
						if(isset($quantities_item_variation_indexed_by_id[$q_i_key])){
							$items_in_report_without_sales[$k]['quantity'] = $quantities_item_variation_indexed_by_id[$items_in_report_without_sales[$k]['item_id']]['quantity'];
							$items_in_report_without_sales[$k]['location_name'] = $quantities_item_variation_indexed_by_id[$items_in_report_without_sales[$k]['item_id']]['location_name'];
						}else if(isset($quantities_item_indexed_by_id[$q_i_key])){
							$items_in_report_without_sales[$k]['quantity'] = $quantities_item_indexed_by_id[$items_in_report_without_sales[$k]['item_id']]['quantity'];
							$items_in_report_without_sales[$k]['location_name'] = $quantities_item_indexed_by_id[$items_in_report_without_sales[$k]['item_id']]['location_name'];
						}
					}
				}
			}
			...
			if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately']){
				$items_in_report_without_sales = $location_separately_report_temp;
			}
		}
