...
class Inventory_summary extends Report
{
	...
	public function getOutputData()
	{
      ...
      $details_data_row[] = array('data'=>to_currency($drow['unit_price']), 'align'=> 'left');
		...
		foreach(array_keys($details_data) as $details_data_key)
		{
			if (isset($this->params['list_each_location_separately']) && $this->params['list_each_location_separately'])
			{
				list($item_id,$location_id) = explode('|',$details_data_key);
			}
			else
			{
				$item_id = $details_data_key;
			}
			$item_quantity = 0;
			...
			for($k=0;$k<count($details_data[$details_data_key]);$k++)
			{
				$item_quantity+=$details_quantity_data[$details_data_key][$k]['quantity'];
				$item_pending_inventory+=$details_quantity_data[$details_data_key][$k]['pending_inventory'];
				$item_reorder_level+=$details_quantity_data[$details_data_key][$k]['reorder_level'];
				$item_replenish_level+=$details_quantity_data[$details_data_key][$k]['replenish_level'];
			}
			
			$variation_quantity_summary_data[$details_data_key] = array(
