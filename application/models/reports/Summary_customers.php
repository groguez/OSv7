...
class Summary_customers extends Report
{
	...
	public function getInputData()
	{
		...
		$specific_entity_data_category['specific_input_name'] = 'category_id';
		$specific_entity_data_category['specific_input_label'] = lang('common_category');
		$specific_entity_data_category['search_suggestion_url'] = site_url('reports/category_search');
		$specific_entity_data_category['view'] = 'specific_entity';

		$specific_entity_data_supplier['specific_input_name'] = 'supplier_id';
		$specific_entity_data_supplier['specific_input_label'] = lang('reports_supplier');
		$specific_entity_data_supplier['search_suggestion_url'] = site_url('reports/supplier_search');
		$specific_entity_data_supplier['view'] = 'specific_entity';
		...
		if ($this->settings['display'] == 'tabular')
		{
			...
			$input_params = array(
				...
				$specific_entity_data_category,
				$specific_entity_data,
				$specific_entity_data_supplier,
	...
	{
		$subquery_item_kit_items_total = null;
		if(isset($this->params['category_id']) && $this->params['category_id'] 
		|| isset($this->params['supplier_id']) && $this->params['supplier_id']){

			$this->db->select($this->db->dbprefix('item_kit_items').'.item_kit_id');
			$this->db->select('SUM('.$this->db->dbprefix('items').'.cost_price * '.$this->db->dbprefix('item_kit_items').'.quantity) AS iki_total_cost_price', FALSE);
			$this->db->select('SUM('.$this->db->dbprefix('items').'.unit_price * '.$this->db->dbprefix('item_kit_items').'.quantity) AS iki_total_unit_price', FALSE);
			$this->db->select('SUM(('.$this->db->dbprefix('items').'.unit_price - '.$this->db->dbprefix('items').'.cost_price) * '.$this->db->dbprefix('item_kit_items').'.quantity) AS iki_total_profit', FALSE);
			$this->db->from('item_kit_items');
			$this->db->join('items', 'item_kit_items.item_id = items.item_id');
			$this->db->group_by('item_kit_items.item_kit_id');
			$subquery_item_kit_items_total = $this->db->get_compiled_select();

			$this->db->select('
				COUNT(DISTINCT '.$this->db->dbprefix('sales').'.sale_id) as count,
				customers.current_sales_for_discount,
				...
				people.city,
				(
					IFNULL(SUM('.$this->db->dbprefix('sales_items').'.subtotal), 0) 
					+ IFNULL( SUM('.$this->db->dbprefix('sales_item_kits').'.subtotal * ( k_items.unit_price / iki_total_unit_price)), 0)
				) as subtotal,
				(
					IFNULL(SUM('.$this->db->dbprefix('sales_items').'.total), 0)
					+ IFNULL(SUM('.$this->db->dbprefix('sales_item_kits').'.total * ( k_items.unit_price / iki_total_unit_price )), 0)
				) as total,
				(
					IFNULL(SUM('.$this->db->dbprefix('sales_items').'.tax), 0)
					+ IFNULL(SUM('.$this->db->dbprefix('sales_item_kits').'.tax * ( k_items.unit_price / iki_total_unit_price )), 0)
				) as tax,
				(
					IFNULL(SUM('.$this->db->dbprefix('sales_items').'.profit), 0)
					+ IFNULL(SUM('.$this->db->dbprefix('sales_item_kits').'.profit * (( k_items.unit_price - k_items.cost_price) / iki_total_profit )), 0)
				) as profit,
				(
					IFNULL(SUM('.$this->db->dbprefix('sales_items').'.quantity_purchased), 0)
					+ IFNULL(SUM('.$this->db->dbprefix('sales_item_kits').'.quantity_purchased * '.$this->db->dbprefix('item_kit_items').'.quantity ), 0)
				) as total_quantity_purchased', false);
		}else{
			$this->db->select('
				COUNT(*) as count,
				customers.current_sales_for_discount,
				...
				sum(total_quantity_purchased) as total_quantity_purchased', false);
		}
		...
		|| isset($this->params['supplier_id']) && $this->params['supplier_id'])
		{
			...
			$this->db->join('item_kit_items', 'item_kit_items.item_kit_id = sales_item_kits.item_kit_id', 'left');
			$this->db->join('items as k_items', 'k_items.item_id = item_kit_items.item_id', 'left');
			$this->db->join("($subquery_item_kit_items_total) as total_ikit", "total_ikit.item_kit_id = sales_item_kits.item_kit_id", 'left');
		}
		if(isset($this->params['category_id']) && $this->params['category_id'])
		{
			$this->db->group_start();
			$this->db->where('items.category_id', $this->params['category_id']);
			$this->db->or_where('k_items.category_id', $this->params['category_id']);
			$this->db->group_end();
		}
		if(isset($this->params['supplier_id']) && $this->params['supplier_id'])
		{
			...
			$this->db->or_where('k_items.supplier_id', $this->params['supplier_id']);
			$this->db->group_end();
		}
		...
		if ($this->params['total_spent_condition'] != 'any' && is_numeric($this->params['total_spent_amount']))
		{
			...
			$this->db->having('ROUND(sum(phppos_sales.total),'.$decimals.') '.$condition.' '.$this->params['total_spent_amount']);
		}
	...
	public function getNoCustomerData()
	{
		...
		|| isset($this->params['supplier_id']) && $this->params['supplier_id']){
			...
			$this->db->select('
				...
				"" as zip,
				COUNT(*) as count,
				'.$this->db->escape(lang('reports_no_customer')).' as customer,
				"-" as person_id,
				"-" as phone_number,
				"-" as email,
				(
