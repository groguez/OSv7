...
class Detailed_profit_and_loss extends Report
{
	...
	public function getData()
	{
		...
		$data = array();
               ...
               $sales_totals = array();
               $this->db->select('sales.sale_id, SUM(phppos_sales.total) as total', false);
               $this->db->from('sales');
		...
		$this->db->select('categories.name as category, categories.id as category_id, sum('.$this->db->dbprefix('sales_items').'.total) as total, sum('.$this->db->dbprefix('sales_items').'.profit) as profit', false);
		...
		$this->db->select('categories.name as category, categories.id as category_id, sum('.$this->db->dbprefix('sales_item_kits').'.total) as total, sum('.$this->db->dbprefix('sales_item_kits').'.profit) as profit', false);
		...
		$query = $this->db->query("select category, category_id,sum(total) as total,sum(profit) as profit from (".$qry1." UNION ".$qry2. ") as alias  group by category_id order by category");
      ...
      $data['expense_amount'] = $expenses_row ['expense_amount'];
		...
		$model->setParams(array('supplier' => '','category_id' => '','customer_id' => '','inventory' => 'all','show_only_pending' => '', 'show_negative_inventory_only' => '', 'list_each_location_separately' => '', 'item_name' => '','excel_export' => '0' ));
		$inv_summary = $model->getSummaryData();
		$data['total_inventory_value'] = $inv_summary['inventory_total'];
		...
		{
		    $balance = $row['balance']; // Adjust the key if the column is actually named 'balance'
		    if ($balance < 0) {
		        $accounts_payable += abs($balance); // Add the absolute value of negative balances
		    } else {
		        $accounts_receivable += $balance; // Add positive balances
		    }
		}
