...
class Summary_items_receivings extends Report
{
	...
	private function get_items_with_receivings_query($paginate,$only_item_id = FALSE)
	{
		...
		if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
			$this->db->join('item_variations', 'receivings_items.item_variation_id = item_variations.id', 'left');
		}
		
		if (isset($this->params['group_by_variation']) && $this->params['group_by_variation']){
			$this->db->group_by(['receivings_items.item_id', 'receivings_items.item_variation_id']);
		}else{
	...
	{		
		...
		{
			$items_receivings_data[$k]['quantity'] = $quantities_indexed_by_id[$items_receivings_data[$k][$primary_key_column]];
		}
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_without_receivings'))
		{
			...
			{
				$items_in_report_without_receivings[$k]['quantity'] = $quantities_indexed_by_id[$items_in_report_without_receivings[$k][$primary_key_column]];
			}			
		}
