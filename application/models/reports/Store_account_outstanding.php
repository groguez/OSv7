...
class Store_account_outstanding extends Report
{
	...
	{		
		...
		$return = array(
			...
			array('data'=>lang('common_address'), 'align'=> 'left'),
			array('data'=>lang('common_sale_date'), 'align'=> 'left'),
			array('data'=>lang('common_current_date'), 'align'=> 'left'),
			array('data'=>lang('common_days_since_charge'), 'align'=> 'left'),
	...
	public function getOutputData()
	{
		...
		{			
			...
			// Calculate the number of days between sale time and current date
			$sale_date = new DateTime(date(get_date_format(), strtotime($row['sale_time'])));
			$current_date = new DateTime(date(get_date_format()));
			$interval = $current_date->diff($sale_date);
			...
			$tab_row = array(
				...
				array('data'=>to_currency($row['payment_amount']), 'align'=> 'left'),
			 	...
			 	array('data'=>$row['customer_address'], 'align'=> 'left'),
			 	array('data'=>date(get_date_format(), strtotime($row['sale_time'])), 'align'=> 'left'),
			 	array('data'=>date(get_date_format()), 'align'=> 'left'),
			 	array('data'=>$days_between, 'align'=> 'left'),
	...
	{
		$this->db->select('locations.name as location,CONCAT(customer_person_info.first_name," ",customer_person_info.last_name) as customer_name, customers.account_number, CONCAT(customer_person_info.address_1," ",customer_person_info.address_2," ",customer_person_info.city,", ",customer_person_info.state," ",customer_person_info.zip) as customer_address,sales.sale_id, sale_time,SUM(transaction_amount) as payment_amount,sales.comment', false);
