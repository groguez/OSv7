...
class Detailed_sales extends Report
{
	...
	public function getInputData()
	{
		...
		if ($this->settings['display'] == 'tabular')
		{
			...
			$exchange_data['specific_input_data'] = $exchange_rates_dropdown;
                        ...
                        {
                                array_splice($input_params,2,0,array(array('view' => 'dropdown','dropdown_label' => lang('reports_sale_source'),'dropdown_name' => 'sale_source','dropdown_options' => array('all' => lang('reports_all'),'in_person' => lang('reports_in_person_sales'),'online' => lang('reports_online_sales')),'dropdown_selected_value' => 'all')));
                        }
	...
	function getOutputData()
	{
		...
		foreach($this->params['export_excel'] == 1 && isset($report_data['summary']) ? $report_data['summary']:$report_data as $key=>$row)
		{
			...
			$summary_data_row[] = array('data'=>$row['customer_company'], 'align'=>'left');
			...
			$summary_data_row[] = array('data'=>boolean_as_string($row['paid_online']), 'align'=>'right');
			...
			if($this->params['export_excel'] == 1)
			{
				foreach($report_data['details'][$key] as $drow)
				{
					...
					for($k3=1; $k3<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k3++) 
					{
						$custom_field = $this->Sale->get_line_item_custom_field($k3);
						if($custom_field !== FALSE)
						{
							if ($this->Sale->get_line_item_custom_field($k3,'type') == 'checkbox')
							{
								$format_function = 'boolean_as_string';
							}
							elseif($this->Sale->get_line_item_custom_field($k3,'type') == 'date')
							{
								$format_function = 'date_as_display_date';				
							}
							elseif($this->Sale->get_line_item_custom_field($k3,'type') == 'email')
							{
								$format_function = 'strsame';					
							}
							elseif($this->Sale->get_line_item_custom_field($k3,'type') == 'url')
							{
								$format_function = 'strsame';					
							}
							elseif($this->Sale->get_line_item_custom_field($k3,'type') == 'phone')
							{
								$format_function = 'strsame';					
							}
							elseif($this->Sale->get_line_item_custom_field($k3,'type') == 'image')
							{
								$this->load->helper('url');
								$format_function = 'file_id_to_image_thumb';					
							}
							elseif($this->Sale->get_line_item_custom_field($k3,'type') == 'file')
							{
								$this->load->helper('url');
								$format_function = 'file_id_to_download_link';					
							}
							...
							$details_data_row[] = array('data'=>$format_function($drow["line_item_custom_field_".$k3."_value"]), 'align'=>'right');
						}
					}
	...
	public function getDataColumns()
	{
		...
		$return['summary'][] = array('data'=>lang('common_company'), 'align'=> 'left');
		...
		$return['summary'][] = array('data'=>lang('common_paid_online'), 'align'=> 'right');
	...
	{		
		...
		if (isset($this->params['currency']) && $this->params['currency'] !== '')
		{
			...
			else
			{
                ...
                if(isset($this->params['sale_source']) && $this->params['sale_source'] !== 'all')
                {
                        $this->db->where('sales.is_ecommerce', $this->params['sale_source'] == 'online' ? 1 : 0);
                }
	...
	{			
		$this->db->select('sales_items.custom_field_1_value AS line_item_custom_field_1_value, sales_items.custom_field_2_value AS line_item_custom_field_2_value, sales_items.custom_field_3_value AS line_item_custom_field_3_value, sales_items.custom_field_4_value AS line_item_custom_field_4_value, sales_items.custom_field_5_value AS line_item_custom_field_5_value, sales_items.custom_field_6_value AS line_item_custom_field_6_value, sales_items.custom_field_7_value AS line_item_custom_field_7_value, sales_items.custom_field_8_value AS line_item_custom_field_8_value, sales_items.custom_field_9_value AS line_item_custom_field_9_value, sales_items.custom_field_10_value AS line_item_custom_field_10_value​, items.custom_field_1_value,items.custom_field_2_value,items.custom_field_3_value,items.custom_field_4_value,items.custom_field_5_value,items.custom_field_6_value,items.custom_field_7_value,items.custom_field_8_value,items.custom_field_9_value,items.custom_field_10_value,manufacturers.name as manufacturer,sales_items.item_unit_price as unit_price,sales_items.item_variation_id, items.item_id, sales_items.sale_id, items.category_id, items.item_id as item_id,items.item_number, items.product_id as item_product_id, items.name as item_name, categories.name as category, quantity_purchased, CONCAT(phppos_items_quantity_units.unit_name," - ",ROUND(phppos_sales_items.unit_quantity,2)) as unit_quantity, serialnumber, sales_items.description, subtotal, total, tax, profit, commission, discount_percent, flat_discount_amount, items.size as size, items.unit_price as current_selling_price, suppliers.company_name as supplier_name, suppliers.person_id as supplier_id, "item" as row_flag', false);
		...
		$this->db->select('sales_item_kits.custom_field_1_value AS line_item_custom_field_1_value, sales_item_kits.custom_field_2_value AS line_item_custom_field_2_value, sales_item_kits.custom_field_3_value AS line_item_custom_field_3_value, sales_item_kits.custom_field_4_value AS line_item_custom_field_4_value, sales_item_kits.custom_field_5_value AS line_item_custom_field_5_value, sales_item_kits.custom_field_6_value AS line_item_custom_field_6_value, sales_item_kits.custom_field_7_value AS line_item_custom_field_7_value, sales_item_kits.custom_field_8_value AS line_item_custom_field_8_value, sales_item_kits.custom_field_9_value AS line_item_custom_field_9_value, sales_item_kits.custom_field_10_value AS line_item_custom_field_10_value,item_kits.custom_field_1_value,item_kits.custom_field_2_value,item_kits.custom_field_3_value,item_kits.custom_field_4_value,item_kits.custom_field_5_value,item_kits.custom_field_6_value,item_kits.custom_field_7_value,item_kits.custom_field_8_value,item_kits.custom_field_9_value,item_kits.custom_field_10_value,manufacturers.name as manufacturer, sales_item_kits.item_kit_unit_price as unit_price, 0 as item_variation_id, item_kits.item_kit_id, sales_item_kits.sale_id,item_kits.category_id, item_kits.item_kit_id as item_id,item_kits.item_kit_number as item_number, item_kits.product_id as item_product_id, item_kits.name as item_name, categories.name as category, quantity_purchased, NULL as unit_quantity,NULL as serialnumber, sales_item_kits.description, subtotal, total, tax, profit, commission, discount_percent, flat_discount_amount, NULL as size, item_kits.unit_price as current_selling_price, NULL as supplier_name, NULL as supplier_id, "item_kit" as row_flag', false);
		...
		foreach($res as $key=>$drow)
			{	
				...
				$details_data_row[] = array('data'=>$drow['flat_discount_amount'] > 0 ? to_currency($drow['flat_discount_amount']) : $drow['discount_percent'].'%', 'align'=> $drow['flat_discount_amount'] > 0 ? 'right' : 'left');
				...
				if($drow['row_flag'] == 'item'){
	  			...
	  			for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
	  			{
	  				...
	  				if($custom_field !== FALSE)
	  				{
	  					if ($this->Sale->get_line_item_custom_field($k,'type') == 'checkbox')
	  					{
	  						$format_function = 'boolean_as_string';
	  					}
	  					elseif($this->Sale->get_line_item_custom_field($k,'type') == 'date')
	  					{
	  						$format_function = 'date_as_display_date';				
	  					}
	  					elseif($this->Sale->get_line_item_custom_field($k,'type') == 'email')
	  					{
	  						$format_function = 'strsame';					
	  					}
	  					elseif($this->Sale->get_line_item_custom_field($k,'type') == 'url')
	  					{
	  						$format_function = 'strsame';					
	  					}
	  					elseif($this->Sale->get_line_item_custom_field($k,'type') == 'phone')
	  					{
	  						$format_function = 'strsame';					
	  					}
	  					...
	  					$details_data_row[] = array('data'=>$format_function($drow["line_item_custom_field_${k}_value"]), 'align'=>'right');					
	  				}
	  			}
