...
class Summary_items_worse_sellers extends Report
{
	...
	public function getInputData()
	{
		...
		if ($this->settings['display'] == 'tabular')
		{
			...
			$input_params[] = array('view' => 'checkbox','checkbox_label' => lang('reports_show_items_not_sold'), 'checkbox_name' => 'show_items_not_sold');
	...
	{		
		...
		if (isset($this->params['show_items_not_sold']) && $this->params['show_items_not_sold'])
		{
			$items_without_sales_data = $this->get_items_without_sales()->get()->result_array();	
			for($k=0;$k<count($items_without_sales_data);$k++)
			{
				$items_without_sales_data[$k]['quantity'] = 0;
			}
			
			$return = array_merge($items_without_sales_data,$items_sales_data);
			return $return;
