...
class Inventory_low extends Report
{
	...
	public function getData()
	{
		...
		if ($this->params['category_id'])
		{
		    ...
		    $query = 'SELECT X.*,Z.name
                    ...
                    COALESCE(NULLIF(phppos_location_item_variations.reorder_level, 0), phppos_item_variations.reorder_level, NULLIF(phppos_location_items.reorder_level, 0), phppos_items.reorder_level) as reorder_level, 
                    COALESCE(NULLIF(phppos_location_item_variations.replenish_level, 0), phppos_item_variations.replenish_level, NULLIF(phppos_location_items.replenish_level, 0), phppos_items.replenish_level) as replenish_level, description 
	...
	private function dataQuery()
	{
		...
		COALESCE(NULLIF('.$this->db->dbprefix('location_items').'.reorder_level, 0), NULLIF('.$this->db->dbprefix('location_items').'.reorder_level, 0), '.$this->db->dbprefix('items').'.reorder_level) as reorder_level,
		COALESCE(NULLIF('.$this->db->dbprefix('location_items').'.replenish_level, 0), NULLIF('.$this->db->dbprefix('location_items').'.replenish_level, 0), '.$this->db->dbprefix('items').'.replenish_level) as replenish_level,
		description', FALSE);
		$this->db->from('items');
		...
		{
			$this->db->where('((COALESCE('.$this->db->dbprefix('location_item_variations').'.quantity,'.$this->db->dbprefix('location_items').'.quantity) <= COALESCE(NULLIF('.$this->db->dbprefix('location_item_variations').'.reorder_level, 0), NULLIF('.$this->db->dbprefix('item_variations').'.reorder_level, 0), NULLIF('.$this->db->dbprefix('location_items').'.reorder_level, 0), '.$this->db->dbprefix('items').'.reorder_level))) and '.$this->db->dbprefix('items').'.deleted=0');
		}
		else
		{
			$this->db->where('(COALESCE('.$this->db->dbprefix('location_item_variations').'.quantity,'.$this->db->dbprefix('location_items').'.quantity) <= 0 or (COALESCE('.$this->db->dbprefix('location_item_variations').'.quantity,'.$this->db->dbprefix('location_items').'.quantity) <= COALESCE(NULLIF('.$this->db->dbprefix('location_item_variations').'.reorder_level, 0), NULLIF('.$this->db->dbprefix('item_variations').'.reorder_level, 0), NULLIF('.$this->db->dbprefix('location_items').'.reorder_level, 0), '.$this->db->dbprefix('items').'.reorder_level))) and '.$this->db->dbprefix('items').'.deleted=0');
		}
