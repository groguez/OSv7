...
class Woo_categories extends MY_Woo
{
	...
	public function save_category($category_id)
	{
		...
		$this->data['name'] = $category_info->name;
		$this->data['description'] = $category_info->category_description;
		$this->data['slug'] = mb_substr_if_possible(mystr_replace(' ',  '-', $category_id.'-'.$category_info->name), 0, 28);
		$parent = $this->CI->Category->get_ecommerce_category_id($category_info->parent_id);
		if($parent)
		{
			$this->data['parent'] = $parent;
	...
	{	
		$data = array(
			'name' => $category['name'],
			'slug' => mb_substr_if_possible(mystr_replace(' ',  '-', $category['id'].'-'.$this->CI->Category->get_full_path($category['id'],'-')), 0, 28),
		);
