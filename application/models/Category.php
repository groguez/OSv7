...
class Category extends MY_Model
{
	...
	*/
	...
	function get_all_categories_and_sub_categories_as_tree()
	{
		...
		foreach ($categories as $k => $v) 
		{
			...
			$node->depth = $v['depth'];
			$node->children = array();
			$node->tier_list = $this->get_category_tiers($k);
			$objects[$k] = $node;
		}
	...
	function get_category_tiers($category_id)
	{
		$this->db->select('tier_id,	category_id, unit_price, percent_off, cost_plus_percent, cost_plus_fixed_amount');
		$this->db->from('categories_tier_prices');
		$this->db->where('category_id', $category_id);
		$tiers = $this->db->get()->result_array();
		if(count($tiers) > 0)
		{
			return $tiers;
		}
	...
	function get_tier_price_row($tier_id, $category_id)
	{
		$this->db->select('tier_id,	category_id, unit_price, percent_off, cost_plus_percent, cost_plus_fixed_amount');
		$this->db->from('categories_tier_prices');
	...
	function save_category_tiers($tier_data, $category_id)
	{
		if($this->tier_exists($tier_data['tier_id'], $category_id))
		{
			$this->db->where('tier_id', $tier_data['tier_id']);
			$this->db->where('category_id', $category_id);
			return $this->db->update('categories_tier_prices', $tier_data);
		}
		return $this->db->insert('categories_tier_prices', $tier_data);
	}

	function delete_tier_price($tier_id, $category_id){
		$this->db->where('tier_id', $tier_id);
		$this->db->where('category_id', $category_id);
		return $this->db->delete('categories_tier_prices');
	}

	function delete_all_tier_prices($category_id)
	{
		$this->db->where('category_id', $category_id);
		return $this->db->delete('categories_tier_prices');
	}

	function tier_exists($tier_id, $category_id)
	{
		$this->db->from('categories_tier_prices');
		$this->db->where('tier_id',$tier_id);
		$this->db->where('category_id',$category_id);
	...
	function get_all_categories_and_sub_categories_as_indexed_by_name_key($can_cache = TRUE, $format_function = 'mystrtoupper')
	{
	...
	*/
	...
	function create_categories_as_needed($category_name, &$categories_indexed_by_name)
	{
		...
		for($k=0;$k<count($category_list);$k++)
		{
			...
			if (!isset($categories_indexed_by_name[mystrtoupper($category_string)]))
			{
				$parent_category_search = mysubstr($category_string, 0, mystrrpos($category_string,'|') === FALSE ? NULL : mystrrpos($category_string,'|'));
				$parent_id = isset($categories_indexed_by_name[mystrtoupper($parent_category_search)]) ? $categories_indexed_by_name[mystrtoupper($parent_category_search)] : NULL;
				$categories_indexed_by_name[mystrtoupper($category_string)] = $this->save($category, NULL, $parent_id);
			}
		}
	}
	...
	*/
	...
	function get_full_path($category_id,$delimiter=' > ')
	{
		...
		{
			return mystr_replace('|', $delimiter,$categories[$category_id]);
		}
