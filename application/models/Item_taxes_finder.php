...
class Item_taxes_finder extends MY_Model
{
	...
	*/
	function get_info($item_id, $transaction_type = 'sale',$the_current_item = NULL) //Can be sale or receiving
	{
		...
		$location_id_to_use = $CI && property_exists($CI,'cart') && property_exists($CI->cart,'location_id') && $CI->cart->location_id ? $CI->cart->location_id : false;
		...
		{			
			...
			if($CI && property_exists($CI,'cart') && method_exists($CI->cart,'is_tax_overrided') && $CI->cart->is_tax_overrided())
			{
				if ($CI->cart->override_tax_class)
				{
					return $this->Tax_class->get_taxes($CI->cart->override_tax_class);
		...
		{
			$CI->load->helper('sale');
			if ($CI && property_exists($CI,'cart') && method_exists($CI->cart,'get_payment_amount') && is_ebt_sale_not_ebt_cash($this->cart) && $this->config->item('enable_ebt_payments') && $item_info->is_ebt_item)
			{
				return array();
			}
		}
