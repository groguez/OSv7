...
class Invoice extends CI_Model
{
	...
	*/
	function search($type, $search, $days_past_due = NULL, $deleted = 0, $limit=20, $offset=0, $column='invoice_date', $orderby='desc', $status="", $location_id_override=null)
	{
		...
		// If type is 'customer', add the subquery to check for paid_online
		if ($type == 'customer') {
			$this->db->select("(SELECT IF(COUNT(*) > 0, 1, 0) FROM phppos_customer_invoice_payments WHERE phppos_customer_invoice_payments.invoice_id = phppos_customer_invoices.invoice_id AND phppos_customer_invoice_payments.paid_online = 1) as paid_online", false);
		}
	...
	*/
	...
	{	
		...
		if ($status == 2) {
			...
			// no additional where clause for status 1
		} else {
			$this->db->where($type.'_'.'invoices.balance >', 0);
		}
	...
	*/
	function save($type,&$invoice_data, $invoice_id = false, $store_account_sale = false)
	{
		
		//If the balance is 0 or less mark any store account sales as paid
		if ($invoice_id && isset($invoice_data['balance']) && $invoice_data['balance'] <= 1e-6 && $store_account_sale == false)
		{
	...
	function get_displayable_columns($type)
	{
		...
		if ($type == 'customer')
		{
			$return['paid_online'] = array('sort_column' => 'paid_online', 'label' => lang('common_paid_online'), 'format_function' => 'boolean_as_string');
		}
	...
	*/
	...
	public function process_payment($amount,$register,$cc_token,$cc_number = NULL, $cc_ccv = NULL,$expire_month = NULL,$expire_year = NULL,$address = NULL, $zip = NULL, $invoice_id = NULL)
	{
		...
		{
			return $this->process_valor($amount,$register,$cc_token,$cc_number,$cc_ccv,$expire_month,$expire_year,$invoice_id);			
		}
	}
	...
	private function process_valor($amount,$invoice_register_id,$cc_token,$cc_number = NULL,$cc_ccv = NULL, $expire_month = NULL,$expire_year = NULL, $invoice_id)
	{
		
		$application_current_register_id = $this->Employee->get_logged_in_employee_current_register_id();
		$application_in_session_register_info = $this->Register->get_info($application_current_register_id);
		if (!$application_in_session_register_info || !$application_in_session_register_info->register_id)
		{
			$application_in_session_register_info = $this->Register->get_default_register_info();
		}
		
		
	   $invoice_register_info = $this->Register->get_info($invoice_register_id);
	   
	   $is_card_not_present = $invoice_register_id == -1;
	   require_once (APPPATH.'libraries/Valorprocessor.php');
	   $credit_card_processor = new Valorprocessor($this, $this->Employee->get_logged_in_employee_current_location_id());
	   
	   
	   
	   if ($invoice_register_info->register_id)
	   {
	   
	   		//Set up register parameters based on $invoice_register_id passed in (we well revert back at end)
			$credit_card_processor->epi = $invoice_register_info && property_exists($invoice_register_info,'emv_terminal_id') ? $invoice_register_info->emv_terminal_id : FALSE;
			$credit_card_processor->appkey = $invoice_register_info && property_exists($invoice_register_info,'valor_appkey') ? $invoice_register_info->valor_appkey : FALSE;	
			$credit_card_processor->register_enable_tips = $invoice_register_info->enable_tips;
		}
	   
	   if ($is_card_not_present)
	   {
 	   	  $response = $credit_card_processor->process_invoice_card_not_present_transaction($cc_number,$amount,$expire_month,$expire_year, $cc_ccv);
	   }
	   elseif($cc_token)
	   {
    	  $invoice_info = $this->get_info('customer',$invoice_id); 
 		  $customer_person_id = $invoice_info->person_id;
 		  $customer_info = $this->Customer->get_info($customer_person_id);
		   
 	   	  $response = $credit_card_processor->process_invoice_token_transaction($amount,$customer_info);
	   }
	   else
	   {
 	   	  $response = $credit_card_processor->process_invoice_terminal_transaction($amount);
	   }
	   
	    //revert to $application_in_session_register_info
		$credit_card_processor->epi = $application_in_session_register_info && property_exists($application_in_session_register_info,'emv_terminal_id') ? $application_in_session_register_info->emv_terminal_id : FALSE;
		$credit_card_processor->appkey = $application_in_session_register_info && property_exists($application_in_session_register_info,'valor_appkey') ? $application_in_session_register_info->valor_appkey : FALSE;	
		$credit_card_processor->register_enable_tips = $application_in_session_register_info->enable_tips;
		...
		$MerchantID =  $credit_card_processor->appid;
		$AcctNo = $response['MASKED_PAN'];
		$TranCode = $response['TRAN_TYPE'];
		$AuthCode = $response['CODE'];
		$RefNo = $response['TXN_ID'];
		$TranNo = $response['TRAN_NO'];
		$surcharge_amount = isset($response['SURCHARGE_AMOUNT']) ? $response['SURCHARGE_AMOUNT'] : '0';
	...
	function get_payments($type, $invoice_id=null, $payment_id=null, $limit=null, $offset=null, $sort_col=null, $sort_dir=null)
	{
		...
		if($sort_col && $sort_dir){
			...
			$this->db->order_by('payment_date DESC, payment_id DESC');
		}
	...
	// 1: valor, 2: coreclear2
	function get_processor_payment_link($invoice_id)
	{
	...
	private function get_auto_match_payment($cart, $pos = 0){
		$cart->get_paid_store_account_ids();
		$calculated_amount_1 = 0;
		$pos_start_amount = 0;
		$pos_stop_amount = 0;
		foreach(array_keys($cart->get_paid_store_account_ids()) as $index => $sale_id_paid){
			$paid_amount = $cart->paid_store_account_amounts[$sale_id_paid];
			if($index == $pos){
				$pos_start_amount = $calculated_amount_1;
				$pos_stop_amount = $calculated_amount_1 + $paid_amount;
				break;
			}
			$calculated_amount_1 += $paid_amount;
		}

		$pos_start_amount_index = -1;
		$pos_stop_amount_index = -1;
		$calculated_amount_start = 0;
		$calculated_amount_end = 0;
		$pos_payment_list = array();

		$payment_temp = null;
		foreach($cart->get_payments() as $index => $payment) {
			$payment_temp = get_object_vars($payment);
			unset($payment_temp['ebt_voucher_no']);
			unset($payment_temp['ebt_auth_code']);
			$payment_amount = $payment_temp['payment_amount'];

			$calculated_amount_end += $payment_amount;

			if($pos_start_amount_index == -1 && $calculated_amount_end > $pos_start_amount){//start point
				$pos_start_amount_index = $index;
				$payment_temp['payment_amount'] -= ( $pos_start_amount - $calculated_amount_start );
			}

			if($pos_stop_amount_index == -1 && $calculated_amount_end >= $pos_stop_amount){
				$pos_stop_amount_index = $index;
				$payment_temp['payment_amount'] -= ( $calculated_amount_end - $pos_stop_amount );
				$pos_payment_list[] = $payment_temp;
				break;
			}else if($pos_start_amount_index > -1){
				$pos_payment_list[] = $payment_temp;
			}
			$calculated_amount_start += $payment_amount;
		}
		return $pos_payment_list;
	}

	function update_store_account_payment_invoice($type, $cart){
		if($type == "customer"){
			...
			if (!empty($paid_sales))
			{
				...
				{
					$paid_amount = $cart->paid_store_account_amounts[$sale_id_paid];
					//check $sale_id_paid invoice exist
					$ref_invoice_detail = $this->get_invoice_details_by_type_id($type, $sale_id_paid);
					if($ref_invoice_detail){
						$ref_invoice_id = $ref_invoice_detail->invoice_id;
						$ref_invoice_info = $this->get_info($type, $ref_invoice_id);
						if($ref_invoice_info->deleted == 1) continue;

						$new_balance = $ref_invoice_info->balance - $paid_amount;
						$payment_list = $this->get_auto_match_payment($cart, $index);//get match payment

						$this->db->trans_start();
						foreach($payment_list as $payment){
							$this->add_payment($type, $ref_invoice_id, $payment);
						}
						$invoice_data = array('balance' => $new_balance, 'last_paid' => date('Y-m-d'));
						$this->save($type, $invoice_data, $ref_invoice_id, true);
		...
		if($type == "supplier"){
			...
			{
				foreach(array_keys($cart->get_paid_store_account_ids()) as $index => $receiving_id_paid)
				{
					$paid_amount = $cart->paid_store_account_amounts[$receiving_id_paid];
					//check $receiving_id_paid invoice exist
					$ref_invoice_detail = $this->get_invoice_details_by_type_id($type, $receiving_id_paid);
					if($ref_invoice_detail){
						$ref_invoice_id = $ref_invoice_detail->invoice_id;
						$ref_invoice_info = $this->get_info($type, $ref_invoice_id);
						if($ref_invoice_info->deleted == 1) continue;
	...
	function get_invoice_details_by_type_id($type, $type_id)
	{
		$this->db->from($type.'_invoice_details');
		$this->db->where(($type == 'customer'? 'sale' : 'receiving') . '_id', $type_id);
		return $this->db->get()->row();
	}

	function create_store_account_transaction($type, $invoice_info, $payment_data, $paid_online = NULL){

		$invoice_id = $invoice_info->invoice_id;

		$has_store_account_paid_amount = 0;

		$cart = NULL;
		if($type == "customer"){

			$invoice_details = $this->Invoice->get_details($type, $invoice_id);
			foreach($invoice_details as $invoice_detail){
				if($invoice_detail['sale_id'] > 0){
					//check store account payment exist
					$unpaid_sales = $this->Sale->get_unpaid_store_account_sales(array($invoice_detail['sale_id']));
					if($unpaid_sales[0]['payment_amount'] > 0) {
						$has_store_account_paid_amount += $unpaid_sales[0]['payment_amount'];
					}
				}
			}
	
			//check full payment
			$is_full_pay_invoice = false;
			if(( $invoice_info->balance - $payment_data['payment_amount']) <= 1e-6 ){
				$is_full_pay_invoice = true;
			}
	
			if(count($invoice_details) > 1 && $is_full_pay_invoice == false) {
				// can not match each store account payments and invoice amount
				return false;
			}
	
			if($has_store_account_paid_amount == 0){
				// don't need to make store account transaction
				return false;
			}

			//change_mode & set customer
			$store_account_payment_item_id = $this->Item->create_or_update_store_account_item();
	
			$cart = new PHPPOSCartSale();
			$cart->empty_items();
			$cart->set_mode('store_account_payment');
			$cart->employee_id = 1; //admin
			$cart->location_id = $invoice_info->location_id; //1
			$cart->select_customer($invoice_info->customer_id.'|FORCE_PERSON_ID|');
			$cart->add_item(new PHPPOSCartItemSale(array(
				'cost_price' => 0,
				'unit_price' => 0,
			...
			$cart->save();
			$invoice_details = $this->Invoice->get_details($type, $invoice_id);
			$invoice_sale_ids = array();
			foreach($invoice_details as $invoice_detail){
				if($invoice_detail['sale_id'] > 0){
					$invoice_sale_ids[] = $invoice_detail['sale_id'];
				}
			}
	
			$unpaid_store_account_sales = $this->Sale->get_unpaid_store_account_sales($invoice_sale_ids);
			$unpaid_store_account_sales_pay_amount = 0;
			$comment = "";
			foreach ($unpaid_store_account_sales as $unpaid_sale) {
				$unpaid_sale_pay_amount = $unpaid_sale['payment_amount'] < $payment_data['payment_amount'] ? $unpaid_sale['payment_amount'] : $payment_data['payment_amount'];
				$unpaid_store_account_sales_pay_amount += $unpaid_sale_pay_amount;
	
				//pay_store_account_sale
				$cart->add_paid_store_account_payment_id($unpaid_sale['sale_id'], $unpaid_sale_pay_amount );
				$cart_items = $cart->get_items();
				foreach($cart_items as $item)
				{
					if ($item->name == lang('common_store_account_payment'))
					{
						$item->unit_price += $unpaid_sale_pay_amount; 
						break;
					}
				}
				$comment = lang('sales_pays_sales'). ' - '.implode(', ',array_keys($cart->get_paid_store_account_ids()));
			}
	
			$cart->comment = $comment;
			$cart->save();
	
			$payment_data['payment_amount'] = $unpaid_store_account_sales_pay_amount;
			
			if ($type == 'customer' && $paid_online !== NULL)
			{
				$payment_data['paid_online'] = $paid_online ? 1 : 0;
			}
			
			$cart->add_payment(new PHPPOSCartPaymentSale($payment_data));
			$cart->save();
	
			// complete
			$cart->change_cart_date = date('Y-m-d H:i:s');
			// $cart->suspended = 0;
			...
			$invoice_details = $this->Invoice->get_details($type, $invoice_id);
			foreach($invoice_details as $invoice_detail){
				if($invoice_detail['receiving_id'] > 0){
					//check store account payment exist
					$unpaid_receivings = $this->Receiving->get_unpaid_store_account_recvs(array($invoice_detail['receiving_id']));
					if($unpaid_receivings[0]['payment_amount'] > 0) {
						$has_store_account_paid_amount += $unpaid_receivings[0]['payment_amount'];
					}
				}
			}
			...
			if(count($invoice_details) > 1 && $is_full_pay_invoice == false) {
				// can not match each sales store account payment and invoice amount
				return false;
			}
			...
			$cart->empty_items();
			$cart->set_mode('store_account_payment');
			$cart->supplier_id = $invoice_info->supplier_id;
			...
			$cart->save();
			$invoice_details = $this->Invoice->get_details($type, $invoice_id);
			$invoice_receiving_ids = array();
			foreach($invoice_details as $invoice_detail){
				if($invoice_detail['receiving_id'] > 0){
					$invoice_receiving_ids[] = $invoice_detail['receiving_id'];
				}
			}

			$unpaid_store_account_receivings = $this->Receiving->get_unpaid_store_account_recvs($invoice_receiving_ids);
			$unpaid_store_account_receivings_pay_amount = 0;
			$comment = "";
			foreach ($unpaid_store_account_receivings as $unpaid_receiving) {
				$unpaid_receiving_pay_amount = $unpaid_receiving['payment_amount'] < $payment_data['payment_amount'] ? $unpaid_receiving['payment_amount'] : $payment_data['payment_amount'];
				$unpaid_store_account_receivings_pay_amount += $unpaid_receiving_pay_amount;

				//pay_store_account_sale
				$cart->add_paid_store_account_payment_id($unpaid_receiving['receiving_id'], $unpaid_receiving_pay_amount );
				$cart_items = $cart->get_items();
				foreach($cart_items as $item)
				{
					if ($item->name == lang('common_store_account_payment'))
					{
						$item->unit_price += $unpaid_receiving_pay_amount; 
						break;
					}
				}
				$comment = lang('receivings_pays_receivings'). ' - '.implode(', ',array_keys($cart->get_paid_store_account_ids()));
			}

			$cart->comment = $comment;
			$cart->save();

			$payment_data['payment_amount'] = $unpaid_store_account_receivings_pay_amount;
			$cart->add_payment(new PHPPOSCartPaymentRecv($payment_data));
			$cart->save();

			// complete
			$cart->change_cart_date = date('Y-m-d H:i:s');
			// $cart->suspended = 0;
			$receiving_id_raw = $this->Receiving->save($cart);
			return $receiving_id_raw;
		}
		return 0;
	}

	function exist_sale_invoice($type, $location_id, $u_id, $type_id){
		$this->db->from($type.'_invoices');
		$this->db->join($type.'_invoice_details', $type.'_invoices.invoice_id = '.$type.'_invoice_details.invoice_id');
		$this->db->where($type.'_invoices.location_id', $location_id);
		$this->db->where($type.'_invoices.'.$type.'_id', $u_id);
		$this->db->where($type.'_invoice_details.'.($type=='customer'? 'sale' : 'receiving').'_id', $type_id);
		$query = $this->db->get();
		if($query->num_rows() > 0) return true;
		return false;
	}

	function send_payment_email_alert($type, $invoice_id, $payment_amount, $balance)
	{
		$invoice_info = $this->get_info($type, $invoice_id);
		$loc_info = $this->Location->get_info($invoice_info->location_id);
		
		if ($type == 'customer') {
			$customer_info = $this->Customer->get_info($invoice_info->person_id);
			$customer_name = $customer_info->first_name . ' ' . $customer_info->last_name;
			
			$subject = $customer_name . ' '.lang('sales_has_made').' ' . to_currency($payment_amount) . ' '.lang('invoices_invoice_payment').'';
			$message = $customer_name . ' '.lang('sales_has_made_payment_of').' ' . to_currency($payment_amount) . ' '.lang('invoice_towards_invoice').' ' . $invoice_id . '. '.lang('invoices_balance_of_inv').' ' . to_currency($balance);
		} else {
			$supplier_info = $this->Supplier->get_info($invoice_info->person_id);
			$supplier_name = $supplier_info->company_name;
			
			$subject = $supplier_name . ' '.lang('sales_has_made').' ' . to_currency($payment_amount) . ' '.lang('invoices_invoice_payment').'';
			$message = $supplier_name . ' '.lang('sales_has_made_payment_of').' ' . to_currency($payment_amount) . ' '.lang('invoice_towards_invoice').' ' . $invoice_id . '. '.lang('invoices_balance_of_inv').' ' . to_currency($balance);
		}
