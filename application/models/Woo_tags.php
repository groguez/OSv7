...
class Woo_tags extends MY_Woo
{
	...
	const put_endpoint = "products/tags/<id>";
	...
	private static function put_endpoint($woo_tag_id)
	{
		return str_replace("<id>", $woo_tag_id, self::put_endpoint);
	}
	...
	public function save_tag($tag_name)
	{
		...
		try
		{
			...
			$ecommerce_tag_id = $this->CI->Tag->get_ecommerce_tag_id($phppos_tag_id);
			...
			{	
				...
				{
					//See if we have an existing tag and attempt to link
				...
				{
					$woo_tag_id = $this->response['id'];
				}
			}
			elseif($ecommerce_tag_id)
			{
				$this->response = parent::do_put(self::put_endpoint($ecommerce_tag_id));
			}
			
			if ($phppos_tag_id !== FALSE && $woo_tag_id !== FALSE)
			{
				$this->woo->link_tag($phppos_tag_id, $woo_tag_id);
			}
	...
	public function batch_tags()
	{
		...
		foreach($this->CI->Tag->get_all_for_ecommerce() as $phppos_tag_id => $phppos_tag_data)
		{
			...
			{
				if(!isset($woo_tags[mystrtoupper($phppos_tag_data['name'])]))
				{
					//create
				...
				{
					//update
					$this->data['update'][] = array('name' => $phppos_tag_data['name'], 'id' => $woo_tags[mystrtoupper($phppos_tag_data['name'])]);
