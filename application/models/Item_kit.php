...
class Item_kit extends MY_Model
{
	...
	*/
	function get_all($deleted= 0,$limit=10000, $offset=0,$col='name',$ord='asc')
	{
		...
		$item_kit_items_table = $this->db->dbprefix('item_kit_items');
		...
		FLOOR(MIN(`'.$location_items_table.'`.`quantity` / `'.$item_kit_items_table.'`.`quantity`)) as quantity');
		$this->db->from('item_kits');
		...
		$this->db->join('item_kit_items', 'item_kit_items.item_kit_id = item_kits.item_kit_id', 'left');
		$this->db->join('location_items', 'location_items.item_id = item_kit_items.item_id and location_items.location_id = '.$current_location, 'left');
	...
	*/
	function get_info($item_kit_id, $can_cache = TRUE)
	{
		...
		if (!is_numeric($item_kit_id))
		{
			...
			$fields = array('dynamic_unit_price','dynamic_cost_price','barcode_name','info_popup','item_kit_id','item_kit_number','product_id','name','category_id','manufacturer_id','description','tax_included','unit_price','cost_price','override_default_tax','is_ebt_item','commission_percent','commission_percent_type','commission_fixed','change_cost_price','disable_loyalty','deleted','tax_class_id','max_discount_percent','max_edit_price','min_edit_price','custom_field_1_value','custom_field_2_value','custom_field_3_value','custom_field_4_value','custom_field_5_value','custom_field_6_value','custom_field_7_value','custom_field_8_value','custom_field_9_value','custom_field_10_value','required_age','verify_age','allow_price_override_regardless_of_permissions','only_integer','is_barcoded','item_kit_inactive','default_quantity','dynamic_pricing','is_favorite','loyalty_multiplier', 'limited_sales_quantity');
	...
	*/
	...
	function get_displayable_columns()
	{
		...
		$return = array(
			...
			'quantity' => 										array('sort_column' => 'quantity', 'label' => lang('item_kits_quantity'),'format_function' => 'to_quantity'),
	...
	*/
	...
	public function get_sale_price(array $params)
	{
		...
		if($item_kit_info->category_id > 0 && $tier_id > 0 && !$item_kit_tier_row){
			$item_kit_category_id = $item_kit_info->category_id;

			$search_category_level = 1;
			while ($search_category_level > 0 && $search_category_level < 100 && $category_tier_row == NULL) {
				if($this->Category->tier_exists($tier_id, $item_kit_category_id)){
					$category_tier_row = $this->Category->get_tier_price_row($tier_id, $item_kit_category_id);
				}else{
					$item_kit_category_info = $this->Category->get_info($item_kit_category_id);
					if($item_kit_category_info->parent_id > 0){
						$item_kit_category_id = $item_kit_category_info->parent_id;
					}else{
						$search_category_level = -1;
						break;
					}
		...
		{
			return to_currency_no_money($category_tier_row->unit_price, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($category_tier_row) && $category_tier_row->percent_off)
		{
			$item_kit_unit_price = (double)$item_kit_location_info->unit_price ? $item_kit_location_info->unit_price : $item_kit_info->unit_price;
			return to_currency_no_money($item_kit_unit_price *(1-($category_tier_row->percent_off/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($category_tier_row) && $category_tier_row->cost_plus_percent)
		{
			$item_kit_cost_price = (double)$item_kit_location_info->cost_price ? $item_kit_location_info->cost_price : $item_kit_info->cost_price;
			return to_currency_no_money($item_kit_cost_price *(1+($category_tier_row->cost_plus_percent/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
