	...
	*/
	function login($username, $password)
	{
		...
		if ($query->num_rows() ==1)
		{
			...
			$allowed_sessions = $this->Employee_appconfig->get('allowed_sessions') ? json_decode($this->Employee_appconfig->get('allowed_sessions')) : array();			
	...
	*/
	function get_logged_in_employee_current_location_id()
	{
		if($this->is_logged_in())
		{
			...
			//Return the first location user is authenticated for and set in session
			$session_location_id = current($this->get_authenticated_location_ids($this->session->userdata('person_id')));
			$this->session->set_userdata('employee_current_location_id', $session_location_id);
