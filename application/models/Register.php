...
class Register extends MY_Model
{
	...
	*/
	...
	function get_register_log_details($id)
	{
		$this->db->select('register_log_audit.*, CONCAT(employee.first_name, " ",employee.last_name) as employee_name', false);
 		...
 		$this->db->join('expenses', 'register_log_audit.expense_id = expenses.id', 'left');
  		$this->db->where('register_log_id',$id);
  		$this->db->where('register_log_audit.amount != 0');
		$this->db->group_start();
			$this->db->where('expenses.deleted', 0);
			$this->db->or_where('expenses.deleted is NULL');
		$this->db->group_end();
	...
	{
		if($data['expense_id'] > 0){
			$this->db->from('register_log_audit');
			$this->db->where('expense_id', $data['expense_id']);
			$this->db->limit(1);
			$register_log_audit = $this->db->get()->result_array();
			if(count($register_log_audit) == 1){
				$this->db->where('id', $register_log_audit[0]['id']);
				if($this->db->update('register_log_audit', $data))
					return  $register_log_audit[0]['id'];
				return false;
			}
		}
  ...
  function search($search, $deleted=0,$limit = 20, $offset = 0, $column = 'register_id', $orderby = 'asc',$location_id_override = NULL) {
	...
	public function clear_cash_register_expenses($expense_data, $action = 0){ // 0:delete, 1:undelete
		$this->db->from('register_log_audit');
		$this->db->where('expense_id', $expense_data->id);
		$this->db->limit(1);
		$register_log_audit = $this->db->get()->result_array();
		if(count($register_log_audit) == 1){
			$register_log_id = $register_log_audit[0]['register_log_id'];

			$this->db->from('register_log_payments');
			$this->db->where('register_log_id', $register_log_id);
			$this->db->limit(1);
			$register_log_payment = $this->db->get()->result_array();
			if(count($register_log_payment) == 1){
				$register_log_payment_data = $register_log_payment[0];
				if($action){
					$register_log_payment_data['total_payment_subtractions'] += ($expense_data->expense_amount + $expense_data->expense_tax);
				}else{
					$register_log_payment_data['total_payment_subtractions'] -= ($expense_data->expense_amount + $expense_data->expense_tax);
				}
				$this->db->where('id', $register_log_payment_data['id']);
				$this->db->update('register_log_payments', $register_log_payment_data);
			}
		}

		$this->db->where('expense_id', $expense_data->id);
		$this->db->update('register_log_audit', array('amount' => 0));
		return true;
	}
}
?>
