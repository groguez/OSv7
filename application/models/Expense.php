...
class Expense extends MY_Model {
    ...
    function delete($expense_id) {
		$this->update_cash_register($expense_id);
        $this->db->where('id', $expense_id);
    ...
    function undelete_list($expense_ids) {
		foreach($expense_ids as $expense_id){
			$this->update_cash_register($expense_id, 1);
		}
        $this->db->where_in('id', $expense_ids);
        return $this->db->update('expenses', array('deleted' => 0));
	}

	function update_cash_register($expense_id, $action = 0){ // 0 - delete, 1 - undelete
		$this->load->model('Register');
		$can_commit = TRUE;
		$this->db->trans_begin();

		$exist_expense_data = $this->get_info($expense_id);
		if($action == 0){ // delete
			$exist_cash_register_id = $exist_expense_data->register_id;
			if($exist_cash_register_id){
				$this->Register->clear_cash_register_expenses($exist_expense_data, $action);
			}
		}else{ // undelete
			$exist_cash_register_id = $exist_expense_data->register_id;
			if($exist_cash_register_id){
				$cash_register = $this->Register->get_register_log_by_id($exist_cash_register_id);
				if($cash_register){
					$register_log_id = $cash_register->register_log_id;
					$amount = $exist_expense_data->expense_amount + $exist_expense_data->expense_tax;
					...
					if(count($register_log_audit) == 1){
						$register_audit_log_data = $register_log_audit[0];
						$register_audit_log_data['amount'] = -$amount;
