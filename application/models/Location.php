...
class Location extends MY_Model
{
	...
	*/
	...
	function get_info_for_key($key, $override_location_id = false, $use_cache = TRUE)
	{
       ...
       function get_currency_info($location_id)
       {
               $this->db->select('currency_exchange_rates.*');
               $this->db->from('locations');
               $this->db->join('currency_exchange_rates','locations.currency_rate_id=currency_exchange_rates.id','left');
               $this->db->where('location_id',$location_id);
