...
class Receiving extends MY_Model
{
	...
	function save ($cart,$async=TRUE)
	{
		...
		if ($receiving_id && $this->is_store_accounts_paid_receiving_already_exist($receiving_id))
		{
			$already_paid_store_account_amount = "";
			
			$this->db->select('SUM(transaction_amount) as already_paid_store_account_amount');
			$this->db->from('supplier_store_accounts_paid_receivings');
			$this->db->join('supplier_store_accounts', 'supplier_store_accounts_paid_receivings.store_account_payment_receiving_id = supplier_store_accounts.receiving_id');
			$this->db->where('supplier_store_accounts_paid_receivings.receiving_id', $receiving_id);
		...
		if ($receiving_id)
		{
            ...
            $receiving_create_date = $before_save_receiving_info->created_at;
		}
		else
		{
			$before_save_receiving_info = FALSE;
            $receiving_create_date = date('Y-m-d H:i:s');
		}
        ...
        'created_at'		               => $receiving_create_date,
        'updated_at'               		   => $receiving_id ? date('Y-m-d H:i:s') : NULL,
		
		);
		
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
		{
			$receivings_data["custom_field_${k}_value"] = $cart->{"custom_field_${k}_value"};
		}
			...
			if($supplier_id > 0 && $balance)
			{
				if($before_save_receiving_info){
					$store_account_transaction = array(
						...
						'balance'=> 0,
						'date' => $before_save_receiving_info->receiving_time
					);
					$this->update_store_account($receiving_id, 1, $store_account_payment, $store_account_transaction, 1);
				}else{
					$store_account_transaction = array(
						'supplier_id'=>$supplier_id,
						'receiving_id'=>$receiving_id,
					...
					$this->update_store_account($receiving_id, 1, $store_account_payment, $store_account_transaction);
				}
			} else if ($supplier_id > 0 && $store_account_payment){
				if($before_save_receiving_info){
					$store_account_transaction = array(
						'supplier_id'=>$supplier_id,
						'receiving_id'=>$receiving_id,
		...
		foreach($items as $line=>$item)
		{
			...
			for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
			{
				$receivings_items_data["custom_field_${k}_value"] = $item->{"line_item_custom_field_${k}_value"};
			}
					...
					if ($this->config->item('calculate_average_cost_price_from_receivings'))
					{
						if ($item->quantity_unit_quantity !== NULL)
						{
							...
							if (isset($quantity_unit_cost_price) || isset($quantity_unit_selling_price))
							{
								...
								if ($this->config->item('update_base_unit_price_from_units'))
								{									
									$unit_price_avg = $quantity_unit_selling_price/$item->quantity_unit_quantity;
				
									//Update selling price					
									$item_data = array('unit_price'=>$unit_price_avg);
	...
	function delete($receiving_id, $all_data = false, $update_quantity = true)
	{
		...
		if ($suspended  == 0)
		{
			$this->update_store_account($receiving_id, 0, $recv_info['store_account_payment']);
		}
	...
	function undelete($receiving_id)
	{
		...
		if ($suspended == 0)
		{
			$this->update_store_account($receiving_id, 2, $recv_info['store_account_payment'], NULL);
		}
		...
		$this->db->where('receiving_id', $receiving_id);
		$ret_update = $this->db->update('receivings', array('deleted' => 0,'deleted_by'=>NULL,'last_modified' => date('Y-m-d H:i:s')));
		if($ret_update){
	...
	function update_store_account($receiving_id, $action = 0, $store_account_payment = 0, $insert_record = NULL, $override = 0)
	{
		...
		if($action == 0){ // delete
			...
			$this->db->where('supplier_store_accounts.state', 0);
			$this->db->where('receiving_id', $receiving_id);
			...
			if( count($store_account_records)>0 && $to_be_paid_result->num_rows()>=1 ){
				foreach($store_account_records as $store_account_record)
				{
					...
					//update after logs
					$this->db->from('supplier_store_accounts');
					$this->db->where('supplier_store_accounts.state', 0); // normal
					$this->db->where('supplier_id', $supplier_id);
					$this->db->group_start();
						$this->db->group_start();
							$this->db->or_where("supplier_store_accounts.date = ", $store_account_record['date']);
							$this->db->where("supplier_store_accounts.sno > ", $store_account_record['sno']);
						$this->db->group_end();
						$this->db->or_where("supplier_store_accounts.date > ", $store_account_record['date']);
					$this->db->group_end();
					$this->db->where("supplier_store_accounts.receiving_id !=", $receiving_id);
					...
					foreach($after_logs as $log){
						...
						$this->db->update('supplier_store_accounts', $log);
					}

					$store_account_record['state'] = 1; // deleted status
					$this->db->where('sno', $store_account_record['sno']);
					$this->db->update('supplier_store_accounts', $store_account_record);
				}
			}
			...
			$this->db->where("supplier_store_accounts.date <= ", $insert_record['date']);
			$this->db->order_by("supplier_store_accounts.date", "desc");
			$this->db->order_by("supplier_store_accounts.sno", "desc");
			$this->db->limit(1);
			...
			if($override == 1){// edit mode
				$this->db->delete('supplier_store_accounts', array('receiving_id' => $receiving_id, 'state' => 1));
			}

			$this->db->insert('supplier_store_accounts', $insert_record);

		} else if($action == 2){ //undelete
			$this->db->reset_query();
			$this->db->from('supplier_store_accounts');
			$this->db->where('supplier_store_accounts.state', 1);// only get deleted state except overwritten state
			$this->db->where("supplier_store_accounts.receiving_id", $receiving_id);
			$this->db->order_by("supplier_store_accounts.date", "asc");
			$this->db->order_by("supplier_store_accounts.sno", "asc");
			...
			foreach($deleted_log_list as $delete_log){
				...
				$this->db->group_start();
					$this->db->group_start();
						$this->db->where("supplier_store_accounts.date = ", $delete_log['date']);
						$this->db->where("supplier_store_accounts.sno < ", $delete_log['sno']);
					$this->db->group_end();
					$this->db->or_where("supplier_store_accounts.date < ", $delete_log['date']);
				$this->db->group_end();
				...
				$this->db->update('supplier_store_accounts', $delete_log);
			}
		}
	}
	...
	function get_line_item_custom_field($number,$key="name")
	{
		...
		if (!$config_data)
		{
			$config_data = $this->config->item('receiving_line_item_custom_field_prefs') ? unserialize($this->config->item('receiving_line_item_custom_field_prefs')) : array();
		}
    ...
    function get_receiving_ids($start_date = false, $end_date = false, $start_date_created = false, $end_date_created = false, $start_date_updated = false, $end_date_updated = false) {
        $this->db->select('receiving_id');
        $this->db->from('receivings');
        ...
        if ($start_date_created) {
			$this->db->where('receivings.created_at >=', $start_date_created);
		}

		if ($end_date_created) {
			$this->db->where('receivings.created_at <=', $end_date_created);
		}

        if ($start_date_updated) {
			$this->db->where('receivings.updated_at >=', $start_date_updated);
		}

		if ($end_date_updated) {
			$this->db->where('receivings.updated_at <=', $end_date_updated);
		}
	...
	function get_receiving_payments($receiving_id)
	{
       ...
       function get_receivings_for_supplier_by_items($item_id, $supplier_id)
       {
               $return = array();

               $this->db->select('receivings.receiving_time, receivings.receiving_id, receivings_items.item_unit_price, receivings_items.discount_percent')
                       ->from('receivings')
                       ->join('receivings_items', 'receivings.receiving_id = receivings_items.receiving_id')
                       ->where('receivings.supplier_id', $supplier_id)
                       ->where('receivings_items.item_id', $item_id)
                       ->order_by('receiving_time DESC');
