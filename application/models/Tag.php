...
class Tag extends MY_Model
{
	function count_all($show_hidden = TRUE)
	{
		...
		if (!$show_hidden)
		{
			$this->db->where('id NOT IN (SELECT tag_id FROM phppos_grid_hidden_tags '. ($location_id ? 'WHERE location_id='.$location_id :'') .')');
		}
	...
	*/
	...
	function save_tags_for_item($item_id, $tags, $replace = TRUE)
	{
		...
		$tags = explode(',', $tags);
		$current_tags_names = $this->get_tags_for_item($item_id);
		$tags = array_map('trim', $tags);
		$merged_tags = array_unique(array_merge($current_tags_names, $tags));	
		foreach($merged_tags as $tag)
		{
			if ($tag != '')
			{
				...
				if (is_numeric($tag) && $this->tag_id_exists($tag)) //Numeric Tag ID
				{
					...
					$this->db->replace('items_tags', array('item_id' => $item_id, 'tag_id' => $tag));
				}
				elseif ($this->tag_name_exists($tag)) //Named tag
				{
					...
					$this->db->replace('items_tags', array('item_id' => $item_id, 'tag_id' => $tag_id));
				}
				else //Create new tag
				{
					$all_tags_list[] = $tag;
