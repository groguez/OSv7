...
class Woo extends Ecom
{
	...
	public function update_item_from_phppos_to_ecommerce($item_id, $data = array())
	{
		try
		{
			$this->log(lang("save_item_fDefault tier is selected when sale is started (No need to pick another tier)rom_phppos_to_ecommerce").' '. $item_id);
	...
	function get_tags($use_cache = TRUE)
	{
		...
		$tags = $woo_tags->get_tags();
 		...
 		foreach($tags as $index => $tag)
 		{
 			...
 			{
 				$phppos_tag_id = $this->Tag->save(myucwords(mystrtolower($tag['name'])));
 			} else {
	...
	function import_ecommerce_items_into_phppos()
	{
		...
		$woo_store_url = $this->config->item('woo_api_url');
		$woo_api_key = $this->config->item('woo_api_key');
		$woo_api_secret = $this->config->item('woo_api_secret');	
		$woo_read_chunk_size = 25;
		$woo_read_sleep = 0;
		$woo_curl_timeout = 3600;
		$woo_api_version = $this->config->item('woo_version') == '3.0.0' ? 3 : 2;
				
		$url_parts = parse_url($woo_store_url);
		$options  = array('wp_api' => true,'version' => 'wc/v'.$woo_api_version, 'timeout' => $woo_curl_timeout,'verify_ssl' => FALSE);
	    ...
	    // 1) Instantiate the official Woo SDK (make sure you’ve run `composer require automattic/woocommerce`)
	    $woocommerce = new Client($woo_store_url, $woo_api_key, $woo_api_secret, $options);

	    $limit = $woo_read_chunk_size;
	    $page  = 1;

	    do {
	           // Pull one page of products (as arrays)
	           ...
	           foreach ($products as $prod) {
	               // Now $prod is an array, not an object
	               if ($prod['catalog_visibility'] === 'hidden') {
	                   continue;
	               }
	               if (! in_array($prod['type'], ['simple', 'variable'], true)) {
	                   continue;
	               }

	               // Single‐row lookup in POS
	               $q    = $this->db->get_where('items', ['ecommerce_product_id' => $prod['id']], 1);
	               $item = $q->row_array();
	               $q->free_result();
	               unset($q);

	               $pos_ts = isset($item['last_modified'])
	                         ? strtotime($item['last_modified'])
	                         : 0;
	               $wc_ts  = strtotime(getDateFromGMT($prod['date_modified_gmt']));

	               if ($wc_ts > $pos_ts) {
	                   $item_id = $this->add_update_item_from_ecommerce_to_phppos($prod, $item);
	               }
	               ...
	               if ($prod['type'] === 'variable') {
	                   $var_page = 1;
	                   do {
	                       $vars = $woocommerce->get(
	                           "products/{$prod['id']}/variations",
	                           ['per_page' => $limit, 'page' => $var_page]
	                       );
	                       ...
	                       foreach ($vars as $v) {
	                           // $v is also an array
	                           $qv = $this->db->get_where(
	                               'item_variations',
	                               ['ecommerce_variation_id' => $v['id']],
	                               1
	                           );
	                           $vr = $qv->row_array();
	                           $qv->free_result();
	                           ...
	                           $pos_var_ts = isset($vr['last_modified'])
	                                         ? strtotime($vr['last_modified'])
	                                         : 0;
	                           $wc_var_ts  = strtotime($v['date_modified_gmt'] . '+00:00');

	                           if ($wc_var_ts > $pos_var_ts) {
	                               $this->add_update_item_variation_from_ecommerce_to_phppos(
	                                   $item ?? [],
	                                   $v,
	                                   $vr
	                               );
	...
	function get_attributes($use_cache = TRUE)
	{
		...
		foreach($attributes as $woo_attr)
		{
			...
			{
				$item_attr_data = array('name' => myucwords(mystrtolower($woo_attr['name'])));
   ...
   function convert_currency_value($currency_value, $exchange_rate) {
      return round(
         (float)$currency_value / (float)$exchange_rate,
	...
	public function update_inventory_from_sale($order,$reverse_inventory=false, $from_ecommerce=false)
	{
		...
		if($sale_info['ecommerce_status'] == 'refunded' || $sale_info['ecommerce_status'] == 'cancelled') {
			...
			if ($reverse_inventory && $sale_id)
			{
				...
				$this->Sale->delete($sale_id, false, !$from_ecommerce, false);
				$this->db->where('sale_id',$sale_id);
