...
{	
	function insert($inventory_data)
	{
		if(is_numeric($inventory_data['trans_inventory']))
		{
			...
			$ret = $this->avoid_unnecessary_inventory_log_records($inventory_data);
			if($ret){
				return TRUE;
			}
	...
	private function avoid_unnecessary_inventory_log_records($inventory_data){
		$this->db->from('inventory');
		if(isset($inventory_data['item_variation_id']) && $inventory_data['item_variation_id']){
			$this->db->where('item_variation_id', $inventory_data['item_variation_id']);
		}
		$this->db->where('trans_items', $inventory_data['trans_items']);
		$this->db->where('location_id', $inventory_data['location_id']);

		$this->db->order_by("trans_id", "desc");
		$this->db->limit(2);
		$history_list = $this->db->get()->result_array();

		if(count($history_list) > 1){
			$before_edit = 0;
			foreach($history_list as $index => $history_item){
				if($index == 0){
					if(
						$history_item['trans_comment'] == $inventory_data['trans_comment']
						&& $history_item['trans_current_quantity'] == ($inventory_data['trans_current_quantity'] - $inventory_data['trans_inventory'])
					){
						$before_edit = 1;
					}else{
						$before_edit = 0;
					...
					if(
						$history_item['trans_comment'] == $inventory_data['trans_comment']
						&& $history_item['trans_current_quantity'] == $inventory_data['trans_current_quantity']
					){
						$before_edit = 1;
					}else{
						$before_edit = 0;
			...
			if($before_edit){
				$unnecessary_inventory_record = $history_list[0];
				return $this->db->delete('inventory', array('trans_id' => $unnecessary_inventory_record['trans_id']));
			}
		}
		return false;
	}
	
	function external_inventory_update($inventory_data)
	{
		if(is_numeric($inventory_data['trans_inventory']))
		{
			if (!isset($inventory_data['trans_date']))
			...
			if ($this->config->item('ecommerce_realtime') && $this->config->item('ecommerce_platform') && in_array('sync_inventory_changes', $ecommerce_cron_sync_operations_settings))
			{
