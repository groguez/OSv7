...
class Customer extends Person
{
	...
	*/
	function get_all($location_id = '',$deleted = 0,$limit=10000, $offset=0,$col='last_name',$order='asc')
	{
		...
		if ($location_id)
		{
			$location_where = 'and ( location_id = '.$location_id.' OR location_id IS NULL) ';
		}
	...
	function count_all($location_id = '',$deleted = 0)
	{
		...
		if ($location_id)
		{
			...
			$this->db->or_where('location_id is NULL');
			$this->db->group_end();
		}
	...
	*/
	...
	{
		$new_customer_action = $customer_id == -1 || $customer_id === false || $customer_id === NULL;
	...
	*/
	...
	function get_customer_search_suggestions($search,$deleted = 0,$limit=25)
	{
		...
		$suggestions = array();
			...
			$this->db->from('customers_phone_numbers');
			$this->db->join('customers','customers_phone_numbers.phone_customer_id=customers.person_id');	
			$this->db->join('people','customers.person_id=people.person_id');	
			$this->db->where('customers.deleted',$deleted);
			$this->db->where("(location_id IS NULL or location_id = $current_location)");	
			$this->db->like("additional_phone_number",alphanumplus($search),$this->config->item('customer_allow_partial_match') ? 'both' : 'after');
			$this->db->limit($limit);
			$by_addt_numbers = $this->db->get();
		
		
			$temp_suggestions = array();
		
			foreach($by_addt_numbers->result() as $row)
			{
				$data = array(
						'name' => format_phone_number($row->additional_phone_number),
						'email' => $row->email,
						'avatar' => $row->image_id ?  cacheable_app_file_url($row->image_id) : base_url()."assets/img/user.png" 
						);
	...
	*/
	function search($search, $location_id = '', $deleted = 0, $limit = 20, $offset = 0, $column = 'last_name', $orderby = 'asc', $search_field = NULL)
	{
	    ...
	    if ($search)
	    {
	        ...
	        else
	        {
	            ...
	            $this->db->where("(first_name LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$search_escaped."%' or 
	            last_name LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$search_escaped."%' or 
	            email LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$search_escaped."%' or 
	            phone_number LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').(!alphanumplus($search) ? '1=2'  : $this->db->escape_like_str(alphanumplus($search)))."%' or 
	            full_name LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$search_escaped."%' or 
	            customers.person_id IN (SELECT phone_customer_id FROM phppos_customers_phone_numbers WHERE additional_phone_number LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').(!alphanumplus($search) ? '1=2'  : $this->db->escape_like_str(alphanumplus($search)))."%')) and deleted=$deleted");
	        }
	    }
	    else
	    {
		...
		elseif ($search)
		{
			...
			$this->db->where("($custom_fields or account_number LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$this->db->escape_like_str($search)."%' or company_name LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$this->db->escape_like_str($search)."%') and deleted=$deleted");
		}
		else
		{
			$this->db->where('customers.deleted', $deleted);
	...
	function search_count_all($search, $location_id = '',$deleted = 0,$limit=10000,$search_field = NULL)
	{
		...
		$this->db->select('COUNT(DISTINCT phppos_people.person_id) as count');
		$this->db->from('customers');
		$this->db->join('people', 'customers.person_id=people.person_id');
	    $this->db->join('price_tiers', 'customers.tier_id=price_tiers.id', 'left');
		...
		$result = $this->db->query($people_search." UNION ".$customer_search)->result_array();
	...
	function get_displayable_columns()
	{
		$columns = array(
			...
			'create_date' =>							array('sort_column' => 'create_date','label' => lang('common_created_at'),'format_function' => 'date_as_display_date'),			
		);
	...
	public function get_additional_phone_numbers($customer_id) 
	{
	        $this->db->select('additional_phone_number');
	        $this->db->from('customers_phone_numbers');
	        $this->db->where('phone_customer_id', $customer_id);
	        $this->db->order_by('id');
	        $query = $this->db->get();

	        $phone_numbers = array();
	        foreach ($query->result() as $row) {
	            $phone_numbers[] = $row->additional_phone_number;
	        }

	        return $phone_numbers;
	  }
	  
	  public function save_additional_phone_numbers($customer_id, $phone_numbers) 
	  {
		  if(empty($phone_numbers))
		  {
		  	$phone_numbers = array();
		  }
		     //Standerized phone numbers
		  	  $phone_numbers = array_map('alphanumplus', $phone_numbers);
			  
	          // Get current phone numbers from the database
	          $existing_numbers = $this->get_additional_phone_numbers($customer_id);

	          // Determine numbers to add and numbers to delete
	          $numbers_to_add = array_diff($phone_numbers, $existing_numbers);
	          $numbers_to_delete = array_diff($existing_numbers, $phone_numbers);

	          // Delete removed phone numbers
	          if (!empty($numbers_to_delete)) {
	              $this->db->where('phone_customer_id', $customer_id);
	              $this->db->where_in('additional_phone_number', $numbers_to_delete);
	              $this->db->delete('customers_phone_numbers');
	          }

	          // Add new phone numbers
	          foreach ($numbers_to_add as $number) 
			  {	  
				  if ($number)
				  {
	              	  $data = array(
	              	...
	              	$this->db->insert('customers_phone_numbers', $data);
				}
			  }
 ...
 public function get_all_phone_numbers($customer_id)
 {
 $phone_numbers = array();
 $cust_info = $this->get_info($customer_id);
			  
			  if ($cust_info->phone_number)
			  {
			  	$phone_numbers[] = $cust_info->phone_number;
			  }
			  
 return array_merge($phone_numbers,$this->get_additional_phone_numbers($customer_id));
 }

/**
 * Update customer tier automatically based on lifetime sales
 *
 * @param int $customer_id
 */
public function update_auto_tier($customer_id)
{
$this->load->model('Sale');
$lifetime_total = $this->Sale->get_customer_total_spent($customer_id);

$this->db->from('auto_tier_ranges');
$this->db->order_by('min_sales','asc');
$ranges = $this->db->get()->result_array();

foreach($ranges as $range)
{
if($lifetime_total >= $range['min_sales'] && $lifetime_total <= $range['max_sales'])
{
$this->db->where('person_id',$customer_id);
$this->db->update('customers', array('tier_id' => $range['tier_id']));
break;
}
}
}
