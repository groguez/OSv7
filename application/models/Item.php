...
class Item extends MY_Model
{
	...
	*/
	...
	function get_displayable_columns()
	{
		...
		$return  = array(
			...
			'ecommerce_shipping_class_id' => 					array('sort_column' => 'ecommerce_shipping_class_id','label' => lang('items_ecommerce_shipping_class')),
			...
			'disable_from_price_rules'  => 			array('sort_column' => 'disable_from_price_rules','label' => character_limiter(lang('common_disable_from_price_rules'),38),'format_function' => 'boolean_as_string'),		
                        'disable_related_sales_prompt'  => array('sort_column' => 'disable_related_sales_prompt','label' => lang('items_dont_prompt_on_sales'),'format_function' => 'boolean_as_string'),
                        'disable_related_receivings_prompt'  => array('sort_column' => 'disable_related_receivings_prompt','label' => lang('items_dont_prompt_on_receivings'),'format_function' => 'boolean_as_string'),
	...
	*/
	function get_all($deleted=0,$limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		if ($attribute_count > 0 && !$this->config->item('dont_show_images_in_search_suggestions') && !$this->config->item('speed_up_search_queries'))
		{
			...
			$phppos_items_tags = $this->db->dbprefix('items_tags');
			$phppos_location_ban_tags = $this->db->dbprefix('location_ban_tags');
			...
			FROM (
			    ...
			    FROM `$phppos_items`
			        ...
			        " LEFT JOIN `$phppos_location_ban_items`
			          ...
			          LEFT JOIN `$phppos_location_ban_tags` 
			            ON `$phppos_location_ban_tags`.`tag_id` = `$phppos_items_tags`.`tag_id` AND `$phppos_location_ban_tags`.`location_id` = $location_id
			          WHERE `$phppos_location_ban_items`.`item_id` IS NULL AND `$phppos_location_ban_tags`.`tag_id` IS NULL":" ")
		...
		else
		{
			...
			else
			{
				...
				$location_ban_tags = $this->db->dbprefix('location_ban_tags');
			...
			if(!$see_all_items)
			{
			    // Join with location_ban_items to exclude items banned at the location
			    $this->db->join('location_ban_items', 'location_ban_items.item_id = items.item_id and location_ban_items.location_id = '.$current_location, 'left');
    
			    // Join with items_tags to get the tags associated with each item
			    $this->db->join('items_tags', 'items_tags.item_id = items.item_id', 'left');
    
			    // Join with location_ban_tags to identify banned tags at the location
			    $this->db->join('location_ban_tags', 'location_ban_tags.tag_id = items_tags.tag_id and location_ban_tags.location_id = '.$current_location, 'left');
			}
			if (!$this->config->item('speed_up_search_queries'))
			{
			...
			if(!$see_all_items)
			{
			    // Exclude items that are banned at the location
			    $this->db->where('location_ban_items.item_id IS NULL');
    
			    // Exclude items that have any tags banned at the location
			    $this->db->where('location_ban_tags.tag_id IS NULL');
			}
	...
	function get_all_by_category($category_id, $hide_out_of_stock_grid = FALSE, $offset=0, $limit = 14,$show_hidden=FALSE)
	{
		...
		if (!$hide_out_of_stock_grid)
		{
			...
			(
				...
				WHERE 
					...
					GROUP BY $items_table.item_id ORDER BY name, size
			) 
			UNION ALL
			(
		...
		else
		{
			...
			(
				...
				GROUP BY item_id ORDER BY name, size
			) 
			UNION ALL 
			(
				SELECT CONCAT('KIT ',$item_kits_table.item_kit_id), unit_price, name, '', main_image_id as image_id 
				...
				ORDER BY name
			) ORDER BY name, size LIMIT $offset, $limit");
		}
		return $result;
	}
	...
	*/
	function get_info($item_id, $can_cache = TRUE)
	{
		...
		else
		{
			...
			//Get all the fields from items table
			$fields = array('weight_unit','ecommerce_inventory_item_id','barcode_name','info_popup','name','category_id','supplier_id','manufacturer_id','item_number','product_id','ecommerce_product_id','ecommerce_product_quantity','description','size','tax_included','cost_price','unit_price','promo_price','start_date','end_date','reorder_level','expire_days','item_id','allow_alt_description','is_serialized','override_default_tax','is_ecommerce','is_service','is_ebt_item','commission_percent','commission_percent_type','commission_fixed','change_cost_price','disable_loyalty','deleted','last_modified','ecommerce_last_modified','tax_class_id','replenish_level','system_item','max_discount_percent','max_edit_price','min_edit_price','custom_field_1_value','custom_field_2_value','custom_field_3_value','custom_field_4_value','custom_field_5_value','custom_field_6_value','custom_field_7_value','custom_field_8_value','custom_field_9_value','custom_field_10_value','required_age','verify_age','weight','length','width','height','ecommerce_shipping_class_id','long_description','allow_price_override_regardless_of_permissions','main_image_id','only_integer','is_series_package','series_quantity','series_days_to_use_within','is_barcoded','item_inactive','default_quantity','disable_related_sales_prompt','disable_related_receivings_prompt','is_favorite','loyalty_multiplier');
					...
					case 'item_variation_item_number':
                            ...
                            case 'product_id':
                                                ...
                                                $result = $this->lookup_item_by_product_id($item_identifer);
                                                        ...
                                                        break;
                            case 'ecommerce_product_id':
                                                        if (!in_array('ecommerce_product_id',$skip_lookup))
                                                        {
                                                $result = $this->lookup_item_by_ecommerce_product_id($item_identifer);
                                }
	...
	private function lookup_item_by_item_variation_item_number_quantity_unit($item_identifer)
	{
       ...
       public function lookup_item_by_ecommerce_product_id($ecommerce_product_id)
       {
               $this->db->from('items');
  ...
  function add_variation_data(&$suggestions, $search, $price_field)
  {
	...
	*/
	...
	function search($search, $deleted=0,$category_id = false, $limit=20, $offset=0, $column='name', $orderby='asc', $fields = 'all')
	{
		...
		if($see_all_items){
			...
			// Define table names with database prefixes
			$phppos_items = $this->db->dbprefix('items');
			$phppos_location_ban_items = $this->db->dbprefix('location_ban_items');
			$phppos_items_tags = $this->db->dbprefix('items_tags');
			$phppos_location_ban_tags = $this->db->dbprefix('location_ban_tags');

			// Update the LEFT JOIN clause to include tag banning logic
			$location_ban_item_list_query_left_join = "
			    LEFT JOIN `$phppos_location_ban_items` 
			        ON `$phppos_items`.`item_id` = `$phppos_location_ban_items`.`item_id` 
			        AND `$phppos_location_ban_items`.`location_id` = $location_id
			    LEFT JOIN `$phppos_items_tags` 
			        ON `$phppos_items_tags`.`item_id` = `$phppos_items`.`item_id`
			    LEFT JOIN `$phppos_location_ban_tags` 
			        ON `$phppos_location_ban_tags`.`tag_id` = `$phppos_items_tags`.`tag_id` 
			        AND `$phppos_location_ban_tags`.`location_id` = $location_id
			";

			// Update the WHERE clause to exclude items banned by tags
			$location_ban_item_list_query_where = "
			    AND `$phppos_location_ban_items`.`location_id` IS NULL 
			    AND `$phppos_location_ban_tags`.`tag_id` IS NULL
			";		
		}
		...
		{		
			...
			$this->db->from('items');
			$this->db->join('tax_classes', 'tax_classes.id = items.tax_class_id', 'left');
			// Apply ban logic when not seeing all items
			if(!$see_all_items)
			{
			    // Join with location_ban_items to identify items banned at the location
			    ...
			    // Join with items_tags to get tags associated with items
			    $this->db->join('items_tags', 'items_tags.item_id = items.item_id', 'left');
    
			    // Join with location_ban_tags to identify tags banned at the location
			    $this->db->join('location_ban_tags', 'location_ban_tags.tag_id = items_tags.tag_id and location_ban_tags.location_id = '.$location_id, 'left');
			}
	...
	*/
	function get_sale_price(array $params)
	{
		...
		$category_tier_row = NULL;
		if($item_info->category_id > 0 && $tier_id > 0 && !$item_tier_row){
			$item_category_id = $item_info->category_id;

			$search_category_level = 1;
			while ($search_category_level > 0 && $search_category_level < 100 && $category_tier_row == NULL) {
				if($this->Category->tier_exists($tier_id, $item_category_id)){
					$category_tier_row = $this->Category->get_tier_price_row($tier_id, $item_category_id);
				}else{
					$item_category_info = $this->Category->get_info($item_category_id);
					if($item_category_info->parent_id > 0){
						$item_category_id = $item_category_info->parent_id;
					}else{
						$search_category_level = -1;
						break;
					}
		...
		if ($quantity_unit_id)
		{
			...
			{
				// Extract unit price, cost price, and set default quantity
				$item_unit_price = $qui->unit_price;
				$item_cost_price = $qui->cost_price ?? 0;
				$quantity_unit_quantity = 1;

				// Check tier-specific pricing adjustments
				if (!empty($tier_info->default_percent_off)) {
					$adjusted_price = $item_unit_price * (1 - ($tier_info->default_percent_off / 100));
				} elseif (!empty($tier_info->default_cost_plus_percent)) {
					$adjusted_price = $item_cost_price * (1 + ($tier_info->default_cost_plus_percent / 100));
				} elseif (!empty($tier_info->default_cost_plus_fixed_amount)) {
					$adjusted_price = $item_cost_price + $tier_info->default_cost_plus_fixed_amount;
				} else {
					$adjusted_price = $item_unit_price = $qui->unit_price;
				}

				// Return the adjusted price with the specified rounding
				$decimal_precision = $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10;
				return to_currency_no_money($adjusted_price * $quantity_unit_quantity, $decimal_precision);
			}
			else
			{	
		...
		elseif (!empty($category_tier_row) && $category_tier_row->unit_price)
		{
			return to_currency_no_money($category_tier_row->unit_price*$quantity_unit_quantity, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($category_tier_row) && $category_tier_row->percent_off)
		{
			if (($variation_info && $variation_info->unit_price )|| ($variation_location_info && $variation_location_info->unit_price))
			{
				$item_unit_price = (double)$variation_location_info->unit_price ?  $variation_location_info->unit_price : $variation_info->unit_price;
			...
			return to_currency_no_money(($item_unit_price *(1-($category_tier_row->percent_off/100)))*$quantity_unit_quantity, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($category_tier_row) && $category_tier_row->cost_plus_percent)
		{
			if ($variation_info && $variation_info->cost_price)
			{
				$item_cost_price = $variation_info->cost_price;
			...
			return to_currency_no_money(($item_cost_price *(1+($category_tier_row->cost_plus_percent/100)))*$quantity_unit_quantity, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($category_tier_row) && $category_tier_row->cost_plus_fixed_amount)
		{
			if ($variation_info && $variation_info->cost_price)
			{
				$item_cost_price = $variation_info->cost_price;
			...
			return to_currency_no_money(($item_cost_price + $category_tier_row->cost_plus_fixed_amount)*$quantity_unit_quantity, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		...
		else
		{
			...
			else
			{
				...
				if ($this->config->item('allow_zero_price_location_price_overrides'))
				{
					if ($item_location_info->unit_price !== NULL && $item_location_info->unit_price !== '')
					{
						$item_unit_price = (double) $item_location_info->unit_price;
					}
					else
					{
						$item_unit_price = (double)$item_info->unit_price;
					}
				}
				else
				{
