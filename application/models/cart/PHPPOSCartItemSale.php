...
class PHPPOSCartItemSale extends PHPPOSCartItem
{
	...
	public function __construct(array $params = array())
	{
		...
		$this->sold_by_employee_id_2 = isset($params['sold_by_employee_id_2']) && $params['sold_by_employee_id_2'] ? $params['sold_by_employee_id_2'] : null;
	...
	function get_price_for_item()
	{
		...
		if (isset($this->quantity_unit_id)) {
			$params['quantity_unit_quantity'] = $quantity_unit_quantity;
			$params['quantity_unit_id'] = $this->quantity_unit_id;
		}
		
		return $CI->Item->get_sale_price($params);
	}	
	...
	function will_be_out_of_stock($additional_quantity)
	{
		...
		elseif (!$CI->Item->has_variations($item_id))
		{
			$item_location_quantity = $CI->Item_location->get_location_quantity($item_id);
		...
		{
			//Set defaults for when we are adding an item that HAS variations but first added the item itself and THEN choose variation
			$item_location_quantity = PHP_INT_MAX;
			$quanity_added = 0;
		}
			...
			if($item_location_quantity === NULL){
				$item_location_quantity = 0;
			}
		}
	...
	public function get_subtotal($sale_id = FALSE)
	{
	    ...
	    return $this->flat_discount_amount > 0 ? to_currency_no_money($this->get_modifiers_subtotal_flat_discount_including_tax() + ($price_to_use - $this->flat_discount_amount) * $this->quantity, 10) : to_currency_no_money(($this->get_modifiers_subtotal_including_tax() - ($this->get_modifiers_subtotal_including_tax() * $this->discount/100)) + ($price_to_use*$this->quantity-$price_to_use*$this->quantity*$this->discount/100),10);
		}
		else
		{
			$price_to_use = $this->unit_price;				
    	return $this->flat_discount_amount > 0 ? to_currency_no_money($this->get_modifiers_subtotal_flat_discount($this->flat_discount_amount) + ($price_to_use - $this->flat_discount_amount) * $this->quantity) : to_currency_no_money(($this->get_modifiers_subtotal() - ($this->get_modifiers_subtotal() * $this->discount / 100)) + ($price_to_use*$this->quantity-$price_to_use*$this->quantity*$this->discount/100));
		}
	}
}