...
abstract class PHPPOSCartItemBase
{
	...
	public $line_item_custom_field_1_value;
	public $line_item_custom_field_2_value;
	public $line_item_custom_field_3_value;
	public $line_item_custom_field_4_value;
	public $line_item_custom_field_5_value;
	public $line_item_custom_field_6_value;
	public $line_item_custom_field_7_value;
	public $line_item_custom_field_8_value;
	public $line_item_custom_field_9_value;
	public $line_item_custom_field_10_value;
	...
	public function get_taxes($cumulative_percent = 0,$taxable_subtotal = NULL)
	{
	    ...
	    $CI =& get_instance();
		if(property_exists($this,'is_repair_item') && $this->is_repair_item == 1 && $CI->config->item('work_repair_item_taxable') == "0"){
			return $taxes;
		}
	...
	function get_modifiers_subtotal_flat_discount($flat)
	{
		$subtotal = 0;
		
		foreach($this->modifier_items as $mod_item)
		{
			$subtotal+=$this->quantity*($mod_item['unit_price'] - $flat);
		}
	...
	function get_modifiers_subtotal_flat_discount_including_tax($flat = 0)
	{
		$subtotal = 0;
		...
		foreach($this->modifier_items as $mod_item)
		{
			...
			$subtotal+=$this->quantity*($unit_price-$flat);
		}
	...
	function get_modifiers_profit_flat_discount($flat)
	{
		$profit = 0;
		
		foreach($this->modifier_items as $mod_item)
		{
			$profit+=($mod_item['unit_price'] - $mod_item['cost_price'] - $flat)*$this->quantity;
		}
	...
	{
		return $this->flat_discount_amount > 0 ? to_currency_no_money($this->get_modifiers_subtotal_flat_discount($this->flat_discount_amount) + ($this->unit_price - $this->flat_discount_amount)*$this->quantity) : to_currency_no_money(($this->get_modifiers_subtotal() - ($this->get_modifiers_subtotal() * $this->discount / 100))+ ($this->unit_price*$this->quantity-$this->unit_price*$this->quantity*$this->discount/100));
	}	
	...
	public function get_profit()
	{
		...
		if ($this->get_id() != $store_account_item_id || get_class($this) == 'PHPPOSCartItemKitSale')
		{
		...
		return $this->flat_discount_amount > 0 ? to_currency_no_money($this->get_modifiers_profit_flat_discount($this->flat_discount_amount) + ($this->unit_price - $this->flat_discount_amount)*$this->quantity - $item_cost_price*$this->quantity) : to_currency_no_money($this->get_modifiers_profit() + (($this->unit_price*$this->quantity-$this->unit_price*$this->quantity*$this->discount/100) - ($item_cost_price*$this->quantity)));
	}
