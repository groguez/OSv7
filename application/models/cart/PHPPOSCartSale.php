...
{		
	...
	public static function get_instance_from_sale_id($sale_id, $cart_id = NULL, $is_editing_previous = FALSE, $copy_price_rules = TRUE, $is_api = FALSE){
		...
		if(isset($work_order_info['sale_id'])){
			$cart->is_work_order = 1;
		}
		$cart->return_sale_id = $sale_info['return_sale_id'];
		$cart->return_reason = $sale_info['return_reason'];
		$customer_info = $CI->Sale->get_customer($sale_id);
		$cart->customer_id = $customer_info? $customer_info->person_id : null;
		...
		if($is_api){
			$cart->employee_id = $sale_info['employee_id'];
			$cart->location_id = $sale_info['location_id'];
		}

		if($cart->customer_id){
			if($CI->Invoice->exist_sale_invoice('customer', $sale_info['location_id'], $cart->customer_id, $sale_id)){
				$cart->create_invoice = 1;
			}
		}
		...
		foreach($CI->Sale->get_sale_items($sale_id)->result() as $row)
		{
			...
			$item_props['weight_unit'] = $cur_item_info->weight_unit;
			$item_props['length'] = $cur_item_info->length;
			$item_props['width'] = $cur_item_info->width;
			$item_props['height'] = $cur_item_info->height;
			...
			$item_props['sold_by_employee_id_2'] = $row->sold_by_employee_id_2;
			...
			if ($item_props['tax_included'] && $cart->is_editing_previous 
				&& ($item_props['taxable'] || $customer_info->taxable == 0 || ( $item_props['is_repair_item'] == 1 && !$CI->config->item('work_repair_item_taxable'))))
			{
				$CI->load->helper('items');
			...
			} else if ($item_props['tax_included'] && !$cart->is_editing_previous && $customer_info && $customer_info->taxable === '0' ){
				$item_props['regular_price'] = get_price_for_item_excluding_taxes($row->item_id, $item_props['regular_price']);
			}
			...
			$item_props['flat_discount_amount'] = $row->flat_discount_amount;
			...
			for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
			{
				$item_props["line_item_custom_field_${k}_value"] = $row->{"custom_field_${k}_value"};
			}
			...
			{
				if (is_array(unserialize($row->override_taxes)))
				{
				...
				{
					$item->override_tax_class = unserialize($row->override_taxes);
				}
			}
			
			if (count($item->get_taxes()) == 0)
			{
				if($item->tax_included == 1 && $customer_info && $customer_info->taxable == 0){
		...
		foreach($CI->Sale->get_sale_item_kits($sale_id)->result() as $row)
		{
			...
			$item_kit_props['is_repair_item'] = $row->is_repair_item;
			...
			$item_kit_props['sold_by_employee_id_2'] = $row->sold_by_employee_id_2;
			...
			if ($item_kit_props['tax_included'] && $cart->is_editing_previous && ($item_kit_props['taxable'] || $customer_info->taxable == 0))
			{
				$CI->load->helper('item_kits');
			...
			$item_kit_props['flat_discount_amount'] = $row->flat_discount_amount;
			...
			for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
			{
				$item_kit_props["line_item_custom_field_${k}_value"] = $row->{"custom_field_${k}_value"};
			}
			...
			if ($row->override_taxes)
			{
				...
				{
					$item_kit->override_tax_class = unserialize($row->override_taxes);
				}
			}
		...
		if ($cart->is_work_order == 1)
		{
			if($CI->config->item('take_inventory_out_of_system_when_creating_updating_work_order'))
			{
				$cart->suspended = 1; //layaway
			}
			else
			{
				$cart->suspended = 2; //estimate
			}
		}
		...
		$override_taxes = $CI->Sale->get_override_taxes($sale_id);
		
		if ($override_taxes)
		{
			if (is_array($override_taxes))
			{
				$cart->set_override_taxes($override_taxes);
			
			}
			else
			{
				$cart->override_tax_class = $override_taxes;
			}
		}
	...
	function setup_defaults()
	{
		...
		$this->has_delivery = FALSE;
                $CI =& get_instance();
                $this->selected_tier_id = $CI->config->item('default_tier_id') ? $CI->config->item('default_tier_id') : 0;
	...
	public function __toString()
	{
		...
		{
			if (!$CI->config->item('remove_customer_name_from_receipt')) {
		...
		if ( $CI->Location->get_info_for_key('twilio_sms_from'))
		{
			...
			$hashids = new Hashids\Hashids($CI->db->database);
	...
	public function to_array()
	{
		...
		$data['is_work_order'] = $this->is_work_order;
		if($customer_id)
		{
			$data['customer_id'] = $customer_id;
			$cust_info=$CI->Customer->get_info($customer_id);
			
			$title = $CI->Person->get_title_as_string($customer_id);
			...
			$data['customer_taxable'] = $cust_info->taxable;
	...
	public function get_subtotal($keep_decimal = 0)
	{
		$CI =& get_instance();
		...
		$subtotal = 0;
		$items_being_repaired = array();
		$work_order_info = null;
		if( $CI->config->item("work_order_all_warranty_repairs_are_free") && $this->is_work_order == 1 ){
			$work_order_info = $CI->Work_order->get_info_by_sale_id($this->sale_id)->row_array();
			$items_being_repaired = $CI->Work_order->get_work_order_items($work_order_info['id'], 1);
		}

		foreach($this->get_items() as $line => $item)
		{
			if($CI->config->item("work_order_all_warranty_repairs_are_free") && $this->is_work_order == 1 && $work_order_info['warranty'] == 1){
				$belong_items_being_repaired = 0;
				foreach($items_being_repaired as $item_bening_repaired){
					if($item->item_id == $item_bening_repaired['item_id']){
						$belong_items_being_repaired = 1;
						break;
					}
				}
				if($belong_items_being_repaired == 1) continue; // calculate 0
			}
			//If we are looking up a previous sale but not editing it the price is already exclusive of tax	
			if ($this->get_previous_receipt_id() && !$this->is_editing_previous)
			{
			...
			if ($item->tax_included)
			{
				...
				{
					$subtotal+=$item->flat_discount_amount > 0 ? bankers_round(($price_to_use-$item->flat_discount_amount)*$item->quantity) : (bankers_round($price_to_use*$item->quantity-bankers_round($price_to_use*$item->quantity*$item->discount/100)));
				}
				else
				{
					$subtotal+=$item->flat_discount_amount > 0 ? ($price_to_use-$item->flat_discount_amount)*$item->quantity : ($price_to_use*$item->quantity-$price_to_use*$item->quantity*$item->discount/100);
				}
			}
			else
			{
	...
	function get_total($keep_decimal = 0)
	{
		$CI =& get_instance();
		...
		if (!$customer->taxable && $customer_id){
			if($this->is_tax_inclusive()){
				$total = $this->get_subtotal(1);
			}else{
				$total = $this->get_subtotal();
			}
		}else{
			if($this->is_tax_inclusive()){
				$total = $this->get_subtotal(1)+$this->get_tax_total_amount(1);
			}else{
	...
	*/
	function determine_new_prices_for_tier_change()
	{
		...
		foreach ($items as $line=>$item )
		{
			if (get_class($item) == 'PHPPOSCartItemSale')
			{
				...
				{
					// Prepare parameters for fetching the price
					$param = [];
					...
					// Include quantity unit ID if it exists
					if (!empty($item->quantity_unit_id)) {
						$param['quantity_unit_id'] = $item->quantity_unit_id;
					}
					$previous_price = $item->get_price_for_item($param);
	...
	function change_credit_card_payments_to_partial()
	{
		...
		{
			//If we have a credit payment that has been processed, change it to partial credit card so we can process again
	...
	function process_barcode_scan($barcode_scan_data,$options = array())
	{
		...
		if (isset($CI->view_data['success']) && $CI->view_data['success'])
		{
			...
			if(isset($all_items[count($all_items) - 1]))
			{
			 	$last_item = $all_items[count($all_items) - 1];
                  ...
                  {
                                  $do_not_allow_below_cost = $CI->Location->get_info_for_key('do_not_allow_below_cost');
                                  if ($do_not_allow_below_cost === '' || $do_not_allow_below_cost === NULL)
                                  {
                                          $do_not_allow_below_cost = $CI->config->item('do_not_allow_below_cost');
                                  }
		...
		else
		{
			...
			$suggestions = $CI->Price_rule->search_coupons($barcode_scan_data);
			$result = array();
			if ($suggestions)
			{
				$current_codes = $this->get_coupons();
			}
			foreach ($suggestions as $k=>$v) 
			{
				if(!empty($v['coupon_code']))
				{
					if ($current_codes)
					{
						$search_key = array_search($v['value'], array_column($current_codes, 'value'));
					}
					else
					{
						$search_key = FALSE;
					...
					$CI->view_data['error'] = NULL;
					
					$this->set_coupons($current_codes);
					return;
				}
			}		
		}
	...
	private function do_add_cart_item_kit(PHPPOSCartItemKitSale $item_kit_to_add, $options = array())
	{
		...
		{
	        // Dynamically set cost price if dynamic_cost_price is enabled
	        if ($item_kit_to_add->dynamic_cost_price) 
			{
	            $item_kit_to_add->cost_price = $item_kit_to_add->get_cost_price_for_kit();
	        }
        
	        // Dynamically set selling price if dynamic_unit_price is enabled
	        if ($item_kit_to_add->dynamic_unit_price) 
			{
	...
	private function do_validate_and_add_cart_item(PHPPOSCartItemSale $item_to_add,$options = array(), $secondary_supplier_id=null)
	{
		...
		if ($item_to_add->serialnumber && $this->is_serial_number_in_cart($item_to_add->serialnumber))
		{
			...
			return FALSE;			
                ...
                {
                        $do_not_allow_out_of_stock = $CI->Location->get_info_for_key('do_not_allow_out_of_stock_items_to_be_sold');
                        if ($do_not_allow_out_of_stock === '' || $do_not_allow_out_of_stock === NULL)
                        {
                                $do_not_allow_out_of_stock = $CI->config->item('do_not_allow_out_of_stock_items_to_be_sold');
                        }
		...
		if($secondary_supplier_id && !$item_to_add->variation_id){
			...
			$item_to_add->cart_line_supplier_id = $item_details->supplier_id;
                ...
                $CI->load->model('Item_related');
                if(!$item_details->disable_related_sales_prompt)
                {
                        $related_items = $CI->Item_related->get_related_items($item_id)->result();
                        if(count($related_items))
                        {
                                foreach($related_items as $related_item)
                                {
                                        $CI->view_data['related_items'][] = $CI->Item->get_info($related_item->related_item_id);
                                }
                        }
                }
		...
		foreach($this->get_items() as $item)
		{
			...
			else
			{
				...
				if ($item->tax_included)
				{
					...
					if (property_exists($item, 'flat_discount_amount') && $item->flat_discount_amount > 0){
						$total_discount += $item->flat_discount_amount * $item->quantity;
					}
				}
				else
				{
