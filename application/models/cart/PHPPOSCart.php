...
{	
	...
	public function merge_item($item_merge_from, $item_merge_into)
	{
		...
		foreach($this->get_items($CI->config->item('always_put_last_added_item_on_top_of_cart') ? NULL : get_class($item_merge_into)) as $item)
		{
			...
			{
				if($this->get_mode() == 'sale'){
					$merge_quantity = $item_merge_into->quantity + $item_merge_from->quantity;
					if(get_class($item) == 'PHPPOSCartItemKitSale'){
						$CI->load->model('Item_kit');
						$CI->load->model('Item_kit_location');
						
						$cur_item_kit_location_info = $CI->Item_kit_location->get_info($item->item_kit_id);
						if (isset($cur_item_location_info->limited_sales_quantity) && $cur_item_location_info->limited_sales_quantity > 0 && $merge_quantity > $cur_item_location_info->limited_sales_quantity) {
							$CI->view_data['error'] = lang('items_kit_quantity_has_exceeded_the_limited_sales_quantity');
							return FALSE;
						}
						
						
						$cur_item_kit_info = $CI->Item_kit->get_info($item->item_kit_id);
						if($cur_item_kit_info->limited_sales_quantity > 0 && $merge_quantity > $cur_item_kit_info->limited_sales_quantity){
							$CI->view_data['error'] = lang('items_kit_quantity_has_exceeded_the_limited_sales_quantity');
							return FALSE;
						}
					}else{
						if($item->variation_id > 0){
							...
							$cur_item_variation_location_info = $CI->Item_variation_location->get_info($item->variation_id);
							if($cur_item_variation_location_info->limited_sales_quantity > 0 && $merge_quantity > $cur_item_variation_location_info->limited_sales_quantity){
								$CI->view_data['error'] = lang('items_variation_quantity_has_exceeded_the_limited_sales_quantity');
								return FALSE;
							}
							
							$cur_item_variation_info = $CI->Item_variations->get_info($item->variation_id);
							if($cur_item_variation_info->limited_sales_quantity > 0 && $merge_quantity > $cur_item_variation_info->limited_sales_quantity){
								$CI->view_data['error'] = lang('items_variation_quantity_has_exceeded_the_limited_sales_quantity');
								return FALSE;
							}
						}else{
							
							$cur_item_location_info = $CI->Item_location->get_info($item->item_id);
							if (isset($cur_item_location_info->limited_sales_quantity) && $cur_item_location_info->limited_sales_quantity > 0 && $merge_quantity > $cur_item_location_info->limited_sales_quantity) {
								$CI->view_data['error'] = lang('items_quantity_has_exceeded_the_limited_sales_quantity');
								return FALSE;
							}
							
							
							$cur_item_info = $CI->Item->get_info($item->item_id);
							if($cur_item_info->limited_sales_quantity > 0 && $merge_quantity > $cur_item_info->limited_sales_quantity){
								$CI->view_data['error'] = lang('items_quantity_has_exceeded_the_limited_sales_quantity');
								return FALSE;
							}
						}
        ...
        public function delete_item_by_line($line)
        {
                for($index=0; $index< count($this->cart_items); $index++){
                        if(isset($this->cart_items[$index]) && $this->cart_items[$index]->line == $line){
                                unset($this->cart_items[$index]);
                                $this->cart_items = array_values($this->cart_items);
                                if (empty($this->cart_items))
                                {
	...
	function get_payments_total($payment_type = NULL, $keep_decimal = 0)
	{
		$payments_total = 0;
		foreach($this->get_payments() as $payment)
		{
	...
	public function get_amount_due($keep_decimal = 0)
	{
		if($keep_decimal){
			return $this->get_total(1) - $this->get_payments_total(NULL, 1);
		}
		return to_currency_no_money($this->get_total(1) - $this->get_payments_total(NULL, 1));
	}
	...
	function get_tax_total_amount($keep_decimal = 0)
	{
		...
		if($keep_decimal == 1){
			return $total_tax*$exchange_rate;
		}
	...
	function get_taxable_subtotal()
	{
		...
		foreach($this->cart_items as $item)
		{
			...
			foreach(array_values($item->get_taxes()) as $tax_amount)
			{
				$total_line_item_tax+=$tax_amount;
			}
			
			if ($total_line_item_tax)
			{
	...
	public function get_override_taxes()
	{
		...
		if ($this->override_tax_class)
		{
			return $this->override_tax_class;
		}
	...
	public function get_credit_card_payments_charge_ids()
     ...
     public function get_not_processed_cc_payment_ids() 
	 {
         ...
         for ($k = 0; $k < count($payments); $k++) {
             if (($payments[$k]->payment_type == lang('common_credit') || (($CI->Location->get_info_for_key('credit_card_processor') == 'valor' || $CI->Location->get_info_for_key('credit_card_processor') == 'coreclear2') && $payments[$k]->payment_type == lang('common_debit')) || $payments[$k]->payment_type == lang('common_ebt') || $payments[$k]->payment_type == lang('common_ebt_cash')) && !$payments[$k]->ref_no) {
                 $payment_ids[] = $k;
             }
         }
