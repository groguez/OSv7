...
{	
	...
	{				
		...
		if ($this->config->item('sms_mobile_tel_username') && $this->config->item('sms_mobile_tel_password'))
		{
			require_once (APPPATH."libraries/ESMSWS.php");
			

			$username = $this->config->item('sms_mobile_tel_username');
			$password = $this->config->item('sms_mobile_tel_password');

			$session = createSession('',$username,$password,'');

			$message = $text_message;
			$recipients =  array($to_phone_number);
			$alias = $this->config->item('sms_mobile_tel_alias') ?? 'Unknown';

			sendMessagesMultiLang($session,$alias,$message,$recipients,0);
