...
abstract class Ecom extends MY_Model
{
	...
	public function get_tax_classes_from_db()
	{
		...
		foreach($this->Tax_class->get_all_for_ecommerce() as $phppos_tax_class_id => $phppos_tax_class)
		{
			...
			{
				$tax_classes[mystrtoupper($phppos_tax_class['name'])] = $phppos_tax_class['ecommerce_tax_class_id'];
			}
		}
	...
	public function get_tags_from_db()
	{
		...
		foreach($this->Tag->get_all_for_ecommerce() as $phppos_tag_id => $phppos_tag)
		{
			...
			{
				$tags[mystrtoupper($phppos_tag['name'])] = $phppos_tag['ecommerce_tag_id'];
			}
		}
	...
	function get_attribute_ids_from_ecommerce_attribute_id($ecommerce_attribute_id, $item_id = false, $attribute_name = false)
	{
		$this->db->from('attributes');
		...
		$this->db->order_by('attributes.deleted','ASC');
		$return = array();
		foreach($this->db->get()->result_array() as $row)
		{
			$return[] = $row['id'];
	...
	*/
	...
	Gets deleted of item with given ecommerce_product_id
	*/
	function get_sale_deleted_for_ecommerce_order_id($ecommerce_order_id)
	{
		$this->load->model('Sale');
		return $this->Sale->get_sale_deleted_for_ecommerce_order_id($ecommerce_order_id);
	}
	...
	*/
	...
	/*
	Gets ecommerce_image_id
	*/
	function get_ecommerce_image_id_for_item($image_id, $item_id)
	{
		$this->db->select('ecommerce_image_id');
		...
		if ($query->num_rows() == 1)
		{
			return $query->row()->ecommerce_image_id;
		}
	...
	*/
	...
	function link_item($item_id, $ecommerce_product_id, $ecommerce_product_quantity, $ecommerce_last_modified,$ecommerce_inventory_item_id=NULL, $ecommerce_first_variation_id=NULL)
	{
			$this->db->where('item_id', $item_id);
			$this->db->update('items', array(
				...
				'last_modified' => $ecommerce_last_modified,
				'ecommerce_inventory_item_id' => $ecommerce_inventory_item_id,
				'ecommerce_first_variation_id' => $ecommerce_first_variation_id,
			));
	}
	...
	function reset_ecom()
	{
		...
		$this->db->update('items', array(
			...
			'ecommerce_last_modified' => NULL,
			'ecommerce_inventory_item_id' => NULL,
			'ecommerce_first_variation_id' => NULL,
		));
