...
foreach($items as $item)
{
	...
	$image = site_url('qrcodegenerator/index?qrcode='.rawurlencode($qr_code).'&s=qr&scale=2&width=1');
	...
	if(!$this->config->item('hide_expire_date_on_qr_codes') && isset($items_expire[$expire_key]) && $items_expire[$expire_key] && !$this->config->item('hide_name_on_qr_codes'))
	{
		$text.= " (".lang('common_expire_date').' '.$items_expire[$expire_key].')';		
	}
	
	if ($this->config->item('hide_qr_code_on_qr_code_labels'))
	{
		$image = NULL;
	}

	$pdf->AveryQRCodeCell($x, $y, $this->config->item('show_qr_code_company_name') ? $company : '', $image,$text,$qr_code);
	$counter++;
    $y++; // next row
    if($y == 6) // end of page wrap to next column
	 { 
        $x++;
        $y = 0;
...
$pdf->output('qr_code Sheet.pdf',$force_download_pdf ? 'D': 'I');
?>