	...
	<div class="col-md-4 form-horizontal">
		<div class="panel panel-piluku">
			...
			<div class="panel-body">
                                <select name="status_id" id="status_id" class="form-control change_delivery_status">
                                        ...
                                        <option value="0" data-status_value="<?php echo H($default);?>"><?php echo lang('common_default');?></option>
                                        <?php
                                                $statuses = array('' => lang('common_change_status'));
                                                foreach($delivery_statuses as $status_id => $status) { ?>
                                                        <option value="<?php echo H($status_id);?>" data-status_value="<?php echo H(str_ireplace('<br />', "\r\n", $status['data']));?>">
		...
		<div class="panel panel-piluku">
			...
			<div class="panel-body shortcuts">
				...
				<a href="javascript:void(0);" class="add_status" data-value="%sale_total%">
					<?php echo lang('work_orders_sale_total');?>
				</a> <br>
				...
				?>
                                        <a href="javascript:void(0);" class="add_status" data-value="%custom_field_<?php echo H($replace_value);?>%">
                                                Custom <?php echo H($custom_field);?>
                                        </a><br>
                                <?php } } ?>
...
$(document).ready(function()
{
		...
		if (!text_content) {
			...
			preview 		= pre_template+ ' ' +$.trim(status_value);
                        ...
                        preview = text_content
                                        .replaceAll('%company_name%', <?php echo json_encode(H($this->config->item('company')));?>)
                                        .replaceAll('%customer_name%', <?php echo json_encode(H(lang('common_customer_name')));?>)
                                        .replaceAll('%work_order_id%', <?php echo json_encode(H($this->config->item('sale_prefix').' 125'));?>)
                                        .replaceAll('%estimated_parts%', "5263")
                                        .replaceAll('%estimated_labor%', "3410")
                                        .replaceAll('%work_order_status%', 'Work Order Status')
                                        .replaceAll('%estimated_repair_date%', <?php echo json_encode(H(lang('work_orders_estimated_repair_date')));?>)
                                        .replaceAll('%warranty_repair%', <?php echo json_encode(H(lang('work_orders_warranty_repair')));?>)
                                        .replaceAll('%customer_notes%', <?php echo json_encode(H(lang('common_notes')));?>)
					...
					<?php for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) { 
						...
						if($custom_field !== FALSE) {
					?>
                                        .replaceAll(<?php echo json_encode('%'.$replace_value.'%');?>, <?php echo json_encode(H($custom_field));?>)
                                        <?php  } } ?>
                                        .replaceAll('%work_images%', <?php echo json_encode(H(lang('common_images')));?>);

		}
                preview = escapeHtml(preview).replaceAll(/\n/g, "<br />");
	...
	function unrenderTemplate()
	{
		...
		<?php
                $company_name = json_encode(H($this->config->item('company')));
                $customer_name = json_encode(H(lang('common_customer_name')));
                $sale_prefix = json_encode(H($this->config->item('sale_prefix') . ' 125'));
                $lang_estimated_repair_date = json_encode(H(lang('work_orders_estimated_repair_date')));
                $lang_warranty_repair = json_encode(H(lang('work_orders_warranty_repair')));
                $lang_customer_notes = json_encode(H(lang('common_notes')));
                $lang_work_images = json_encode(H(lang('common_images')));
		...
		for ($k = 1; $k <= NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k++) {
		    ...
		    if ($custom_field !== FALSE) {
		        $replace_value = str_replace(' ', '_', strtolower($custom_field));
		        $custom_fields[$custom_field] = '%' . $replace_value . '%';
		    }
		}
		?>

		$(document).ready(function() {
		    var preview = text_content.replaceAll(/<br>/g, "\n").replaceAll(/<br \/>/g, "\n")
		        .replaceAll(<?php echo $company_name; ?>, '%company_name%')
		        .replaceAll(<?php echo $customer_name; ?>, '%customer_name%')
		        .replaceAll(<?php echo $sale_prefix; ?>, '%work_order_id%')
		        ...
		        .replaceAll(<?php echo $lang_estimated_repair_date; ?>, '%estimated_repair_date%')
		        .replaceAll(<?php echo $lang_warranty_repair; ?>, '%warranty_repair%')
		        .replaceAll(<?php echo $lang_customer_notes; ?>, '%customer_notes%');

                    <?php foreach ($custom_fields as $custom_field => $replace_value) { ?>
                        preview = preview.replaceAll(<?php echo json_encode(H($custom_field)); ?>, <?php echo json_encode($replace_value); ?>);
                    <?php } ?>

		    preview = preview.replaceAll(<?php echo $lang_work_images; ?>, '%work_images%');
