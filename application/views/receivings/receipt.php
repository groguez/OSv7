...
for ($k = 1; $k <= NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k++) {
	...
	$line_item_custom_field = $this->Receiving->get_line_item_custom_field($k, 'show_on_receipt');
		...
		<div class="col-md-7">
			<span class="hidden-print search no-left-border">
				<ul class="list-inline print-buttons">
					...
					<div class="btn-group piluku-dropdown hidden-prin" role="group">
						...
						<ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
							<li><?php echo anchor('items/generate_barcodes_labels_from_recv/' . $receiving_id_raw, lang("common_barcode_labels"), array('id' => 'barcode_labels_button')); ?></li>
							<li><?php echo anchor('items/generate_qr_codes_labels_from_recv/' . $receiving_id_raw, lang("items_qrcode_labels"), array('id' => 'qr_code_labels_button')); ?></li>
						</ul>
					</div>
					<div class="btn-group piluku-dropdown hidden-prin" role="group">
						<button class="btn btn-primary btn-lg dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
							<?php echo lang("common_sheet"); ?>
							<span class="caret"></span>
						</button>
						<ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
							<li><?php echo anchor('items/generate_barcodes_from_recv/' . $receiving_id_raw, lang("common_barcode_sheet"), array('id' => 'barcode_sheet_button')); ?></li>
							<li><?php echo anchor('items/generate_qr_codes_from_recv/' . $receiving_id_raw, lang("items_qrcode_sheet"), array('id' => 'qr_code_sheet_button')); ?></li>
						</ul>
					</div>


					<li>
						<button class="btn btn-primary btn-lg hidden-print" id="export_excel_button" onClick="window.open('<?php echo site_url('reports/export_recv/' . $receiving_id_raw); ?>','_blank');"> <?php echo lang('common_excel_export', '', array(), TRUE); ?> </button>
					</li>
					<li>
										...
										<div class="row receipt-row-item-holder">
											<div class="<?php echo $this->config->item('wide_printer_receipt_format') ? 'col-md-' . $x_col . ' col-sm-' . $x_col . ' col-xs-' . $x_col : 'col-md-12 col-sm-12 col-xs-12' ?>">
												<div class="invoice-content invoice-con">
													...
													?>
														<?php
														if (get_class($item) == 'PHPPOSCartItemRecv' && $this->Receiving->get_line_item_custom_field($custom_field_id) !== false) {
															$item_info = $this->Item->get_info($item->item_id);
														...
														?>
																<div class="invoice-desc">
																	...
																	if ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'checkbox') {
																		$format_function = 'boolean_as_string';
																	} elseif ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'date') {
																		$format_function = 'date_as_display_date';
																	} elseif ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'email') {
																		$format_function = 'strsame';
																	} elseif ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'url') {
																		$format_function = 'strsame';
																	} elseif ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'phone') {
																		$format_function = 'format_phone_number';
																	} elseif ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'image') {
																		$this->load->helper('url');
																		$format_function = 'file_id_to_image_thumb_right';
																	} elseif ($this->Receiving->get_line_item_custom_field($custom_field_id, 'type') == 'file') {
																		$this->load->helper('url');
																		$format_function = 'file_id_to_download_link';
																	} else {
																		$format_function = 'strsame';
																	...
																	echo ($this->Receiving->get_line_item_custom_field($custom_field_id, 'hide_field_label') ? '' : $this->Receiving->get_line_item_custom_field($custom_field_id, 'name') . ':') . ' ' . $format_function($item->{"line_item_custom_field_${custom_field_id}_value"});
																	?>
																</div>
													<?php
															}
