						...
						<tr>
							...
							<td>
								...
								if ($transaction['transactionType'] == 'charge' && $transaction['approved'])
								{
									...
									?>
									<input type="hidden" name="sale_id" value="<?php echo $sale_id > 0 ? $sale_id : ''; ?>">
									<input type="hidden" name="amount" value="<?php echo $transaction['authorizedAmount']; ?>">
									<input type="hidden" name="start_date" value="<?php echo $start_date; ?>">
									<input type="hidden" name="end_date" value="<?php echo $end_date; ?>">
									<input type="submit" name="submitf" value="<?php
									
									// Extract transaction date
									$transaction_date = date('Y-m-d', strtotime($transaction['timestamp']));

									// Get current time in default timezone
									$current_timestamp = time();
									$default_timezone = date_default_timezone_get();
									$timezone_offset = timezone_offset_get(new DateTimeZone($default_timezone), new DateTime("@$current_timestamp"));
									$adjusted_current_timestamp = $current_timestamp + $timezone_offset;
									$current_date = date('Y-m-d', $adjusted_current_timestamp);

									// Compare
									if ($transaction_date == $current_date) { 
											echo lang('common_void'); 
										}else{
											echo lang('common_return'); 
										}
									?>" id="submit_delete" class="btn btn-danger">
									<?php echo form_close(); ?>
								<?php
								}
								?>
