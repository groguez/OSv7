...
$is_on_device_tip_processor = $this->Location->get_info_for_key('credit_card_processor', isset($override_location_id) ? $override_location_id : FALSE) == 'card_connect' || $this->Location->get_info_for_key('credit_card_processor', isset($override_location_id) ? $override_location_id : FALSE) == 'coreclear2' || $this->Location->get_info_for_key('credit_card_processor', isset($override_location_id) ? $override_location_id : FALSE) == 'valor' || $this->Location->get_info_for_key('credit_card_processor', isset($override_location_id) ? $override_location_id : FALSE) == 'square_terminal';
...
$line_item_custom_fields_to_display = array();
...
for ($k = 1; $k <= NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k++) {
	...
	$line_item_custom_field = $this->Sale->get_line_item_custom_field($k, 'show_on_receipt');
	...
	if ($line_item_custom_field) {
		$line_item_custom_fields_to_display[] = $k;
	}
				...
				<div class="hidden-print search no-left-border">
					<ul class="list-inline print-buttons">
						...
						if ((empty($deleted) || (!$deleted))) { ?>
							<li>
								<?php
								if ($sale_id_raw != lang('sales_test_mode_transaction', '', array(), TRUE) && !$is_purchase_points && !$is_ecommerce && $this->Employee->has_module_action_permission('sales', 'edit_sale', $this->Employee->get_logged_in_employee_info()->person_id)) {
						...
						<?php if ($sale_id_raw != lang('sales_test_mode_transaction', '', array(), TRUE) && !empty($customer_phone) && ($this->Location->get_info_for_key('twilio_sms_from')|| $this->config->item('sms_mobile_tel_username'))) { ?>
							<li>
			...
			<div class="panel-body panel-pad">
				<div class="row">
					...
					<div class="col-md-4 col-sm-4 col-xs-12">
						<ul class="list-unstyled invoice-address" style="margin-bottom:2px;">
							...
							<?php
							if ($this->config->item('show_email_of_company_on_receipt'))
							{
							?>
					...
					<div class="col-md-4 col-sm-4 col-xs-12">
						<ul class="list-unstyled invoice-detail" style="margin-bottom:2px;">
							...
							if (version_compare(PHP_VERSION, '7.2', '>=')  && function_exists('bcadd')) {
								...
								$hashids = new Hashids\Hashids($this->db->database);
							...
							?>
								...
								{
								?>
									<li><?php echo lang('sales_return', '', array(), TRUE); ?> <?php echo lang('common_sale_id', '', array(), TRUE) . ":"; ?> <?php echo ($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS') . ' ' . $return_sale_id ?></li>
								<?php
								}
								?>
				...
				<table style="width:100%;" id='receipt-draggable'>
							...
							<th class="invoice-table">
								<div class="row">
									...
									<?php if ($discount_exists) { ?>
										<div class="col-md-<?php echo $xs_col; ?> col-sm-<?php echo $xs_col; ?> col-xs-<?php echo $xs_col; ?> gift_receipt_element">
											<div class="invoice-head text-right item-discount"><?php echo lang('common_discount', '', array(), TRUE); ?></div>
										</div>
					...
					foreach (array_reverse($cart_items, true) as $line1 => $item) {
						...
						if ($line1 === $cart->get_index_for_credit_card_fee_item())
						{
							continue;
						}
						
						if($this->config->item('hide_parts_and_labor_items_on_receipt') && $is_work_order){
							if(!$item->is_repair_item == 1){
								continue;
							}
						}
						...
						if(isset($customer_taxable) && $customer_taxable == 0){
							$unit_price = $price_excluding_tax;
						}
									...
									<div class="row receipt-row-item-holder">
										<div class="<?php echo $this->config->item('wide_printer_receipt_format') ? 'col-md-' . $x_col . ' col-sm-' . $x_col . ' col-xs-' . $x_col : 'col-md-12 col-sm-12 col-xs-12' ?>">
											<div class="invoice-content invoice-con">
												...
												if ($this->config->item('show_line_item_sales_person_on_receipt') && $item->sold_by_employee_id_2)
												{
												?>
												<div class="invoice-desc">
													<?php
													echo lang('common_employee') . ': '.$this->Employee->get_info($item->sold_by_employee_id_2)->first_name.' '.$this->Employee->get_info($item->sold_by_employee_id_2)->last_name;
													?>
												</div>
												<?php
												}
												...
												<div class="invoice-desc">
													<?php
													if ($this->config->item('show_weight_and_dim_on_receipt') && ($item->weight || ($item->length && $item->width && $item->height))) {
														if ($item->weight)
														{
															echo to_quantity(H($item->weight)).' '.H($item->weight_unit);
															echo '<br />';
														}
														if ($item->length && $item->width && $item->height)
														{
															echo to_quantity(H($item->length)).' x '.to_quantity(($item->width)).' x '.to_quantity(H($item->height));
														}
													}
													?>
												</div>
												...
												<div class="invoice-desc" style="max-width: 30%;">
													<?php
												...
												?>
												<?php
												if (count($line_item_custom_fields_to_display))
												{
													echo '<hr >';
												}
												
												foreach ($line_item_custom_fields_to_display as $custom_field_id) {
													if (get_class($item) == 'PHPPOSCartItemSale' && $this->Sale->get_line_item_custom_field($custom_field_id) !== false) {

														if ($item->{"line_item_custom_field_${custom_field_id}_value"}) {
												?>
															<div class="invoice-desc">
																<?php

																if ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'checkbox') {
																	$format_function = 'boolean_as_string';
																} elseif ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'date') {
																	$format_function = 'date_as_display_date';
																} elseif ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'email') {
																	$format_function = 'strsame';
																} elseif ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'url') {
																	$format_function = 'strsame';
																} elseif ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'phone') {
																	$format_function = 'format_phone_number';
																} elseif ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'image') {
																	$this->load->helper('url');
																	$format_function = 'file_id_to_image_thumb_right';
																} elseif ($this->Sale->get_line_item_custom_field($custom_field_id, 'type') == 'file') {
																	$this->load->helper('url');
																	$format_function = 'file_id_to_download_link';
																} else {
																	$format_function = 'strsame';
																...
																echo ($this->Sale->get_line_item_custom_field($custom_field_id, 'hide_field_label') ? '' : $this->Sale->get_line_item_custom_field($custom_field_id, 'name') . ':') . ' ' . $format_function($item->{"line_item_custom_field_${custom_field_id}_value"});
																?>
															</div>
													<?php
														}
												...
												if (get_class($item) == 'PHPPOSCartItemKitSale' && $this->config->item('show_item_kit_items_on_receipt')) {
													...
													<div class="invoice-desc">
														<?php
														$item_kit_items_or_item_kit_item_kits = array();
														
														foreach ($this->Item_kit_items->get_info_kits($item->get_id()) as $ikik) {
															$item_kit_info = $this->Item_kit->get_info($ikik->item_kit_id);
															$item_kit_items_or_item_kit_item_kits[] = to_quantity($ikik->quantity) . '- ' . $item_kit_info->name . '<br />';
														}

														foreach ($this->Item_kit_items->get_info($item->get_id()) as $iki) {
															$item_info = $this->Item->get_info($iki->item_id);
															$item_kit_items_or_item_kit_item_kits[]= to_quantity($iki->quantity) . '- ' . $item_info->name . '<br />';
														}
														
														$output_for_item_kit_items = '';
														foreach ($item_kit_items_or_item_kit_item_kits as $item_kit_items_or_item_kit_item_kits_row)
														{
															$output_for_item_kit_items.= $item_kit_items_or_item_kit_item_kits_row;
														}
														$output_for_item_kit_items = trim($output_for_item_kit_items,'<br />');
														echo $output_for_item_kit_items;
												...
												foreach ($line_item_custom_fields_to_display as $custom_field_id) {
													if (get_class($item) == 'PHPPOSCartItemKitSale' && $this->Sale->get_line_item_custom_field($custom_field_id) !== false && $this->Sale->get_line_item_custom_field($custom_field_id) !== false) {
														$item_info = $this->Item_kit->get_info($item->item_kit_id);
												...
												if (isset($item->rule['type']) && !$this->config->item('hide_price_rule_on_receipt')) {
										...
										<?php if ($discount_exists) { ?>
											<div class="col-md-<?php echo $xs_col; ?> col-sm-<?php echo $xs_col; ?> col-xs-<?php echo $xs_col; ?> gift_receipt_element">
												<div class="invoice-content item-discount text-right"><?php echo $item->flat_discount_amount > 0 ? to_currency($item->flat_discount_amount) . " ( ". lang('common_flat_discount') . ")" : to_quantity($item->discount) . '%'; ?></div>
											</div>
										<?php } ?>

										<div class="col-md-<?php echo $xs_col; ?> col-sm-<?php echo $xs_col; ?> col-xs-<?php echo $xs_col; ?> gift_receipt_element">
											<div class="invoice-content item-total pull-right">
												<?php echo $item->flat_discount_amount > 0 ? to_currency(($unit_price - $item->flat_discount_amount) * $item->quantity + $item->get_modifiers_subtotal_flat_discount($item->flat_discount_amount), 10) . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . to_currency($item_tax_amount * $item->quantity) : '') : to_currency(($unit_price * $item->quantity - $unit_price * $item->quantity * $item->discount / 100) + $item->get_modifiers_subtotal() - ($item->get_modifiers_subtotal() * $item->discount / 100), 10) . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . to_currency($item_tax_amount * $item->quantity) : ''); ?>
				...
				<div class="invoice-footer gift_receipt_element">
					...
					foreach (array_reverse($cart_items, true) as $line1 => $item) 
					{
						...
						?>
							...
							if (isset($exchange_name) && $exchange_name) {
								$fee_addon = ($item->unit_price * $item->quantity);
							} else {
								$fee_addon = ($item->unit_price * $item->quantity);
							}
							?>
					...
					<div class="row">
						...
						<div class="col-md-2 col-sm-2 col-xs-4">
							<div class="invoice-footer-value">
								<?php
								if (isset($exchange_name) && $exchange_name) {
									echo to_currency_as_exchange($cart, $subtotal - $fee_addon);
								} else {
									echo to_currency($subtotal - $fee_addon);
								}
								?>
							</div>
						</div>
					...
					foreach (array_reverse($cart_items, true) as $line1 => $item) 
					{
						...
						<div class="row">
							<div class="col-md-offset-4 col-sm-offset-4 col-md-6 col-sm-6 col-xs-8">
								<div class="invoice-footer-heading"><?php echo H($item->description); ?></div>
							</div>
							<div class="col-md-2 col-sm-2 col-xs-4">
								<div class="invoice-footer-value">
									<?php
									if (isset($exchange_name) && $exchange_name) {
										echo to_currency_as_exchange($cart, $item->unit_price * $item->quantity);
									} else {
										echo to_currency($item->unit_price * $item->quantity);
									}
									?>
								</div>
							</div>
					...
					?>
						...
						foreach ($taxes as $name => $value) { 
							$total_tax += $value;
						?>
							<div class="row" style="<?php echo $display_tax? "" : "display:none;" ; ?>">
								<div class="col-md-offset-4 col-sm-offset-4 col-md-6 col-sm-6 col-xs-8">
					...
					?>
						<div class="row">
							...
							<div class="col-md-2 col-sm-2 col-xs-4">
								<?php
								if (
									($is_integrated_credit_sale || sale_has_partial_credit_card_payment($cart) || $is_sale_integrated_ebt_sale || sale_has_partial_ebt_payment($cart)) 
									&& 
									($payment->payment_type == lang('common_credit', '', array(), TRUE) || $payment->payment_type == lang('common_debit', '', array(), TRUE) ||   $payment->payment_type == lang('sales_partial_credit', '', array(), TRUE) || $payment->payment_type == lang('common_ebt', '', array(), TRUE) || $payment->payment_type == lang('common_partial_ebt', '', array(), TRUE) ||  $payment->payment_type == lang('common_ebt_cash', '', array(), TRUE) ||  $payment->payment_type == lang('common_partial_ebt_cash', '', array(), TRUE))
									&& ($payment->card_issuer || $payment->truncated_card)
								) { ?>
									<div class="invoice-footer-value">
							...
							<?php if (($is_integrated_credit_sale || sale_has_partial_credit_card_payment($cart) || $is_sale_integrated_ebt_sale || sale_has_partial_ebt_payment($cart)) && ($payment->payment_type == lang('common_credit', '', array(), TRUE) || $payment->payment_type == lang('common_debit', '', array(), TRUE) ||   $payment->payment_type == lang('sales_partial_credit', '', array(), TRUE) || $payment->payment_type == lang('common_ebt', '', array(), TRUE) || $payment->payment_type == lang('common_partial_ebt', '', array(), TRUE) ||  $payment->payment_type == lang('common_ebt_cash', '', array(), TRUE) ||  $payment->payment_type == lang('common_partial_ebt_cash', '', array(), TRUE))) { ?>
						...
						<div class="col-md-6 col-sm-6" style="margin-top: 30px;">
							<div id="signature">
								...
								if ($is_credit_card_sale) {
									echo isset($sales_card_statement) ? $sales_card_statement : "";
								}
								?>
...
<?php echo lang('common_item', '', array(), TRUE); ?>            <?php echo lang('common_price', '', array(), TRUE); ?> <?php echo lang('common_quantity', '', array(), TRUE); ?><?php if ($discount_exists) {
																																														echo ' ' . lang('common_discount', '', array(), TRUE);
...
<?php echo lang('common_sub_total', '', array(), TRUE); ?>: <?php echo str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency($subtotal - $fee_addon)); ?>
...
<?php echo (isset($show_payment_times) && $show_payment_times) ?  date(get_date_format() . ' ' . get_time_format(), strtotime($payment->payment_date)) : lang('common_payment', '', array(), TRUE); ?>  <?php if (($is_integrated_credit_sale || sale_has_partial_credit_card_payment($cart) || sale_has_partial_ebt_payment($cart)) && ($payment->payment_type == lang('common_credit', '', array(), TRUE) || $payment->payment_type == lang('common_debit', '', array(), TRUE) ||   $payment->payment_type == lang('sales_partial_credit', '', array(), TRUE) || $payment->payment_type == lang('common_ebt', '', array(), TRUE) || $payment->payment_type == lang('common_partial_ebt', '', array(), TRUE) ||  $payment->payment_type == lang('common_ebt_cash', '', array(), TRUE) ||  $payment->payment_type == lang('common_partial_ebt_cash', '', array(), TRUE))) {
