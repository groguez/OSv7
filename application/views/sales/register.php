...
<?php if ($this->session->userdata('test_mode')) { ?>
	<div class="alert alert-danger">
...
if($this->config->item('advanced_sales_interface')){
			...
			<div class="register-box register-items-form">
				...
				<div id="itemForm" class="item-form">
					...
					<div class="input-group contacts register-input-group">
						...
						<input type="text" id="<?php echo $this->config->item('advanced_sales_interface') ? "item_advanced_input" : "item" ?>" name="item" <?php echo ($mode == "store_account_payment" || $mode == 'purchase_points') ? 'disabled="disabled"' : '' ?> class="add-item-input pull-left keyboardTop" placeholder="<?php echo lang('common_start_typing_item_name'); ?>" data-title="<?php echo lang('common_item_name'); ?>">
			...
			<div class="register-items-holder">
				<?php if ($mode != 'store_account_payment') { ?>
					...
					<style>
						...
						.edit_discount:hover, .edit_discount:focus {
							text-decoration: unset;
						}	
						.discount-percent-btn, .discount-flat-btn {
							background: #489ee7;
							color: white;
						}
						.discount-percent-btn:hover, .discount-flat-btn:hover {
							background: #3192e4 !important;
						...
						.discount-flat-btn.active {
							background: black;
							color: white;
						}
						.discount-percent-btn.active:focus, .discount-flat-btn.active:focus {
							background: black;
							color: white;
							border: none;
						}
						.discount-percent-btn.active:hover, .discount-flat-btn.active:hover {
					...
					<table id="register" class="table table-hover">
						<thead>
							<tr class="register-items-header">
								...
								<th class="item_sort_able sales_discount <?php echo $this->cart->sort_column && $this->cart->sort_column == 'discount'? ($this->cart->sort_type=='asc'?"ion-arrow-down-b":"ion-arrow-up-b"):"";?>"><?php echo lang('common_discount'); ?></th>
									...
									<tbody class="register-item-content" data-line="<?php echo $line; ?>">
										<tr class="register-item-details">
											...
											<td>
												...
												<a tabindex="-1" href="<?php echo isset($item->item_id) ? site_url('home/view_item_modal/' . $item->item_id . '/' . $line . '/sales/' . $customer_id) . "?redirect=sales" : site_url('home/view_item_kit_modal/' . $item->item_kit_id) . "?redirect=sales"; ?>" data-toggle="modal" data-target="#myModal" class="register-item-name"><?php echo H($item->name).(property_exists($item, 'variation_name') && $item->variation_name ? '<span class="show-collpased" style="display:none">  ['.$item->variation_name.']</span>' : '') ?><?php echo $item->size ? ' (' . H($item->size) . ')' : ''; ?></a>
											</td>
											...
											<td class="text-center">
												<?php
												$discount_type = $item->flat_discount_amount > 0 ? 'flat' : 'percent';
												...
												?>
													<?php if ($line !== $line_for_flat_discount_item && $this->Employee->has_module_action_permission('sales', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
														<a href="#" id="discount_<?php echo $line; ?>" class="xeditable xeditable_discount_option" data-type="text" data-validate-number="true" data-pk="1" data-name="<?php echo $discount_type == 'flat' ? 'flat_discount_amount' : 'discount';?>" data-value="<?php echo $discount_type == 'flat' ? to_quantity($item->flat_discount_amount) : to_quantity($item->discount); ?>" data-url="<?php echo site_url('sales/edit_item/' . $line); ?>" data-title="<?php echo lang('common_discount') ?>"><?php echo $discount_type == "percent" ? to_quantity($item->discount) . "%" : to_currency($item->flat_discount_amount); ?></a>
														<input type="hidden" value="percent" id="discount_type_hidden">
													<?php } else { ?>
											...
											<td class="text-center">
												...
												{
													$total_price = $discount_type == "percent" ? $item->unit_price * $item->quantity - $item->unit_price * $item->quantity * $item->discount / 100 : ($item->unit_price - $item->flat_discount_amount) * $item->quantity;
												...
												?>
													...
													?>
														<a href="#" id="total_<?php echo $line; ?>" class="xeditable" data-type="text" data-validate-number="true" data-pk="1" data-name="total" data-value="<?php echo H(to_currency_no_money($total_price)); ?>" data-url="<?php echo site_url('sales/edit_line_total/' . $line); ?>" data-title="<?php echo lang('common_total') ?>"><?php echo to_currency($total_price); ?></a>
													<?php } else {
														echo to_currency($total_price);
													}	?>
										...
										<tr class="register-item-bottom">
											...
											<?php } ?>
												...
												<?php if (!$this->config->item('hide_description_on_sales_and_recv')) { ?>
													...
													<dd>
														<?php if (isset($item->allow_alt_description) && $item->allow_alt_description == 1 && $item->name != lang('common_giftcard')) { ?>
...
<?php if(isset($item->location_at_store) && $item->location_at_store){ ?>
<dt><?php echo lang('items_location_at_store'); ?></dt>
<dd><?php echo H($item->location_at_store); ?></dd>
<?php } ?>
													...
													{
														$item_pricing_info = $this->Item->get_info($item->item_id);
														$promo_price = $item_location_info->promo_price ? $item_location_info->promo_price : $item_pricing_info->promo_price;
														$retail_price = $item_location_info->unit_price ? $item_location_info->unit_price : $item_pricing_info->unit_price;
														if ($promo_price)
														{
														?>
															<dt><?php echo lang('common_promo_price'); ?></dt>
															<dd><?php echo to_currency($promo_price); ?></dd>
														<?php } ?>
														
														<dt><?php echo lang('items_retail_price'); ?></dt>
														<dd><?php echo to_currency($retail_price); ?></dd>
													...
													<?php if ($this->config->item('select_sales_person_during_sale')) {  ?>
														<dt><?php echo lang('common_sales_person'); ?></dt>
														<dd class="">
															<a href="#" id="sales_person_<?php echo $line; ?>" data-name="sales_person" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item_sales_person/' . $line); ?>" data-title="<?php echo H(lang('common_sales_person')); ?>">
																<?php echo character_limiter( $employees[$item->sold_by_employee_id_2], 50) ?>
															</a>
														</dd>
														<dd>
															<?php
															...
															foreach ($employees as $key => $employee) {
																$source_data[] = array('value' => $key, 'text' => $employee);
															}
															?>
															<script>
																$('#sales_person_<?php echo $line; ?>').editable({
																	value: <?php echo json_encode(H($item->sold_by_employee_id_2) ? H($item->sold_by_employee_id_2) : ''); ?>,
																	source: <?php echo json_encode($source_data); ?>,
																	success: function(response, newValue) {
																		last_focused_id = $(this).attr('id');
																		$("#register_container").html(response);
										...
										<?php
											$line_item_custom_field_count = $this->Sale->get_line_item_custom_field_count();
										?>
														...
														<?php 
															...
															for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) {
																...
																if($custom_field !== FALSE) {
																	$required = false;
																	$required_text = '';
																	if($this->Sale->get_line_item_custom_field($k,'required') && in_array($current_location,$this->Sale->get_line_item_custom_field($k,'locations'))){
																		$required = true;
																...
																<div class="form-group">
																	...
																	<div class="col-sm-9 col-md-9 col-lg-10">
																		<?php if ( $this->Sale->get_line_item_custom_field($k,'type') == 'checkbox') { ?>
																		...
																		<?php } elseif( $this->Sale->get_line_item_custom_field($k,'type') == 'date') { ?>
																			<?php
																			echo form_input(
																				array(
																		...
																		<?php } elseif(( $this->Sale->get_line_item_custom_field($k,'type') ) == 'dropdown') { ?>
																			<?php 
																			$choices = explode('|', ( $this->Sale->get_line_item_custom_field($k,'choices')));
																		...
																		<?php } elseif( $this->Sale->get_line_item_custom_field($k,'type') == 'image') {
																			echo form_input(
																				array(
																					'name'=>"line_${line}_custom_field_${k}_value",
																					'id'=>"line_${line}_custom_field_${k}_value",
																			...
																			if ($item_info->{"line_item_custom_field_${k}_value"})
																			{
																				...
																				echo "<div class='delete-custom-image'><a href='".site_url('sales/delete_line_item_custom_field_value/'.$line.'/'.$k)."'>".lang('common_delete')."</a></div>";
																			}
																		}
																		elseif( $this->Sale->get_line_item_custom_field($k,'type') == 'file' )
																		{
																			...
																			if ($item_info->{"line_item_custom_field_${k}_value"})
																			{
																				...
																				echo "<div class='delete-custom-image'><a href='".site_url('sales/delete_line_item_custom_field_value/'.(isset($item_info->item_id) ? $item_info->item_id : $item_info->item_kit_id).'/'.$k)."'>".lang('common_delete')."</a></div>";
																			}
																		} else {
																			echo form_input(
																				array(
					...
					<script>
						$('.line-item-custom-fields').change(function() {
							$.post('<?php echo site_url("sales/save_line_item_custom_field"); ?>', {
								name: $(this).attr('name'),
								value: $(this).val()
							});
						});
						...
						$('.line-item-custom-fields-file').change(function() {
							...
							$.ajax({
								url: '<?php echo site_url("sales/save_line_item_custom_field"); ?>',
								type: 'POST',
								data: formData,
								processData: false,
								contentType: false
			...
			<table id="register" class="table table-hover ">
				...
				<tbody id="cart_contents">
					...
					?>
						...
						<tr id="reg_item_top">
							<td class="text text-center text-success"><a tabindex="-1" href="<?php echo isset($item->item_id) ? site_url("home/view_item_modal/" . $item->item_id . '/'. $line . '/sales/' . $customer_id) : site_url('home/view_item_kit_modal/' . $item->item_kit_id . "?redirect=sales"); ?>" data-toggle="modal" data-target="#myModal"><?php echo H($item->name); ?></a></td>
							<td class="text-center">
								<?php
...
<div class="col-lg-4 col-md-5 col-sm-12 col-xs-12">
	<div class="register-box register-right">
		...
		<div class="sale-buttons">
			...
			<div class="btn-group">
				...
				<ul class="dropdown-menu sales-dropdown" role="menu">
					<?php if ($mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
						<?php if ($this->Employee->has_module_action_permission('giftcards', 'can_sell_giftcard', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
							<?php
							...
							<li>
								<?php echo
									anchor(
										"sales/check_giftcard",
										'<i class="ion-card"></i> ' . lang('giftcards_check_balance'),
										array(
											'class' => '',
											'title' => lang('giftcards_check_balance')
										)
									);
								?>
							</li>
						...
						<?php if (($cc_processor_class_name == 'CORECLEARBLOCKCHYPPROCESSOR' || $cc_processor_class_name == 'VALORPROCESSOR') && $this->Employee->has_module_action_permission('sales', 'view_edit_transaction_history', $this->Employee->get_logged_in_employee_info()->person_id)) {?>				
							
							<?php if ($cc_processor_class_name == 'CORECLEARBLOCKCHYPPROCESSOR') { ?>
							<li>
								<a href="/coreclear/" title=<?php echo json_encode(lang('sales_coreclear_portal')); ?> target="_blank"><span class="ion-ios-world"> <?php echo lang('sales_coreclear_portal'); ?></span></a>
							</li>
							<?php } ?>
					...
					<?php if (!is_on_demo_host()) { ?>
						...
						<li>
							<?php if (!$this->session->userdata('test_mode') && !$this->config->item('disable_test_mode')) { ?>
			...
			<?php if (count($cart_items) > 0) { ?>
				...
				<?php if ($mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
					...
					<?php if ($this->Employee->has_module_action_permission('sales', 'suspend_sale', $this->Employee->get_logged_in_employee_info()->person_id) && $customer_required_check && $suspended_sale_customer_required_check && !$this->session->userdata('test_mode')) { ?>
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu sales-dropdown" role="menu">
								...
								<?php if ($this->Employee->has_module_action_permission('sales', 'can_create_estimate', $this->Employee->get_logged_in_employee_info()->person_id)){ ?>
		...
		<?php if (isset($customer)) { ?>
			...
			<div class="customer-badge">
				...
				<div class="details">
					<?php if (!$this->config->item('hide_customer_recent_sales') && isset($customer)) { ?>
						<a href="<?php echo site_url('home/customer_modal/' . $customer_id); ?>" data-toggle="modal" data-target="#myModal" class="name"><?php echo character_limiter(H($customer), 30); ?></a>
			...
			<div class="customer-action-buttons  btn-group btn-group-justified ">
				...
				<?php if (($this->Location->get_info_for_key('twilio_sms_from') && $this->Location->get_info_for_key('twilio_token') && $this->Location->get_info_for_key('twilio_sid')) || ($this->config->item('sms_mobile_tel_username') && $this->config->item('sms_mobile_tel_password'))) { ?>
	...
	<div class="register-box register-summary paper-cut">
		...
		<ul class="list-group">
			<?php if ($this->Employee->has_module_action_permission('sales', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id) && $mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
				<?php if(!$this->config->item('disable_discount_by_percentage')){ ?>
					<li class="list-group-item global-discount-group">
						...
						<?php
							$has_flat_discount = false;
							foreach($cart_items as $item) {
								$has_flat_discount = $item->flat_discount_amount > 0;
								if($has_flat_discount) continue;
							}

							$discount_all_percent_value = '';
							if($has_flat_discount) {
								$discount_all_percent_value = '';
								$discount_percent_symbol = '';
							} elseif(isset($discount_all_percent) &&  $discount_all_percent > 0) {
								$discount_all_percent_value = to_quantity($discount_all_percent);
								$discount_percent_symbol = '%';
							} else {
								$discount_percent_symbol = '';
							}
			...
			<?php
			$display_tax = 1;
			if(isset($customer_taxable) && $customer_taxable == 0){
				$display_tax = 0;
			}
			?>
			<?php foreach ($taxes as $name => $value) { ?>
				<li class="list-group-item" style="<?php echo $display_tax?"":"display:none;"; ?>">
					<span class="key">
			...
			<?php } else { ?>
				<div id="finish_sale" class="finish-sale">
					...
					<?php
						...
						if ($cc_processor_parent_class_name == 'DATACAPUSBPROCESSOR' || $cc_processor_parent_class_name == 'DATACAPTRANSCLOUDPROCESSOR' || $cc_processor_class_name == 'CARDCONNECTPROCESSOR' || $cc_processor_class_name == 'CORECLEARBLOCKCHYPPROCESSOR' || $cc_processor_class_name == 'SQUARETERMINALPROCESSOR'|| $cc_processor_class_name == 'VALORPROCESSOR') {
					...
					<?php
						...
						if ($cc_processor_class_name != 'VALORPROCESSOR')
						{
...
<?php if($this->config->item('advanced_sales_interface')){ ?>
	<div class="modal fade advanced_sales_interface" id="advanced_sales_interface" role="dialog" aria-labelledby="lookUpItems" aria-hidden="true">
		<div class="modal-dialog customer-recent-sales">
			<div class="modal-content">
				...
				<div class="modal-body clearfix"
					...
					<?php echo form_open("sales/add", array('id' => 'add_item_dialog', 'class' => 'form-inline', 'autocomplete' => 'off')); ?>
						<div class="row">
							<div class="col-12">
...
<script type="text/javascript">
var relatedDialog = bootbox.dialog({
       ...
       buttons:{
               ...
               add:{label: <?php echo json_encode(lang('common_add')); ?>, className:'btn-primary', callback:function(){
                       var ids=[]; $('#related_items_modal input:checked').each(function(){ ids.push($(this).val()); });
                       function addNext(i){ if(i<ids.length){ $.post(<?php echo json_encode(site_url('sales/add')); ?>,{item:ids[i]},function(){ addNext(i+1); }); } else { $('#register_container').load(<?php echo json_encode(site_url('sales/reload')); ?>); } }
                       addNext(0);
               }}
       }
});
...
<?php if (!$this->config->item('disable_sale_notifications')) { ?>
	<script type="text/javascript">
								...
								callback: function(){
									...
									$("#item").autocomplete('option', 'response', 
										function(event, ui){
											...
											if(ui.content.length == 0 && (source_url.indexOf('sales/item_search') > -1) && $("<?php echo $search_from_selector;?> #item").val().trim() != "" ){
		...
		if ($this->session->flashdata('edit_failed_message')) {
			echo "show_feedback('error', " . json_encode($this->session->flashdata('edit_failed_message')) . ", " . json_encode(lang('common_error')) . ");";
		}
...
<script type="text/javascript" language="javascript">
	...
	$(document).ready(function() {
		...
		$('.xeditable_discount_option').on('shown', function (e, editable) 
		{
			var $buttons = $('<button class="btn btn-default btn-sm discount-percent-btn" type="button">%</button>' +
                        '<button class="btn btn-default btn-sm discount-flat-btn" type="button" style="margin-right: 5px; padding-left: 12px; padding-right: 12px;">$</button>');
    
			$(this).next().find('.editable-buttons').prepend($buttons);

			if($(this).data('name') == 'discount') {
				$('.discount-percent-btn').addClass('active');	
				$('.discount-flat-btn').removeClass('active');
			} else {
				$('.discount-percent-btn').removeClass('active');
				$('.discount-flat-btn').addClass('active');
			}

			$('.discount-percent-btn').on('click', function() {
				$('.discount-percent-btn').addClass('active');	
				$('.discount-flat-btn').removeClass('active');
				$('.xeditable_discount_option').editable('option', 'name', 'discount');
				$(this).closest('.editable-buttons').prev().find('input').focus();
			});

			$('.discount-flat-btn').on('click', function() {
				$('.discount-percent-btn').removeClass('active');
				$('.discount-flat-btn').addClass('active');
				$('.xeditable_discount_option').editable('option', 'name', 'flat_discount_amount');
				$(this).closest('.editable-buttons').prev().find('input').focus();
			});
		});
		...
		$(document).on('click', '#add_supplier', function(){
			...
			var default_supplier_id = $("<?php echo $search_from_selector; ?> .default_supplier_id").val();
								...
								'<div class="advanced-item-suggestions-container" style="'+ (variable_id?'padding-left:50px;':'') +'">' +
									...
									'<div class="advanced-item-details">' +
										...
										'<div class="attributes-container">' +
											...
											'<?php if(!$this->config->item('hide_supplier_on_sales_interface') && !$this->config->item('hide_supplier_in_item_search_result')){ ?>'+
		...
		<?php
		if ($selected_payment == lang('common_integrated_gift_card') || ($selected_payment == lang('common_debit') && $this->Location->get_info_for_key('enable_credit_card_processing')) || ($selected_payment == lang('common_credit') && $this->Location->get_info_for_key('enable_credit_card_processing')) || ($selected_payment == lang('common_ebt') && $this->Location->get_info_for_key('enable_credit_card_processing')) || ($selected_payment == lang('common_ebt_cash') && $this->Location->get_info_for_key('enable_credit_card_processing'))) {
		?>
	...
	function checkPaymentTypes() {
		...
		switch (paymentType) {
			...
			case <?php echo json_encode(lang('common_giftcard')); ?>:
				...
				<?php if($this->config->item('enable_giftcard_security')){ ?>
					$("#amount_tendered").attr('giftcard_pin', '');
				<?php } ?>
			...
			case <?php echo json_encode(lang('common_store_account')); ?>:
				$("#amount_tendered").val(<?php 
					$amount_tendered_value = "";
					if( isset($customer_balance) && $customer_balance < 0 && $this->config->item('customer_with_negative_store_account_balances_must_keep_balance_at_or_below_0')) {
						$amount_tendered_value = min ( -$customer_balance, $amount_due );
					}else{
						$amount_tendered_value = $amount_due;
					}
					echo json_encode(to_currency_no_money($amount_tendered_value));
				?>);
	...
	<?php if($this->config->item('enable_giftcard_security')){ ?>
		function check_giftcard_pin(e) {
			var giftcard_pin = $('#add_payment_form input[name="giftcard_pin"]').val();
			if(!giftcard_pin){
				bootbox.prompt({
					title: "Enter your PIN code:",
					inputType: 'password',  // Use password input type for PIN
					callback: function(result) {
						console.log("bootbox prompt in result");
						if (result !== null) {
							$('#add_payment_form input[name="giftcard_pin"]').remove();
							$("#add_payment_form").append(
								'<input type="hidden" name="giftcard_pin" value="' + result + '"/>'
							);
							$("#finish_sale_alternate_button").trigger("click");
						} else {
							show_feedback('error', 'PIN entry canceled', <?php echo json_encode(lang('common_error')); ?>);
						}
					},
					onShown: function() {
						$('#bootbox-input').attr('autocomplete', 'new-password');
					}				
				});
				console.log("bootbox prompt out result");
				return false;
			}
			return true;
		}
	...
	function salesBeforeSubmit(formData, jqForm, options) {
		<?php if($this->config->item('enable_giftcard_security')){ ?>
			if($("#amount_tendered").length>0 && $("#amount_tendered").val().trim()=='') return false;
			if($('#payment_types').val() == <?php echo json_encode(lang('common_giftcard')); ?> ) {
				if(!check_giftcard_pin()) return false;
			}
		<?php } ?>
...
<?php
       ...
       <?php
       if (isset($prompt_edit_or_return_sale) && $prompt_edit_or_return_sale == TRUE) {
       ?>

               bootbox.dialog({
                       message: <?php echo json_encode(lang('sales_scanned_past_sale')); ?>,
                       buttons: {
                               edit: {
                                       label: <?php echo json_encode(lang('sales_edit_sale')); ?>,
                                       className: 'btn-primary',
                                       callback: function() {
                                               $.get('<?php echo site_url("sales/change_sale/"); ?>' + encodeURIComponent(<?php echo json_encode($scanned_sale_id); ?>)+'/1', function(response) {
                                                       $("#register_container").html(response);
                                               });
                                       }
                               },
                               return_sale: {
                                       label: <?php echo json_encode(lang('sales_return')); ?>,
                                       className: 'btn-info',
                                       callback: function() {
                                               $.get('<?php echo site_url("sales/return_order/"); ?>' + encodeURIComponent(<?php echo json_encode($scanned_sale_id); ?>) + '/1/1', function(response) {
                                                       $("#register_container").html(response);
                                               });
                                       }
                               }
...
<?php
	...
	var dialog = bootbox.dialog({
	    ...
	    message: '<input type="text" id="sale_id_input" class="form-control" placeholder="<?php echo json_encode(lang('sales_enter_sale_id')); ?>">',
	    size: 'large',
	    buttons: {
			
	        dont_load_items: {
	            label: <?php echo json_encode(lang('sales_dont_load_sale_items')); ?>,
	            className: 'btn-info',
	            callback: function() {
	                return processSaleId(false);
	            }
	        },
	        load_items: {
	            label: <?php echo json_encode(lang('sales_load_sale_items')); ?>,
	            className: 'btn-success',
	            callback: function() {
	                return processSaleId(true);
	            }
	        },
	    }
	});

	// Ensure input field is focused after the modal is added to the DOM
	dialog.on('shown.bs.modal', function() {
	    var inputField = $('#sale_id_input');
	    inputField.focus();

	    // Handle Enter key inside the modal (without document-wide listeners)
	    inputField.off('keypress').on('keypress', function(e) {
	        if (e.which === 13) { // 13 = Enter key
	            e.preventDefault();
	            dialog.find('.btn-success').trigger('click'); // Simulate "Load Sale Items" button click
	        }
	    });
	});

	// Function to process Sale ID and navigate
	function processSaleId(loadItems) {
	    var sale_id = $('#sale_id_input').val().trim();
	    if (sale_id) {
	        var load_flag = loadItems ? '1' : '0';
	        window.location.href = "<?php echo site_url('sales/return_order/'); ?>" + encodeURIComponent(sale_id) + '/' + load_flag;
	    } else {
	        bootbox.alert(<?php echo json_encode(lang('sales_enter_sale_id')); ?>);
	        return false; // Prevent modal from closing if input is empty
	    }
	}
...
<script>
	...
	<?php if($this->config->item('advanced_sales_interface')){ ?>
		...
		$("#item_advanced_input").on('keydown', function(){
			$('#advanced_sales_interface').modal('show');
			clear_advanced_item_list();
		});

		$('#advanced_sales_interface').on('shown.bs.modal', function() {
		...
		$('#advanced_sales_interface').on('hidden.bs.modal', function() {
			clear_advanced_item_list();
		});
		...
		function close_advanced_item_list(){
			$('#advanced_sales_interface').modal('hide');
			clear_advanced_item_list();
		}
	...
	<?php if (isset($customer_balance_warning) && $customer_balance_warning && $this->session->userdata('customer_balance_negative') == 0) { ?>
		showCustomerBalanceWarning(<?php echo json_encode($customer_balance_warning); ?>);
	<?php } ?>


	function showCustomerBalanceWarning(customer_balance_warning) {
		var dialog = bootbox.confirm({
			message: customer_balance_warning,
			buttons: {
				confirm: {
					label: "<i class='ion-checkmark'></i> <?php echo lang('sales_select_store_account'); ?>",
					className: 'btn-primary'
				},
				cancel: {
					label: "<i class='ion-close'></i> <?php echo lang('common_cancel'); ?>",
			...
			callback: function(result) {
				if (result) {
					$.post('<?php echo site_url("sales/set_selected_payment"); ?>', {
						payment: "<?php echo lang('common_store_account'); ?>"
					}, function(response) {
						$("#register_container").load('<?php echo site_url("sales/reload"); ?>');
					});
				} else {
					$.post('<?php echo site_url("sales/update_customer_balance_negative_status"); ?>', {
						status: 1
					}, function(response) {
						$("#register_container").load('<?php echo site_url("sales/reload"); ?>');
					});
