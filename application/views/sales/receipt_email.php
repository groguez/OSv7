...
<body style="Margin:0;padding-top:0;padding-bottom:0;padding-right:0;padding-left:0;min-width:100%;background-color:#E8EBF1;line-height:20px;" >	
	...
	<center class="wrapper" style="width:100%;table-layout:fixed;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;" >
									...
									<td class="inner" style="padding-top:10px;padding-bottom:10px;padding-right:10px;padding-left:10px;" >
										<table class="contents" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;width:100%;font-size:13px;text-align:left;" >
					                    	...
					                    	<div class="invoice-desc">
												...
												<?php if($delivery_info && $delivery_info['contact_preference']){ ?>
													<span>
														<?php echo lang('deliveries_contact_preference'); ?> : <?php echo implode(", ", is_serialized($delivery_info['contact_preference']) ? unserialize($delivery_info['contact_preference']) : $delivery_info['contact_preference']); ?>
													</span>
													<br />
												<?php } ?>
					...
					<td class="one-column" style="padding-top:0;padding-bottom:0;padding-right:0;padding-left:0;" >
						<table width="100%" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;font-size:13px;" >
	                    	...
	                    	<div class="invoice-desc">
											...
											<?php if($discount_exists) { ?>
												<td  align="center" style="padding-right:0;padding-top:10px !important;padding-left:10px;border-width:1px;border-style:solid;border-color:#DCE0E6;border-bottom-width:0px;border-right-width:0px;padding-bottom:10px;" >
													<?php echo $item->flat_discount_amount > 0 ? to_currency($item->flat_discount_amount) . " ( ". lang('common_flat_discount') . ")" : to_quantity($item->discount); ?>
												</td>
											<?php } ?>
										
											<td  align="center" style="padding-right:0;padding-top:10px !important;padding-left:10px;border-width:1px;border-style:solid;border-color:#DCE0E6;border-bottom-width:0px;border-right-width:0px;padding-bottom:10px;" >
												<?php echo $item->flat_discount_amount > 0 ? to_currency($item->get_modifiers_subtotal_flat_discount($item->flat_discount_amount) + ($unit_price - $item->flat_discount_amount) * $item->quantity, 10) : to_currency(+($item->get_modifiers_subtotal() - ($item->get_modifiers_subtotal() * $item->discount / 100) )+($unit_price*$item->quantity-$item->unit_price*$item->quantity*$item->discount/100),10); ?>
											</td>
										</tr>
		...
		?>
			...
			<?php if ( $this->config->item('allow_customers_to_pay_suspended_sales_online') && ($this->Location->get_info_for_key('credit_card_processor') == 'coreclear2' || $this->Location->get_info_for_key('credit_card_processor') == 'valor') && $amount_due > 0 ) { ?>
				<tr class="text-right item-row">
					<td  colspan="<?php echo $total_columns; ?>" align="center" style="padding-right:0;padding-top:10px !important;padding-left:10px;border-width:1px;border-style:solid;border-color:#DCE0E6;border-bottom-width:0px;border-right-width:0px;padding-bottom:10px; font-size: larger;" >
					<?php
						echo anchor($this->Sale->get_processor_payment_link($sale_id_raw), lang('common_pay'), array('style' => 'background: #3498db;background-image: linear-gradient(to bottom, #3498db, #2980b9);-webkit-border-radius: 28;-moz-border-radius: 28;border-radius: 28px;font-family: Arial;color: #ffffff;padding: 10px;text-decoration: none;font-weight: normal; margin-right:30px;'));
					?>
					</td>
				</tr>
			<?php } ?>
