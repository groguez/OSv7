...
<tr>
    ...
    <td colspan="2" valign="top" align="center" style="width:30%;">
        ...
        <p>
            <?php if ($receipt_title && (!isset($sale_type) || ($sale_type != ($this->config->item('user_configured_estimate_name') ? $this->config->item('user_configured_estimate_name') : lang('common_estimate')) && $sale_type != ($this->config->item('user_configured_layaway_name') ? $this->config->item('user_configured_layaway_name') : lang('common_layaway'))))) {?>
                    ...
                    <div class="invoice-desc">
                        ...
                        if ($this->Employee->get_custom_field($custom_field_id, 'type') == 'checkbox') {
                            ...
                            $format_function = 'strsame';
												...
												{
													$format_function = 'file_id_to_image_thumb_right_pdf_safe';
												}
												else {
                            $format_function = 'strsame';
                        }
...
foreach (array_reverse($cart_items, true) as $line => $item) {
					...
					?>
						<div class="invoice-desc">
							...
							echo img(array(
								...
								'src' => secure_app_file_url_pdf_safe($item->main_image_id)
							));
							?>
						</div>
					<?php
    ...
    <tr>
        ...
        <td align="right">
            ...
            ?>
            <?php echo $item->flat_discount_amount > 0 ? to_currency($item->get_modifiers_subtotal_flat_discount($item->flat_discount_amount) + ($unit_price - $item->flat_discount_amount) * $item->quantity, 10) . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . to_currency($item_tax_amount * $item->quantity) : '') : to_currency(+($item->get_modifiers_subtotal() - ($item->get_modifiers_subtotal() * $item->discount / 100)) + ($unit_price * $item->quantity - $unit_price * $item->quantity * $item->discount / 100), 10) . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . to_currency($item_tax_amount * $item->quantity) : ''); ?>
        </td>
