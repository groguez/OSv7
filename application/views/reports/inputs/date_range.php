...
?>	
<input type="hidden" name="report_type<?php echo $compare_suffix; ?>" id="report_type<?php echo $compare_suffix; ?>" value='<?php echo H($report_type_value); ?>' />
	...
	<div id="report_date_range_complex<?php echo $compare_suffix; ?>" class="col-sm-6 col-md-6 col-lg-8 <?php echo $report_type_value == 'simple' ? 'hidden' : '' ?>">
		<div class="row">
			<div class="col-md-6">
             ...
             <input type="text" class="form-control start_date" name="start_date<?php echo $compare_suffix; ?>" id="start_date<?php echo $compare_suffix; ?>" <?php echo isset($start_date_value) ? "value='".H($start_date_value)."'" : "value='".H($this->input->get('start_date_'.$compare_middle_suffix.'formatted'))."'" ?>>
        </div>
			</div>
        ...
        <span class="input-group-addon bg date-picker"><?php echo lang('reports_to'); ?></span>
            <input type="text" class="form-control end_date" name="end_date<?php echo $compare_suffix; ?>" id="end_date<?php echo $compare_suffix; ?>" <?php echo isset($end_date_value) ? "value='".H($end_date_value)."'" : "value='".H($this->input->get('end_date_'.$compare_middle_suffix.'formatted'))."'" ?>>
      	</div>	
			</div>
		</div>
	</div>
