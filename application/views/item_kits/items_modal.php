						...
						<table class="table table-bordered table-hover table-striped">
							...
							foreach ($this->Item_kit_items->get_info($item_kit_info->item_kit_id) as $item_kit_item) {?>
								<tr>
									...
									$location_item = $this->Item_location->get_info($item_info->item_id, $current_logged_in_location_id);
									if($location_item->quantity && $item_kit_item->quantity){
										$item_kit_count_predict = to_quantity(floor($location_item->quantity/ $item_kit_item->quantity));
									}else{
										$item_kit_count = lang("common_not_set");
									}

									if($item_kit_count != lang("common_not_set")){
										if($item_kit_count == -1){
											$item_kit_count = $item_kit_count_predict;
										}else if($item_kit_count > $item_kit_count_predict){
											$item_kit_count = $item_kit_count_predict;
											if($item_kit_count < 0){
												$item_kit_count = 0;
											}
									...
									<td> <?php echo to_quantity($item_kit_item->quantity) ?> ( <?php echo $location_item->quantity ? to_quantity($location_item->quantity) : lang("common_not_set"); ?> ) </td>
								</tr>
							<?php } ?>
							<tr>
								<td><?php echo lang("item_kits_quantity"); ?>:</td>
								<td><?php echo $item_kit_count; ?></td>
							</tr>
						</table>
					</div>
				</div>
