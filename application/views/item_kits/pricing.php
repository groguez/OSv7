		...
		<div class="panel panel-piluku">
	      ...
	      <h3 class="panel-title">
			...
			<div class="panel-body">
				...
				<div id="dynamic_pricing_options">
				    <div class="form-group">
				        <label class="col-sm-3 col-md-3 col-lg-2 control-label">Dynamic Fields:</label>
				        <div class="col-sm-9 col-md-9 col-lg-10 dynamic-fields-container">
				            <div class="dynamic-field">
								<?php echo form_label(lang('common_only_cost_price').':', 'dynamic_unit_price',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label wide')); ?>
				                <input type="checkbox" id="dynamic_cost_price" name="dynamic_cost_price" value="1" 
				                       <?php echo $item_kit_info->dynamic_cost_price ? 'checked' : ''; ?>>
							<label for="dynamic_cost_price"><span></span></label>
				            </div>
				            <div class="dynamic-field">
								<?php echo form_label(lang('common_only_selling_price').':', 'dynamic_unit_price',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label wide')); ?>
				                <input type="checkbox" id="dynamic_unit_price" name="dynamic_unit_price" value="1" 
				                       <?php echo $item_kit_info->dynamic_unit_price ? 'checked' : ''; ?>>
							<label for="dynamic_unit_price"><span></span></label>
				            </div>
				        </div>
				    </div>
				</div>
				...
				<div class="form-group">
					<?php echo form_label(lang('common_item_kit_non_profit_sale').':', '',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label wide')); ?>
					<div class="col-sm-9 col-md-9 col-lg-10">
						<?php echo form_checkbox(array(
							'name'=>'non_profit_sale',
							'id'=>'non_profit_sale',
							'class' => 'non_profit_sale_checkbox delete-checkbox',
							'value'=>1,
							'checked'=>(boolean)$item_kit_info->non_profit_sale));
						?>
						<label for="non_profit_sale"><span></span></label>
					</div>
				</div>
...
<?php $this->load->view("partial/common_js"); ?>
	...
	$(document).ready(function () {
	    function toggleDynamicPricingUI() {
	        if ($('#dynamic_pricing').prop('checked')) {
	            $('#dynamic_pricing_options').removeClass('hidden');
	            toggleManualFields();
	        } else {
	            $('#dynamic_pricing_options').addClass('hidden');
	            $('#cost_price').closest('.form-group').show();
	            $('#unit_price').closest('.form-group').show();
	        }
	    }

	    function toggleManualFields() {
	        if ($('#dynamic_cost_price').prop('checked')) {
	            $('#cost_price').closest('.form-group').hide();
	        } else {
	            $('#cost_price').closest('.form-group').show();
	        }

	        if ($('#dynamic_unit_price').prop('checked')) {
	            $('#unit_price').closest('.form-group').hide();
	        } else {
	            $('#unit_price').closest('.form-group').show();
	        }
	    }

	    // Initialize the UI based on checkbox states
	    toggleDynamicPricingUI();

	    // Attach event listeners
	    $('#dynamic_pricing').change(toggleDynamicPricingUI);
	    $('#dynamic_cost_price, #dynamic_unit_price').change(toggleManualFields);
	});
