...
<script type="text/javascript">
	...
	{	
		...
		enable_add_to_sale(<?php echo json_encode(lang($controller_name."_confirm_add_to_sale"));?>,<?php echo json_encode(lang($controller_name."_none_selected"));?>);
		enable_add_to_receiving(<?php echo json_encode(lang($controller_name."_confirm_add_to_receiving"));?>,<?php echo json_encode(lang($controller_name."_none_selected"));?>);
		...
		$('#generate_qr_codes').click(function()
		{
			var selected = get_selected_values();
			...
			$("#skip-labels-qr-code").modal('show');
			return false;
		});
		...
		{
			var selected = get_selected_values();
			var num_labels_barcode_skip = $("#num_labels_barcode_skip").val() ? $("#num_labels_barcode_skip").val() : 0;
			var url = '<?php echo site_url("items/generate_barcodes");?>'+'/'+selected.join('~')+'/'+num_labels_barcode_skip;
			window.location = url;
			return false;
		});
		$("#generate_qr_codes_form").submit(function()
		{
			var selected = get_selected_values();
			var num_labels_qr_code_skip = $("#num_labels_qr_code_skip").val() ? $("#num_labels_qr_code_skip").val() : 0;
			var url = '<?php echo site_url("items/generate_qr_codes");?>'+'/'+selected.join('~')+'/'+num_labels_qr_code_skip;
			window.location = url;
			return false;
		});
		...
		$('#generate_qr_code_labels').click(function()
		{
			var selected = get_selected_values();
			if (selected.length == 0)
			{
			...
			$(this).attr('href','<?php echo site_url("items/generate_qr_code_labels");?>/'+selected.join('~'));
		});
    ...
    <div class="modal-dialog customer-recent-sales">
      	<div class="modal-content">
	        ...
	        <div class="modal-body">
				...
				<input type="text" class="form-control text-center" name="num_labels_barcode_skip" id="num_labels_barcode_skip" placeholder="<?php echo lang('common_skip_labels') ?>">
...
<div class="modal fade skip-labels" id="skip-labels-qr-code" role="dialog" aria-labelledby="skipLabels" aria-hidden="true">
    <div class="modal-dialog customer-recent-sales">
      	<div class="modal-content">
	        <div class="modal-header">
	          	<button type="button" class="close" data-dismiss="modal" aria-label=<?php echo json_encode(lang('common_close')); ?>><span aria-hidden="true">&times;</span></button>
	        ...
	        <div class="modal-body">
	          	...
	          	<?php echo form_open("items/generate_qr_codes", array('id'=>'generate_qr_codes_form','autocomplete'=> 'off')); ?>				
				<input type="text" class="form-control text-center" name="num_labels_qr_code_skip" id="num_labels_qr_code_skip" placeholder="<?php echo lang('common_skip_labels') ?>">
					<?php echo form_submit('generate_qr_codes_form',lang("common_submit"),'class="btn btn-block btn-primary"'); ?>
				<?php echo form_close(); ?>
		...
		<div class="email_buttons items">
			...
			<div class="btn-group piluku-dropdown" role="group">
			  ...
			  <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
			    <li><?php echo anchor("$controller_name/generate_barcode_labels", lang("common_barcode").' '.lang("common_label_printer"), array('id' => 'generate_barcode_labels')); ?></li>
			    <li><?php echo anchor("$controller_name/generate_barcodes", lang("common_barcode").' '.lang("common_standard_printer"), array('id' => 'generate_barcodes')); ?></li>
			    <li><?php echo anchor("$controller_name/generate_qr_code_labels", lang("common_qr_code").' '.lang("common_label_printer"), array('id' => 'generate_qr_code_labels')); ?></li>
			    <li><?php echo anchor("$controller_name/generate_qr_codes", lang("common_qr_code").' '.lang("common_standard_printer"), array('id' => 'generate_qr_codes')); ?></li>
			  </ul>
			</div>
			...
			<?php echo 
				anchor("sales/add_to_sale",
				'<span class="icon ti-shopping-cart"></span> '.'<span class="hidden-xs">'.lang("common_add_to_sale").'</span>',
				array('id'=>'add_to_sale','class'=>'btn btn-success btn-lg','title'=>lang("common_add_to_sale"))); 
			?>

			<?php echo 
				anchor("receivings/add_to_receiving",
				'<span class="icon ti-shopping-cart"></span> '.'<span class="hidden-xs">'.lang("common_add_to_receiving").'</span>',
				array('id'=>'add_to_receiving','class'=>'btn btn-info btn-lg','title'=>lang("common_add_to_receiving"))); 
			?>
            ...
            <div class="rect3"></div>
					...
					<ul class="dropdown-menu" role="menu">
						...
						<?php if ($this->Employee->has_module_action_permission($controller_name, 'add_update', $this->Employee->get_logged_in_employee_info()->person_id)) {?>				
							...
							<li>
								<?php echo anchor("$controller_name/price_list",
								'<span class="ion-cash"> '.lang("items_price_list").'</span>',
								array('class'=>' ', 'title'=>lang('items_price_list')));
								?>
							</li>
