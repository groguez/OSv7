			...
			<div class="panel panel-piluku">
		      ...
		      <h3 class="panel-title"><i class="ion-android-clipboard"></i> <?php echo lang("common_inventory"); ?> <small>(<?php echo lang('common_fields_required_message'); ?>)</small></h3>
				...
				<div class="panel-body">
					...
					?>
					<div class="form-group">	
					    <?php echo form_label(lang('items_shopify_inventory_policy'), 'shopify_item_level_inventory_policy', array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label')); ?>
					    <div class="col-sm-9 col-md-9 col-lg-10">
					        <?php 
					            $options = array(
					                'DENY' =>  lang('items_not_allowed_to_be_sold_online_if_out_of_stock'),
					                'CONTINUE' => lang('items_allowed_to_be_sold_online_if_out_of_stock')
					            );
					            echo form_dropdown('shopify_item_level_inventory_policy', $options, $item_info->shopify_item_level_inventory_policy, 'class="form-control" id="shopify_item_level_inventory_policy"');
					        ?>
					    </div>
					</div>					
					<?php } ?>
					...
					<?php if(!count($item_variations) > 0) { ?>
						...
						<div class="form-group">
							<?php echo form_label(lang('items_limited_sales_quantity').':', null,array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label')); ?>
							<div class="col-sm-9 col-md-9 col-lg-10 form-text">
								<?php echo form_input(array(
									'name'=>'limited_sales_quantity',
									'id'=>'limited_sales_quantity',
									'placeholder' => to_quantity(null, FALSE),
									'data-original' => isset($item_info->limited_sales_quantity) ? to_quantity($item_info->limited_sales_quantity, FALSE) : to_quantity('', FALSE),
									'value' => to_quantity($item_info->limited_sales_quantity, FALSE),
									'class'=>'form-control'
									)
								);?>
							</div>
								...
								<table class="table table-striped table-hover custom-table">
									<thead>
										<tr>
											...
											<?php } ?>
											<th><?php echo lang('items_limited_sales_quantity'); ?></th>
											<?php if ($this->Employee->has_module_action_permission('items','edit_quantity', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
									...
									<tbody>
										<?php foreach($item_variations as $item_variation_id => $item_variation) { ?>
											<tr>
												...
												<?php } ?>
												<td><input type="text" class="form-control limited_sales_quantity" name="item_variations[<?php echo $item_variation_id; ?>][limited_sales_quantity]" value="<?php echo H(to_quantity($item_variation['limited_sales_quantity'], false)); ?>"></td>
						...
						<tbody>
							<?php foreach($inventory_data as $row) { ?>
								<tr>
									...
									$row['trans_comment'] = preg_replace_callback(
										'/'.$this->config->item('sale_prefix').' ([0-9]+)/',
										function ($matches) {
											// $matches[0] is "POS 31", $matches[1] is "31"
											return '<span class="sale">' . anchor('sales/receipt/' . $matches[1], $matches[0]) . '</span>';
										},
										$row['trans_comment']
									);
...
{	
	$("#generate_barcode_labels").click(function()
	{
		...
		var number_of_barcodes = $("#items_number_of_qr_barcodes").val();
