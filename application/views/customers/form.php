				...
				<div class="panel-body">
					<div class="user-badge">
						...
						<?php echo H($person_info->first_name.' '.$person_info->last_name); ?>
							<?php if ($person_info->account_number):?>
							<div>
								<?php echo lang("customers_account_number") . ': '; ?>
								<?php echo $person_info->account_number; ?>
							</div>
							<?php endif; ?>
						...
						<ul class="list-inline pull-right">
                                                       <li><a target="_blank" href="<?php echo site_url('reports/generate/specific_customer?report_type=simple&report_date_range_simple=ALL_TIME&customer_id='.$person_info->person_id.'&sale_type=all&export_excel=0'); ?>" class="btn btn-success"><?php echo lang('common_view_report'); ?></a></li>
              ...
              <?php echo lang("customers_basic_information"); ?>
				...
				<div class="form-group">	
					...
					<div class="col-sm-9 col-md-9 col-lg-10">
						...
						$credit_limit_field = array(
							'name'=>'credit_limit',
							'id'=>'credit_limit',
							'class'=>'form-control credit_limit',
							'value'=>$person_info->person_id ? ($person_info->credit_limit ? to_currency_no_money($person_info->credit_limit) : '') : ($this->Employee->has_module_action_permission('customers', 'can_set_credit_limit', $this->Employee->get_logged_in_employee_info()->person_id) ? ($this->config->item('default_credit_limit')!== '' ? to_currency_no_money($this->config->item('default_credit_limit')): '') : to_currency_no_money(0)));
						 
							if (!$this->Employee->has_module_action_permission('customers', 'can_set_credit_limit', $this->Employee->get_logged_in_employee_info()->person_id))
							{
								$credit_limit_field['readonly'] = TRUE;
							}
						 
						 echo form_input($credit_limit_field);
				...
				{
					echo form_hidden('credit_limit', $person_info->person_id ? ($person_info->credit_limit ? to_currency_no_money($person_info->credit_limit) : '') : ($this->Employee->has_module_action_permission('customers', 'can_set_credit_limit', $this->Employee->get_logged_in_employee_info()->person_id) ? ($this->config->item('default_credit_limit')!=='' ? to_currency_no_money($this->config->item('default_credit_limit')): '') : to_currency_no_money(0)));
				?>
				...
				if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'simple')
				{
				   $sales_until_discount = (double)$this->config->item('number_of_sales_for_discount') - (double)$person_info->current_sales_for_discount;
