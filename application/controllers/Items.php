...
class Items extends Secure_area implements Idata_controller
{
	...
	function get_category($category_id = FALSE){
		...
		echo json_encode(array('success'=>true,'message'=> ' ', 'category_info' => $category_info));
	}
	...
	{	
		...
		if ($category_id = $this->Category->save($category_name, $hide_from_grid, $parent_id, $category_id, $category_color, $category_image_id,0,$exclude_from_e_commerce,$category_info_popup,$category_description))
		{
			...
			if ($this->input->post('category_tier'))
			{
				foreach($this->input->post('category_tier') as $tier_id => $price_or_percent)
				{
					if ($price_or_percent !== '')
					{
						$tier_data=array('tier_id'=>$tier_id);
						$tier_data['category_id'] = $category_id;
						...
						$this->Category->save_category_tiers($tier_data,$category_id);
					}
					else
					{
						$this->Category->delete_tier_price($tier_id, $category_id);
					}
				}
			}
			echo json_encode(array('success'=>true,'message'=>lang('items_category_successful_adding').' '.H($category_name), 'categories' => $categories, 'selected' => $category_id, 'category_name'=>$category_name ));
		}
		else
		{
	...
	function _category_tree_list($tree) 
	{
		...
		if(!is_null($tree) && count($tree) > 0) {
			...
			foreach($tree as $node){
				$return .='<li data-category-id=".'.$node->id.'"><span class="category_name">'.H($node->name). '</span> <a href="javascript:void(0);" class="add_child_category" data-category_id="'.$node->id.'">['.lang('items_add_child_category').']</a> '.
					'<a href="javascript:void(0);" class="edit_category" data-exclude_from_e_commerce="'.($node->exclude_from_e_commerce ? 1 : 0).'" data-color="'.H($node->color).'" data-image_id="'.H($node->image_id).'" data-image_timestamp="'.$this->Appfile->get_file_timestamp($node->image_id).'" data-name = "'.H($node->name).'" data-description = "'.H($node->category_description).'" data-info-popup = "'.H($node->category_info_popup).'" data-parent_id = "'.$node->parent_id.'" data-category_id="'.$node->id.'" data-tier-list='. ($node->tier_list ? json_encode($node->tier_list) : "").'>['.lang('common_edit').']</a> '.
	...
	*/
	...
	function view($item_id=-1, $sale_or_receiving = 'sale')
	{
       ...
       function related_products($item_id=-1)
       {
               $this->check_action_permission('add_update');

               $this->load->model('Item_related');
               ...
               $data['related_items'] = $this->Item_related->get_related_items($item_id)->result();

               $this->load->view("items/related_products",$data);
       }

       function save_related_products($item_id=-1)
       {
               $this->check_action_permission('add_update');
               $this->load->model('Item_related');
               ...
               $item_data = array(
                       'disable_related_sales_prompt' => $this->input->post('disable_related_sales_prompt') ? 1 : 0,
                       'disable_related_receivings_prompt' => $this->input->post('disable_related_receivings_prompt') ? 1 : 0,
               );

               $this->Item->save($item_data,$item_id);

               $related_items = $this->input->post('related_items') ? $this->input->post('related_items') : array();
               $this->Item_related->save($related_items,$item_id);

               $this->session->set_flashdata('manage_success_message', lang('common_saved_successfully'));

               redirect("items/related_products/$item_id".($redirect ? "?redirect=$redirect" : '').($progression ? "&progression=1" : '').($quick_edit ? "&quick_edit=1" : ''));
       }
	...
	{		
		...
		$item_data = array(
				...
				'non_profit_sale'=>$item_info->non_profit_sale ? $item_info->non_profit_sale : 0,
	...
	function qr_barcodes($item_id=-1,$offset=0)
	{
		$this->load->model('Item_location');
		$this->load->model('Item_variations');
		$this->load->model('Item_variation_location');
		...
		$this->load->view("items/qr_barcodes",$data);
	}
	
	function print_qr_barcodes()
	{
		$item_id = $this->input->post('item_id');
		$quantity = $this->input->post('items_number_of_qr_barcodes');
		...
		$skip = 0;
		$item_variations_number_of_barcodes = $this->input->post('item_variations_number_of_qr_barcodes');
		$variation_ids = false;
		...
		elseif(null !== $this->input->post('qr_code_labels_action'))
		{
			$this->generate_qr_code_labels($item_ids, $variation_ids);
		}
		elseif(null !== $this->input->post('qr_code_sheet_action'))
		{
			$skip = $this->input->post('skip');
			$this->generate_qr_codes($item_ids, $skip, $variation_ids);
		}
	}
	...
	function generate_qr_codes($item_ids, $skip=0, $variation_ids = false)
	{				
		$select_all_inventory=$this->get_select_inventory();
		...
		$data['items'] = $variation_ids ? get_item_variations_qr_code_data($variation_ids) : get_items_qr_code_data($item_ids);
		
		$data['scale'] = 1;
		$data['skip'] = $skip;
		
		$this->load->view("qr_code_sheet", $data);
	}

	function generate_qr_code_labels($item_ids, $variation_ids = false)
	{				
		$select_all_inventory=$this->get_select_inventory();
		...
		$this->load->helper('items');
		$data['items'] = $variation_ids ? get_item_variations_qr_code_data($variation_ids) : get_items_qr_code_data($item_ids);
		$data['excel_url'] = site_url('items/generate_qr_code_labels_excel/'.($item_ids ? $item_ids : '-1').'/'.$variation_ids);
		$this->load->view("qr_code_labels", $data);
	}
	
	function generate_qr_code_labels_excel($item_ids, $variation_ids = false)
	{
		if ($this->input->post('item_id'))
		{
			$item_id = $this->input->post('item_id');
			$quantity = $this->input->post('items_number_of_qr_codes');
	
			$item_ids_to_make = array_fill(0, $quantity, $item_id);
			$item_ids = implode('~', $item_ids_to_make);
				
			$skip = 0;
			$item_variations_number_of_qr_codes = $this->input->post('item_variations_number_of_qr_codes');
			$variation_ids = false;
		
			if($item_variations_number_of_qr_codes && count($item_variations_number_of_qr_codes) > 0)
			{			
				foreach($item_variations_number_of_qr_codes as $item_variation_id => $quantity)
				{
					for ($x = 1; $x <= $quantity; $x++)
					{
						$variation_ids .= $item_variation_id."~";
	...
	function generate_qr_codes_from_recv($recv_id, $skip=0)
	{
		$this->load->model('Item_taxes');
		$this->load->model('Item_location');
		$this->load->model('Item_location_taxes');
		...
		$data = array();
		$this->load->helper('items');
		$items_qr_codes = array();
		
		if (!empty($item_ids))
		{
			$items_qr_codes = get_items_qr_code_data(implode('~',$item_ids));
		}
		
		$variations_qr_codes = array();
		if (!empty($variation_ids))
		{
			$variations_qr_codes = get_item_variations_qr_code_data(implode('~',$variation_ids));
		}
		$data['items'] = array_merge($items_qr_codes, $variations_qr_codes);
		$data['items_expire'] = $items_expire;
		$data['scale'] = 1;
		$data['from_recv'] = $recv_id;
		$data['skip'] = $skip;
	...
	function generate_qr_codes_labels_from_recv($recv_id)
	{
		$this->load->model('Item_taxes');
		$this->load->model('Item_location');
		$this->load->model('Item_location_taxes');
		...
		$data['items_expire'] = $items_expire;
		$data['from_recv'] = $recv_id;
		$data['excel_url'] = site_url('items/generate_qr_codes_labels_from_recv_excel/'.$recv_id);
		$this->load->view("qr_code_labels", $data);
	}
	
	function generate_qr_codes_labels_from_recv_excel($recv_id)
	{
		$this->load->model('Item_taxes');
		$this->load->model('Item_location');
		$this->load->model('Item_location_taxes');
	...
	function generate_qr_codes_from_count($count_id, $skip=0)
	{
		$this->load->model('Item_taxes');
		$this->load->model('Item_location');
		$this->load->model('Item_location_taxes');
	...
	function generate_qr_codes_labels_from_count($count_id)
	{
		$this->load->model('Item_taxes');
		$this->load->model('Item_location');
		$this->load->model('Item_location_taxes');
			...
			{	
				if($this->Employee->is_location_authenticated($location_id)){
					...
					$data = array(
						...
						'limited_sales_quantity'=> isset($item_location_data['limited_sales_quantity']) && $item_location_data['limited_sales_quantity'] ? $item_location_data['limited_sales_quantity'] : NULL,
					...
					if (isset($item_location_data['item_variations']))
					{
						foreach($item_location_data['item_variations'] as $item_variation_id=>$item_variation_location_data)
						{
							...
							if ($item_variation_location_data['limited_sales_quantity']==='')
							{
								$item_variation_location_data['limited_sales_quantity'] = NULL;
							}
	...
	function save_item_pricing($item_id=-1)
	{
		...
		$item_data = array(
			...
			'non_profit_sale'=>$this->input->post('non_profit_sale') ? $this->input->post('non_profit_sale') : 0,
	...
	function save_variations($item_id=-1)
	{
		...
		if ($this->input->post('item_variations') && is_array($this->input->post('item_variations'))){
			...
			if(count($attribute_values) > count(array_unique($attribute_values))){
				echo json_encode(array('success'=>false,'message'=>lang('items_duplicate_attributes_are_not_permitted'),'item_id'=>-1));
				exit;
			}
		}
	...
	function save_inventory($item_id=-1)
	{
		...
		'limited_sales_quantity' => $this->input->post('limited_sales_quantity') ? intval($this->input->post('limited_sales_quantity')) : NULL,
		);
		...
		if(!empty($item_variations))
		{
			...
			foreach($item_variations as $item_variation_id => $item_variation)
			{
				...
				$variation_data = array(
					...
					'limited_sales_quantity' => $item_variation['limited_sales_quantity'] == '' ? null : $item_variation['limited_sales_quantity']
				);
		...
		if($error)
		{
			...
			if (isset($this->ecom_model) && strtolower(get_class($this->ecom_model)) == 'shopify')
			{
				$this->ecom_model->update_inventory_policy($item_id);
			}		
	...
	function bulk_update()
	{
		...
		{
			if ($key == 'submit' || $key == 'tags' || $key == 'tags_replace' || $key == 'tier_types' || $key == 'tier_values' || $key == "image_files" || $key == "replace_images" )
			{
				continue;
			}
		...
		if(empty($item_data) || $this->Item->update_multiple($item_data,$items_to_update,$select_inventory))
		{
			...
			if ($this->input->post('tags'))
			{
				if ($select_inventory == 0)
				{
					...
					{
						$this->Tag->save_tags_for_item($item_id, $this->input->post('tags'), $this->input->post('tags_replace'));
					}
				}
				else
				{
					...
					{
						$this->Tag->save_tags_for_item($item->item_id, $this->input->post('tags'), $this->input->post('tags_replace'));
					}
				}
			}
	...
	function _excel_get_header_row($include_location_columns = 0)
	{
		...
		if($this->config->item("ecommerce_platform"))
		{
			$header_row[] = lang('items_is_ecommerce');
                ...
                $header_row[] = lang('common_image_url');
	...
	function price_list($offset=0,$export_excel = 0)
	{
		$params = $this->session->userdata('items_search_data') ? $this->session->userdata('items_search_data') : array('offset' => 0, 'order_col' => 'name', 'order_dir' => 'asc', 'search' => FALSE, 'category_id' => FALSE, 'fields' => 'all','deleted' => 0);
		...
		$config = array();
		$config['base_url'] = site_url("items/price_list");
		...
		{
			$offset = 0;
			$config['per_page'] = 9000000;
		}
		
		if ($search || $category_id)
		{
			$result_data = $this->Item->search($search,$params['deleted'],$category_id,$config['per_page'],$offset,$params['order_col'],$params['order_dir'])->result_object();
			$config['total_rows'] = $this->Item->count_last_query_results();
		}
		else
		{
			$result_data = $this->Item->get_all($params['deleted'],$config['per_page'],$offset,$params['order_col'],$params['order_dir'])->result_object();
			$config['total_rows'] = $this->Item->count_last_query_results();
		}
		...
		foreach($result_data as $row)
		{
			...
			{
				$row_array['tier_'.$tier->id] = $this->Item->get_sale_price(array('item_id' => $row->item_id,'tier_id' => $tier->id,'variation_id' => NULL,'quantity_unit_quantity' => NULL)); 
			}
			
			$data[] = $row_array;
			
			$variations = $this->Item_variations->get_variations($row->item_id);
			...
			if (count($variations) > 0)
			{
				...
				foreach($variations as $var_id=>$var_info)
				{
					$row_array = array(
						...
						'supplier' => '',
						'unit_price' => to_currency_no_money($var_info['unit_price'] ?? $row->unit_price),
					);
					
					foreach($tiers as $tier)
					{
						$row_array['tier_'.$tier->id] = $this->Item->get_sale_price(array('item_id' => $row->item_id,'tier_id' => $tier->id,'variation_id' => $var_id,'quantity_unit_quantity' => NULL)); 
					}
					$data[] = $row_array;
		...
		if ($export_excel)
		{
			...
			foreach($tiers as $tier)
			{
				$header_row['tier_'.$tier->id] = $tier->name;
			}
			
			array_unshift($data,$header_row);
			
			array_to_spreadsheet($data,'price_list.'.($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
			
		}
		else
		{
			$this->load->view("items/price_list", array('items' => $data, 'tiers' => $tiers, 'pagination' => $pagination));
		}
	}
	...
	{		
		...
		if($this->config->item("ecommerce_platform"))
		{
			$fields[] = array('Name' => lang('items_is_ecommerce'), 'key' => 'is_ecommerce');
                ...
                $fields[] = array('Name' => lang('common_image_url'), 'key' => 'image_url');
	...
	function complete_excel_import()
	{
		...
		for ($i = 0; $i < $numRows -1; $i++)
		{
						...
						else
						{
							if (isset($additional_item_numbers))
							{
								unset($additional_item_numbers);
                                       ...
                                       elseif($field['key'] == 'image_url')
                                       {
                                               $image_urls = $columns_with_data[$key]['data'][$i];
                                       }
                                       else
                                       {
			...
			if($item_id_key){
				...
				if (!$variation && strpos($columns_with_data[$item_id_key]['data'][$i],'@') !== FALSE)
				{
					continue;
                        ...
                        $is_new_item = $item_id ? FALSE : TRUE;
			...
			$item_id = isset($item_data['item_id']) ? $item_data['item_id'] :  $item_id;
                       ...
                       if ($is_new_item && $image_urls)
                       {
                               $urls = array_map('trim', explode('|', $image_urls));
                               foreach($urls as $idx => $url)
                               {
                                       ...
                                       @$image_contents = file_get_contents_curl($url);
                                       $tmpFilename = tempnam(ini_get('upload_tmp_dir') ? ini_get('upload_tmp_dir') : sys_get_temp_dir(), 'import');
                                       ...
                                       if ($image_contents)
                                       {
                                               $image_file_id = $this->Appfile->save(clean_filename($url), $image_contents);
                                               $this->Item->add_image($item_id, $image_file_id);
