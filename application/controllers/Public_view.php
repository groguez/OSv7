...
class Public_view extends MY_Controller 
{
	...
	{ 
		...
		if (!$this->session->userdata('employee_current_location_id'))
		{
			$this->session->set_userdata('employee_current_location_id',$sale_info['location_id']);
		}
		
		$receipt_cart = PHPPOSCartSale::get_instance_from_sale_id($sale_id, NULL, FALSE, TRUE, TRUE);
	...
	function pay_sale($sms_sale_id)
	{

		require_once (APPPATH."libraries/hashids/vendor/autoload.php");
		
		$this->load->model('Sale');	
		$sale_id = do_decrypt($sms_sale_id,$this->Appconfig->get_secure_key());
		if ($sale_id === FALSE)
		{
			return;
		}
	...
	public function start_cc_processing()
	{
		...
		$type 					= $this->input->post('type'); //2 - sales, null - invoice
		...
		{
			if($type == 2){//suspended sales
				$credit_card_processor->do_start_cc_processing_without_login_suspended_sales($id);
			}else{
				$credit_card_processor->do_start_cc_processing_without_login($cc_amount, $total, $id, $type);
			}
		}
		else
		{
	...
	function _get_cc_processor($emp_location_id = NULL)
	{
		...
		if ($this->Location->get_info_for_key('credit_card_processor', $emp_location_id) == 'coreclear2')
		{
		...
		elseif ($this->Location->get_info_for_key('credit_card_processor', $emp_location_id) == 'valor')
		{
			require_once (APPPATH.'libraries/Valorprocessor.php');
			$credit_card_processor = new Valorprocessor($this, $emp_location_id);
		}
	...
	function view_payment_receipt2($id)
	{
		$sale_id = do_decrypt($id,$this->Appconfig->get_secure_key());
		$this->load->view("invoices/payment_receipt",$data);
	}
			...
			if (count($this->input->post()))
			{
				...
				'comments'		=>	$this->input->post('comments') ? $this->input->post('comments') : '',
                               ...
                               for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++)
                               {
                                       ...
                                       {
                                               $value = $this->input->post("custom_field_{$k}_value");
                                               if($this->Customer->get_custom_field($k,'type') == 'date')
                                               {
                                                       $customer_data["custom_field_{$k}_value"] = $value !== '' ? strtotime($value) : NULL;
                                               }
                                               elseif($this->Customer->get_custom_field($k,'type') == 'checkbox')
                                               {
                                                       $customer_data["custom_field_{$k}_value"] = $value ? 1 : 0;
                                               }
                                               else
                                               {
                                                       $customer_data["custom_field_{$k}_value"] = $value;
                                               }
                                       }
                               }
