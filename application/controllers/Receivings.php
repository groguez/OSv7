...
{	
	...
	function item_search()
	{
		...
		if($this->input->get('mode') == 'transfer'){
			$location_from_id = $this->input->get('location_from');
			$location_to_id = $this->input->get('location_to');
			foreach($suggestions as $key => &$value){
				if($location_from_id){
					if( strpos($value['value'], '#') > -1 ){
						$item_variation_location_qty = $this->Item_variation_location->get_location_quantity(explode('#', $value['value'])[1], $location_from_id);
						$value['location_from_qty'] = to_quantity($item_variation_location_qty);
					} else {
						$item_location_qty = $this->Item_location->get_location_quantity($value['value'], $location_from_id);
						$value['location_from_qty'] = to_quantity($item_location_qty);
					}
				}
				if($location_to_id){
					if( strpos($value['value'], '#') > -1 ){
						$item_variation_location_qty = $this->Item_variation_location->get_location_quantity(explode('#', $value['value'])[1], $location_to_id);
						$value['location_to_qty'] = to_quantity($item_variation_location_qty);
					} else {
						$item_location_qty = $this->Item_location->get_location_quantity($value['value'], $location_to_id);
						$value['location_to_qty'] = to_quantity($item_location_qty);
					}
				}
			}
		}
	...
	function add()
	{
		...
		if($advanced_item_len > 0){
			$barcode_scan_list = explode("-", $this->input->post("item"));
			foreach($barcode_scan_list as $barcode_scan_element){
				$this->cart->process_barcode_scan(
					$barcode_scan_element, 
					array(
	...
	function add_to_receiving(){
		...
		try{
			foreach($item_list as $item_id){
				...
				$this->cart->sort_clean();
				if($this->cart->is_valid_receipt($barcode_scan_data) && $this->cart->get_mode()=='receive')
				{
					continue;
				}

				$this->cart->process_barcode_scan($barcode_scan_data,array('quantity' => $quantity));
				$this->cart->save();
			}
			$this->_reload();
		}
		catch (Exception $e) {
			echo json_encode(array('success'=>true,'message'=>lang('items_cannot_add_to_receiving')));
			exit;
		}
		...
		echo json_encode(
			array(
				'success'=>true,
				'message'=>lang('items_successfully_added_to_receiving'),
				'confirm_go_to_sale_screen'=>lang('common_ask_go_to_receiving_screen'),
				'sale_url'=>site_url("receivings"),
			));
		exit;
	}
	...
	public function edit_item_supplier($line)
	{
		if ($item = $this->cart->get_item($line))
		{
			...
			if(!$this->Item->has_variations($item->item_id) && 
				($this->Item->has_secondary_supplier($item->item_id) || $item_info->supplier_id == $supplier_id)){
	...
	function edit_item($line)
	{
		...
		$this->form_validation->set_rules('location_selling_price', 'lang:common_price', 'numeric');
				...
				if ($item = $this->cart->get_item($line))
				{
					...
					try
					{
						...
						{							
							...
							{
								$new_selling_price = $this->input->post("value");

								// Convert to float for comparison, handling potential locale differences
								$new_selling_price_float = (float)$new_selling_price;
								$cost_price_float = (float)$item->cost_price; // Cost price from the cart item

								if ($new_selling_price_float < $cost_price_float)
								{
									// Throw an exception to be caught by the catch block below
									throw new Exception(lang('common_selling_price_must_be_at_least_cost_price'));
								}
							}
							// End Validation
							
							// Validation: Selling price cannot be below cost price
							if ($variable == 'location_selling_price')
							{
								$new_selling_price = $this->input->post("value");
					...
					{
						// Use the message from the thrown exception
	...
	{		
		...
		if ($this->cart->transfer_from_location_id && !$this->Employee->has_module_action_permission('receivings', 'complete_transfer', $this->Employee->get_logged_in_employee_info()->person_id))
		{
			...
			return;				
                ...
                $do_not_allow_out_of_stock_transfer = $this->Location->get_info_for_key('do_not_allow_items_to_go_out_of_stock_when_transfering');
                if ($do_not_allow_out_of_stock_transfer === '' || $do_not_allow_out_of_stock_transfer === NULL)
                {
                        $do_not_allow_out_of_stock_transfer = $this->config->item('do_not_allow_items_to_go_out_of_stock_when_transfering');
                }
                if ($this->cart->get_mode() == 'transfer' && $do_not_allow_out_of_stock_transfer && $this->cart->transfer_from_location_id && $this->cart->will_be_out_of_stock())
		{
		...
		if ($this->cart->get_mode() == 'receive')
		{
			...
			for ($k = 1; $k <= NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k++) 
			{ 
				if ($this->Receiving->get_line_item_custom_field($k) !== FALSE)
				{
					$custom_field = $this->Receiving->get_line_item_custom_field($k);
				
					foreach($this->cart->get_items() as $line => $item)
					{
						if($this->Receiving->get_line_item_custom_field($k,'required') && in_array($current_location,$this->Receiving->get_line_item_custom_field($k,'locations')) && !$item->{'line_item_custom_field_'.$k.'_value'})
						{
							$this->_reload(array('error' => $custom_field.' '.lang('is_required')), false);
							return FALSE;
						}
		...
		if( $this->cart->get_mode() == 'store_account_payment' && $receiving_id_raw > 0 ){
			$this->Invoice->update_store_account_payment_invoice('supplier', $this->cart);
		}
  ...
  function cancel_receiving()
  {
	...
	function create_po()
	{
		...
		foreach($item_ids as $item_id)
		{
        	...
        	$cur_item_info = $this->Item->get_info($item_id);
					...
					$replenish_level = ($cur_item_location_info && $cur_item_location_info->replenish_level > 0) ? $cur_item_location_info->replenish_level : $cur_item_info->replenish_level;
					$reorder_level = ($cur_item_location_info && $cur_item_location_info->reorder_level > 0) ? $cur_item_location_info->reorder_level : $cur_item_info->reorder_level;
	...
	{
		$this->lang->load('config');
		$fields_prefs = $this->config->item('receiving_line_item_custom_field_prefs') ? unserialize($this->config->item('receiving_line_item_custom_field_prefs')) : array();
		$data = array_merge(array('controller_name' => strtolower(get_class())),$fields_prefs);
		$locations_list = $this->Location->get_all()->result();
		$data['locations'] = $locations_list;
	...
	{
		$this->load->model('Appconfig');
		$this->Appconfig->save('receiving_line_item_custom_field_prefs',serialize($this->input->post()));
	}
	...
	function save_line_item_custom_field()
	{
		...
		if( $this->Receiving->get_line_item_custom_field($k) !== FALSE )
		{
			...
			if($this->Receiving->get_line_item_custom_field($k,'type') == 'date')
			{
				$line_item->{'line_item_custom_field_'.$k.'_value'} = (string)strtotime($this->input->post('value'));
			}
			elseif($this->Receiving->get_line_item_custom_field($k,'type') == 'image')
			{
				$this->load->library('image_lib');
