...
class Invoices extends Secure_area
{
	function __construct()
	{
		...
		//We need this for valor processing so it does NOT error out
	...
	function sorting($type='customer')
	{
		...
		$search 	= $this->input->post('search') ? $this->input->post('search') : $params['search'];
		$status 	= $this->input->post('status') ? $this->input->post('status') : $params['status'];
	...
	function index($type='customer',$offset=0)
	{
		...
		$params = $this->session->userdata($this->invoice_type.'_invoices_search_data') ? $this->session->userdata($this->invoice_type.'_invoices_search_data') : array('offset' => 0, 'order_col' => 'invoice_id', 'order_dir' => 'desc', 'search' => FALSE,'deleted' => 0,'days_past_due' => NULL, 'status' => 2);
		...
		{	
			$config['total_rows'] = $this->Invoice->count_all($this->invoice_type,$data['days_past_due'],$params['deleted'], isset($params['status']) ? $params['status'] : NULL );
			$table_data = $this->Invoice->get_all($this->invoice_type,$data['days_past_due'],$params['deleted'],$data['per_page'], $params['offset'],$params['order_col'],$params['order_dir'], isset($params['status']) ? $params['status'] : NULL );
		}
	...
	*/
	...
	function reload_invoice_table($type='customer')
	{
		...
		if ($data['search'])
		{
			$config['total_rows'] = $this->Invoice->search_count_all($this->invoice_type,$data['search'],$data['days_past_due'], $params['deleted'],isset($params['status']) ? $params['status'] : NULL);
			$table_data = $this->Invoice->search($this->invoice_type,$data['search'],$data['days_past_due'], $params['deleted'],$data['per_page'],$params['offset'],$params['order_col'],$params['order_dir'], isset($params['status']) ? $params['status'] : NULL);
		}
		else
		{
	...
	function pay($type,$invoice_id)
	{
		...
		$this->load->model('Sale');
               ...
               {
                       $payment_types[$additional_payment_type] = $additional_payment_type;
               }
		...
		$is_credit_card_processor_we_can_accept_payments_for_invoices = $this->Location->get_info_for_key('credit_card_processor') == 'coreclear' || $this->Location->get_info_for_key('credit_card_processor') == 'coreclear2' || $this->Location->get_info_for_key('credit_card_processor') == 'valor';
		$data['is_credit_card_processor_we_can_accept_payments_for_invoices'] = $is_credit_card_processor_we_can_accept_payments_for_invoices;
	...
	function process_payment($type,$invoice_id)
	{
		...
		$is_credit_card_processor_we_can_accept_payments_for_invoices = $this->Location->get_info_for_key('credit_card_processor') == 'coreclear' || $this->Location->get_info_for_key('credit_card_processor') == 'coreclear2' || $this->Location->get_info_for_key('credit_card_processor') == 'valor';
		if ($type == 'customer' && $payment_type == lang('common_credit') && $is_credit_card_processor_we_can_accept_payments_for_invoices)
		{
			if ($register == -2)
			{
			...
			$process_payment_response = $this->Invoice->process_payment($amount,$register,$cc_token,$cc_number,$ccv,$expire_month,$expire_year,$address,$zip,$invoice_id);
			...
			if($process_payment_response['success'])
			{
				...
				$store_account_payment_sale_id = false;
				$store_account_payment_sale_id = $this->Invoice->create_store_account_transaction($type, $invoice_info, $payment_data);
				...
				$this->Invoice->save($type, $invoice_data, $invoice_id, $store_account_payment_sale_id > 0 ? true : false);

				// Send email alert for invoice payment
				$this->Invoice->send_payment_email_alert($type, $invoice_id, $payment_data['payment_amount'], $invoice_data['balance']);
