...
class Sales extends Secure_area
{
	...
	{		
       ...
       $currency_info = $this->Location->get_currency_info($location_id);
       if($currency_info && !$this->cart->get_exchange_details())
       {
               $rate = $currency_info->exchange_rate.'|'.$currency_info->currency_code_to.'|'.$currency_info->currency_symbol.'|'.$currency_info->currency_symbol_location.'|'.$currency_info->number_of_decimals.'|'.$currency_info->thousands_separator.'|'.$currency_info->decimal_point;
		...
		if ($this->config->item('track_payment_types') && !empty($track_payment_types)) 
		{
			...
			if ($this->input->post('opening_amount') != '' && !$this->Register->is_register_log_open())  
			{
				...
				{
					//Use alphabet as rep. of space since NOT allowed in from array keys
					$payment_type = str_replace('ABCDEFGHIJKLMNOPQRSTUVWXYZ', ' ', $payment_type);
	...
	function select_customer()
	{
		...
		{
			$warning = $this->check_customer_balance($this->cart->customer_id);
			if (!empty($warning)) {
				$data['customer_balance_warning'] = $warning;
			}
		}
	...
	function change_mode($mode = false, $redirect = false)
	{
		...
		else
		{
			...
			if ($previous_mode == 'sale' && $mode =='return')
			{
				...
				{
					$data  = array('prompt_convert_sale_to_return' => FALSE, 'prompt_for_return_sale_id' => TRUE, 'replace_cart' => TRUE);
				}
				else
				{
	...
	function set_tier_id() 
	{
	  ...
	  foreach($items as $line=>$item)
	  {
	  	  ...
	  	  {
                                 $do_not_allow_below_cost = $this->Location->get_info_for_key('do_not_allow_below_cost');
                                 if ($do_not_allow_below_cost === '' || $do_not_allow_below_cost === NULL)
                                 {
                                         $do_not_allow_below_cost = $this->config->item('do_not_allow_below_cost');
                                 }
                                 if ($do_not_allow_below_cost)
				  {
	...
	{		
		/**
		 * Checks if a valid customer is selected with a suitable payment type and ensures the customer's balance is negative.
		 *
		 * @return void Immediately reloads the view with a warning message.
		 */
		if (
			$this->cart->customer_id
			&& $this->input->post('payment_type') !== lang('common_store_account')
			&& $this->session->userdata('customer_balance_negative') === 0
		) {
			if ($warning = $this->check_customer_balance($this->cart->customer_id)) {
				$data['customer_balance_warning'] = $warning;
				$this->_reload($data);
				return;
			}
		...
		elseif ( $payment_type == lang('common_giftcard') )
		{
			...
			if($this->config->item('enable_giftcard_security')){
				$giftcard_pin = $this->input->post('giftcard_pin');
				$giftcard_id = $this->Giftcard->get_giftcard_id_from_number_pin($giftcard_number, $giftcard_pin);
			} else {
				$giftcard_id = $this->Giftcard->get_giftcard_id($giftcard_number);
			}

			if(!$this->Giftcard->exists($giftcard_id)
		...
		if($payment_type == lang('common_store_account')){
			if($this->config->item('do_not_allow_charge_to_store_account_greater_than_amount_due')){
				if($payment_amount > $this->cart->get_amount_due()){
					$data['error'] = lang('common_customer_cannot_charge_to_store_account_greater_than_amount_due');
					$this->_reload($data);
					return;
				}
			}
			if($this->config->item('customer_with_negative_store_account_balances_must_keep_balance_at_or_below_0')){
				$customer_info = $this->Customer->get_info($this->cart->customer_id);
				
				$current_store_account_amount = $this->cart->get_payment_amount(lang('common_store_account'));
				
				if((-$customer_info->balance) <  $payment_amount + $current_store_account_amount){
					$data['error'] = lang('common_customer_with_negative_store_account_balances_must_keep_balance_at_or_below_0');
					$this->_reload($data);
					return;
				}
			}
		...
		if($markup_markdown && $this->cart->get_total() > 0 && !$this->Location->get_info_for_key('disable_markup_markdown'))
		{
			...
			if (isset($markup_markdown_config[$this->input->post('payment_type')]) && $markup_markdown_config[$this->input->post('payment_type')])
			{
				...
				$fee_amount = to_currency_no_money( min(($this->cart->get_total() +  (-$fee_already_added)), ($total_payment_amount - $fee_already_added)) * ( $fee_percent/100 ) );
				...
				$payment_amount += ($fee_amount - $fee_already_added);
	...
	{				
		...
		$this->cart->sort_clean();
                ...
                {
                        $data = array('prompt_edit_or_return_sale' => TRUE,'scanned_sale_id' => $barcode_scan_data);
                        $this->_reload($data);
                        return;
                }

		$advanced_item_len = $this->input->post("advanced_item_len");
		if($advanced_item_len > 0){
			$barcode_scan_list = explode("-", $barcode_scan_data);
			foreach($barcode_scan_list as $barcode_scan_element){
				$this->cart->process_barcode_scan($barcode_scan_element, array('quantity' => $quantity,'run_price_rules' => TRUE, 'secondary_supplier_id' => $secondary_supplier_id, 'default_supplier_id'=> $default_supplier_id));
			}
		}else{
	...
	function add_to_sale(){
		$item_list=$this->input->post('ids');
		$item_type=$this->input->post('item_type');
		try{
			foreach($item_list as $item_id){
				$barcode_scan_data = "";
				if($item_type == 'item_kit'){
					$barcode_scan_data = "KIT ".$item_id."|FORCE_ITEM_ID|";
				}else{
					$barcode_scan_data = $item_id."|FORCE_ITEM_ID|";
				}
				$quantity = 1;
			...
			echo json_encode(array('success'=>true,'message'=>lang('items_cannot_add_to_sale')));
			exit;
		}
		...
		echo json_encode(
			array(
				'success'=>true,
				'message'=>lang('items_successfully_added_to_sale'),
				'confirm_go_to_sale_screen'=>lang('common_ask_go_to_sale_screen'),
				'sale_url'=>site_url("sales"),
			));
		exit;
	}
	...
	public function edit_item_sales_person($line)
	{
		if ($item = $this->cart->get_item($line))
		{
			$sales_person_id = $this->input->post('value');
			if($sales_person_id > 0){
				$employee_info = $this->Employee->get_info($sales_person_id);
				$item->sold_by_employee_id_2 = $employee_info->person_id;
			}else{
				$item->sold_by_employee_id_2 = NULL;
			}
			$this->cart->save();
		}
		$this->_reload();
	...
	public function edit_item_supplier($line)
	{
		if ($item = $this->cart->get_item($line))
		{
			...
			if(!$this->Item->has_variations($item->item_id) && ($this->Item->has_secondary_supplier($item->item_id) || $item_info->supplier_id == $supplier_id)){
				...
				$this->cart->save();
				$this->_reload();
			}else if ($this->Item->has_variations($item->item_id)){
				$selected_supplier = $item_info->supplier_id;
				$item_variation_info = $this->Item_variations->get_info($item->variation_id);

				if($item_variation_info->supplier_id){
					$selected_supplier = $item_variation_info->supplier_id;
				}

				if(
					$this->Item->has_secondary_supplier($item->item_id)
					...
					if(!empty($attribute_group) && !empty($variation_group)){
						...
						if($variation_id){
							...
							// error
							$data['state'] = 0;
							$data['message']=lang('common_can_not_set_item_variation_supplier');
				...
				// error
				$data['state'] = 0;
				$data['message']=lang('common_can_not_set_item_supplier');
				echo json_encode($data);
				return;
			}
		}
	...
	function edit_item($line, $sub_line = 0)
	{
		...
		if ($this->cart->get_mode() == 'store_account_payment' && $this->config->item('do_not_allow_store_payment_more_than_balance')) 
		{
			$customer_info 		= $this->Customer->get_info($this->cart->customer_id);
			$customer_balance 	= $customer_info->balance;
			if ($$variable > $customer_balance)
			{
				$data['error'] = lang('common_store_account_payment_amount_greater_than_balance');
				$this->_reload($data);
				return;
			}
		}

		if ($this->cart->get_mode() == 'store_account_payment' && $this->config->item('require_payment_against_store_account_charge')) 
		{
			if (empty($this->cart->get_paid_store_account_ids())) {
				$data['error'] = lang('config_require_payment_against_store_account_charge');
				$this->_reload($data);
				return;
			}
		}
		...
		try
		{
			if($variable == "modifier_price"){
				...
				if($this->cart->get_mode() == 'sale'){
					if(get_class($item) == 'PHPPOSCartItemKitSale'){
						$this->load->model('Item_kit');
						
						$cur_item_location_kit_info = $this->Item_kit_location->get_info($item->item_kit_id);
						if($cur_item_location_kit_info->limited_sales_quantity > 0 && $$variable > $cur_item_location_kit_info->limited_sales_quantity){
							$data['error'] = lang('items_kit_quantity_has_exceeded_the_limited_sales_quantity');
							$this->_reload($data);
							return;
						}
						
						$cur_item_kit_info = $this->Item_kit->get_info($item->item_kit_id);
						if($cur_item_kit_info->limited_sales_quantity > 0 && $$variable > $cur_item_kit_info->limited_sales_quantity){
							$data['error'] = lang('items_kit_quantity_has_exceeded_the_limited_sales_quantity');
							$this->_reload($data);
							return;
						}
						...
						if($item->variation_id > 0){
							...
							$cur_item_variation_location_info = $this->Item_variation_location->get_info($item->variation_id);
							if($cur_item_variation_location_info->limited_sales_quantity > 0 && $$variable > $cur_item_variation_location_info->limited_sales_quantity){
								$data['error'] = lang('items_variation_quantity_has_exceeded_the_limited_sales_quantity');
								$this->_reload($data);
								return;
							}
							...
							$cur_item_variation_info = $this->Item_variations->get_info($item->variation_id);
							if($cur_item_variation_info->limited_sales_quantity > 0 && $$variable > $cur_item_variation_info->limited_sales_quantity){
								$data['error'] = lang('items_variation_quantity_has_exceeded_the_limited_sales_quantity');
								$this->_reload($data);
								return;
							}
							...
							if($cur_item_location_info->limited_sales_quantity > 0 && $$variable > $cur_item_location_info->limited_sales_quantity){
								$data['error'] = lang('items_quantity_has_exceeded_the_limited_sales_quantity');
								$this->_reload($data);
								return;
							}
							...
							if($cur_item_info->limited_sales_quantity > 0 && $$variable > $cur_item_info->limited_sales_quantity){
								$data['error'] = lang('items_quantity_has_exceeded_the_limited_sales_quantity');
								$this->_reload($data);
								return;
							}
				...
				} elseif($variable == 'flat_discount_amount') {
					$item->discount = 0;
				}
			}
		...
		if (isset($discount) && $discount !== NULL)
		{
			...
			$max_discount_flat = 0;
			if($max_discount > 0){
				$max_discount_flat = floatval($item->unit_price * $max_discount / 100);
			}

			if(!$can_override_price_adjustments & $max_discount!==NULL && floatval($flat_discount_amount) > floatval($max_discount_flat))
			{
				$item->flat_discount_amount = $max_discount_flat;
				$data['warning'] = lang('sales_could_not_discount_item_above_max')." ".to_currency($max_discount_flat);
			}
		}

		$can_edit = TRUE;

                $do_not_allow_out_of_stock = $this->Location->get_info_for_key('do_not_allow_out_of_stock_items_to_be_sold');
                if ($do_not_allow_out_of_stock === '' || $do_not_allow_out_of_stock === NULL)
                {
                        $do_not_allow_out_of_stock = $this->config->item('do_not_allow_out_of_stock_items_to_be_sold');
                }
                if ($do_not_allow_out_of_stock)
		{
			if (isset($quantity))
			{
		...
		if($can_edit && isset($unit_price))
		{
			...
			if(!$can_override_price_adjustments && isset($max) && floatval($unit_price) > floatval($max))
			{
                ...
                if($this->cart->get_item($line)->out_of_stock() && !$do_not_allow_out_of_stock)
		{
	...
	function delete_customer()
	{
		...
		$this->session->unset_userdata('customer_balance_negative');
		$this->cart->save();
		$this->_reload();
	}
	...
	function start_cc_processing()
	{
		...
		if($this->cart->sale_id){
			...
			if($sale_info['deleted'] == 1){
				$this->cart->empty_items();
				$this->cart->save();
				$this->session->set_flashdata('edit_failed_message', lang('sales_cannot_edited_already_deleted'));
				redirect(site_url('sales'));
			}
		}
		...
		{
			$this->_reload(array('error' => lang('sales_receipt_required_for_return'), 'prompt_for_return_sale_id' => TRUE, 'replace_cart' => FALSE), false);
			return;
		}
		...
		{
			$this->_reload(array('error' => lang(
				'sales_credit_card_payment_is_greater_than_total_cannot_complete')), false);
		}
	...
	public function start_cc_processing_valor()
	{
		$credit_card_processor = $this->_get_cc_processor();
	...
	private function _validate_custom_fields()
	{
		...
		for ($k = 1; $k <= NUMBER_OF_PEOPLE_CUSTOM_FIELDS; $k++) 
		{ 
			if ($this->Sale->get_line_item_custom_field($k) !== FALSE)
			{
				$custom_field = $this->Sale->get_line_item_custom_field($k);
				
				foreach($this->cart->get_items() as $line => $item)
				{
					if($this->Sale->get_line_item_custom_field($k,'required') && in_array($current_location,$this->Sale->get_line_item_custom_field($k,'locations')) && !$item->{'line_item_custom_field_'.$k.'_value'})
					{
						$this->_reload(array('error' => $custom_field.' '.lang('is_required')), false);
						return FALSE;
					}
	...
	function complete()
	{
		...
		if($this->config->item('do_not_allow_sales_with_zero_value_line_items')){
			...
			foreach($this->cart->get_items() as $line => $item)
			{
				$line_total = $item->flat_discount_amount > 0 ? ($item->unit_price - $item->flat_discount_amount) * $item->quantity : $item->unit_price * $item->quantity - $item->unit_price * $item->quantity * $item->discount / 100;
				if($line_total == 0)
				{
					$this->session->set_flashdata('error_if_total_is_zero', lang('common_error_if_total_is_zero_line_item'));
					redirect('sales');
		...
		if ($this->config->item('do_not_allow_item_with_variations_to_be_sold_without_selecting_variation') && !$this->cart->do_all_variation_items_have_variation_selected())
		{
			...
			return;
                ...
                if ($this->cart->get_mode() != 'return' && $this->cart->get_mode() != 'estimate' && $do_not_allow_out_of_stock)
		{
			foreach($this->cart->get_items() as $item)
			{
		...
		if($data['balance'] > 0 && !$customer_id){
			$this->_reload(array('error' => lang('sales_customer_required_store_account')), false);
			return;
		}
		...
		$tran_no = $this->session->userdata('tran_no') ? $this->session->userdata('tran_no') : '';
		...
		$cash_discount = $this->session->userdata('cash_discount') ? $this->session->userdata('cash_discount') : 0;
		...
		$this->cart->suspended = 0;
                ...
                if($customer_id)
                {
                        $this->Customer->update_auto_tier($customer_id);
                }

		if( $this->cart->get_mode() == 'store_account_payment' && $sale_id_raw > 0 ){
			$this->Invoice->update_store_account_payment_invoice('customer', $this->cart);
		}
		...
		if($customer_id)
		{
			...
			$title = $this->Person->get_title_as_string($customer_id);
			
			$data['customer']= ($title ? $title.' ' : '').$cust_info->first_name.' '.$cust_info->last_name.($cust_info->account_number==''  ? '':' - '.$cust_info->account_number);
			$data['customer_company']= $cust_info->company_name;
			$data['customer_address_1'] = $cust_info->address_1;
			$data['customer_address_2'] = $cust_info->address_2;
			$data['customer_city'] = $cust_info->city;
		...
		if($email_send === true || (isset($cust_info) && $cust_info->auto_email_receipt == 1)){
			...
			if($this->config->item('enable_pdf_receipts'))
			{
				if(isset($pdf_content) && $pdf_content)
				{
					...
					if ( $this->config->item('allow_customers_to_pay_suspended_sales_online') && ($this->Location->get_info_for_key('credit_card_processor') == 'coreclear2' || $this->Location->get_info_for_key('credit_card_processor') == 'valor') && $this->cart->get_amount_due() > 0 ) 
					{
						$payment_link = '<br />'.anchor($this->Sale->get_processor_payment_link($sale_id_raw), lang('common_pay'), array('style' => 'background: #3498db;background-image: linear-gradient(to bottom, #3498db, #2980b9);-webkit-border-radius: 28;-moz-border-radius: 28;border-radius: 28px;font-family: Arial;color: #ffffff;padding: 10px;text-decoration: none;font-weight: normal; margin-right:30px;')).'<br /><br /><br />';
					}
	...
	{
		$this->check_action_permission('can_lookup_receipt');
	...
	{
		$this->check_action_permission('can_lookup_last_receipt');
		$last_sale_id = $this->Sale->get_last_sale_id();
		$this->receipt($last_sale_id,array('permission_override' => 'can_lookup_last_receipt'));
	}

	function receipt($sale_id, $options = null)
	{		
		if (isset($options['permission_override']))
		{
			$this->check_action_permission($options['permission_override']);
		}
		else
		{
			$this->check_action_permission('can_lookup_receipt');
	...
	function suspend($suspend_type = 1)
	{
		...
		if($this->cart->sale_id){
			...
			if($sale_info['deleted'] == 1){
				$this->cart->empty_items();
				$this->cart->save();
				$data['error']=lang('sales_cannot_edited_already_deleted');
				$this->_reload($data);
				return;
			}
		}
		...
		$work_order_id = isset($work_order_info['id']) ? $work_order_info['id'] : null;
	...
	{
		if (!$this->Employee->has_module_action_permission('giftcards', 'can_sell_giftcard', $this->Employee->get_logged_in_employee_info()->person_id))
		{
			redirect('no_access/'.$this->module_id);
		}
	...
	function check_giftcard($giftcard_id = 0)
	{
		if (!$this->Employee->has_module_action_permission('giftcards', 'can_sell_giftcard', $this->Employee->get_logged_in_employee_info()->person_id))
		{
			redirect('no_access/'.$this->module_id);
		...
		$giftcard_data = NULL;
		$giftcard_last_log = NULL;
		if($giftcard_id > 0){
			$giftcard_data = $this->Giftcard->get_info($giftcard_id);
			$giftcard_logs = $this->Giftcard->get_giftcard_log($giftcard_id);
			if(count($giftcard_logs) > 0){
				$giftcard_last_log = $giftcard_logs[0];
			}
		}
		...
		$data['giftcard_last_log'] = $giftcard_last_log;

		$this->load->view("sales/giftcard_check", $data);
	}
	...
	function change_sale($sale_id,$is_ajax = false)
	{
		$sale_id =  str_replace(($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS').'%20','',$sale_id);
	...
	function unsuspend($sale_id = 0)
	{
		...
		$sale_info = $this->Sale->get_info($sale_id)->row_array();
		if($sale_info['suspended'] == 0){
			$this->session->set_flashdata('unsuspend_failed_message', lang('sales_cannot_unsuspended_already_completed'));
			redirect(site_url('sales/suspended'));
		}
		if($sale_info['deleted'] == 1){
			$this->session->set_flashdata('unsuspend_failed_message', lang('sales_cannot_unsuspended_already_deleted'));
			redirect(site_url('sales/suspended'));
		}

		if($this->config->item('do_not_suspended_receipt_created_by_someone_else')){
			$employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
			if($sale_info['employee_id'] != $employee_id){
				$this->session->set_flashdata('unsuspend_failed_message', lang('sales_do_not_have_permission_to_unsuspend'));
				redirect(site_url('sales/suspended'));
			}
		}
	...
	function disable_test_mode()
	{
		...
		if (!is_on_demo_host())
		{
			$this->session->set_userdata('test_mode', 0);
		}
	...
	function line_item_custom_fields()
	{
		$this->lang->load('config');
		$fields_prefs = $this->config->item('sale_line_item_custom_field_prefs') ? unserialize($this->config->item('sale_line_item_custom_field_prefs')) : array();
		$data = array_merge(array('controller_name' => strtolower(get_class())),$fields_prefs);
		$locations_list = $this->Location->get_all()->result();
		$data['locations'] = $locations_list;
		$data['action'] = 'save_line_item_custom_fields';
		$this->load->view('custom_fields',$data);
	}
	...
	function save_line_item_custom_fields()
	{
		$this->load->model('Appconfig');
		$this->Appconfig->save('sale_line_item_custom_field_prefs',serialize($this->input->post()));
	}
	...
	function save_line_item_custom_field()
	{
		$line_k = str_replace(array('line_','custom_field_','_value'),array('','',''),$this->input->post('name'));
		$line = explode("_",$line_k)[0];
		$k = explode("_",$line_k)[1];
		if ($this->Sale->get_line_item_custom_field($k) !== FALSE)
		{
			$line_item = $this->cart->get_item($line);

			if(($this->Sale->get_line_item_custom_field($k,'type')) == 'date')
			{
				$line_item->{'line_item_custom_field_'.$k.'_value'} = (string)strtotime($this->input->post('value'));
			}
			elseif(($this->Sale->get_line_item_custom_field($k,'type')) == 'image')
			{
				$this->load->library('image_lib');
				...
				$this->load->model('Appfile');
				$image_file_id = $this->Appfile->save($_FILES['value']['name'], file_get_contents($_FILES["value"]['tmp_name']));
				$line_item->{'line_item_custom_field_'.$k.'_value'} = $image_file_id;
			}
			elseif(($this->Sale->get_line_item_custom_field($k,'type')) == 'file')
			{
				$this->load->model('Appfile');
				$image_file_id = $this->Appfile->save($_FILES['value']['name'], file_get_contents($_FILES["value"]['tmp_name']));
				$line_item->{'line_item_custom_field_'.$k.'_value'} = $image_file_id;
			...
			{
				$line_item->{'line_item_custom_field_'.$k.'_value'} = $this->input->post('value');
			}
		}
	...
	function get_attributes_values() {
				...
				{ 	
					...
					if($variation_tbl)
						...
						$variation_qty = 0;
                                        ...
                                        if($do_not_allow_out_of_stock && $variation_qty <= 0 && $is_service == 0) {
		...
		function delete_line_item_custom_field_value($line, $k)
		{
			$line_item = $this->cart->get_item($line);
			$line_item->{'line_item_custom_field_'.$k.'_value'} = NULL;
			$this->cart->save();
		}
		...
		function view_transaction_history()
		{
			...
			$params = [
			    ...
			    'maxResults' => strtoupper(get_class($credit_card_processor)) =='VALORPROCESSOR' ? 1000 : 50,
				'startIndex' => 0,
			];
			
			if (strtoupper(get_class($credit_card_processor)) =='VALORPROCESSOR')
			{
				$params['show_declines'] = $this->input->get('show_declines') ? TRUE : FALSE;
			}
			...
			$this->load->view('sales/phppos_payments_transaction_history',$data);		
		}
		...
		function void_return_by_transaction_id($transactionId)
		{
			...
			if (!$credit_card_processor || !method_exists($credit_card_processor,'void_or_return_transaction_by_id'))
			{
				$this->_reload(array('error' => lang('sales_credit_card_processing_is_down')), false);
				return;
			}
			...
			if ($response = $credit_card_processor->void_or_return_transaction_by_id($transactionId,$amount))
			{
				$sale_id = $this->input->post('sale_id');
				...
				if($sale_id > 0){
					if( $this->Sale->get_sale_total($sale_id) - $response['authorizedAmount'] <= 1e-6 ) {
						// fully void or fully returned transaction
						$this->cart->destroy();
						$this->cart = new PHPPOSCartSale();
						$this->cart->set_mode('return');
						$this->cart->return_order($sale_id);

						$this->cart->add_payment(
							new PHPPOSCartPaymentSale(
								array(
						...
						$this->cart->destroy();
						$this->cart = new PHPPOSCartSale();
						$refund_item_id = $this->Item->create_or_update_item_by_name(lang("common_refund"));
						$refund_item_to_add = new PHPPOSCartItemSale(array('cart' => $this->cart, 'cost_price' => 0,'unit_price' => $response['authorizedAmount'],'scan' => $refund_item_id.'|FORCE_ITEM_ID|', 'quantity' => -1 ));
						$this->cart->add_item($refund_item_to_add);
						$this->cart->add_payment(
							new PHPPOSCartPaymentSale(
								array(
									'payment_type' => lang('common_credit'),
	...
	function return_order($order_id, $replace_cart = TRUE, $is_ajax= false){
		...
		if ($this->Sale->exists($order_id))
		{
			...
			{
				$this->cart->return_sale_id = $order_id;
				$this->cart->save();
			}
		}
    ...
    function complete_excel_import_sale() {
        ...
        for ($i = 0; $i < $numRows - 1; $i++) {
            ...
            $orig_sale_id = $sale_id;

            if ($this->config->item('do_not_preserve_sale_id_when_importing_sales')) {
                if ($orig_sale_id && isset($sale_id_mapping[$orig_sale_id])) {
                    $sale_id = $sale_id_mapping[$orig_sale_id];
                } else if ($orig_sale_id && !isset($sale_id_mapping[$orig_sale_id])) {
                    $sale_id = FALSE;
                    $is_new_sale = TRUE;
                }
            }
            ...
            if ($is_new_sale) {
                ...
                if ($this->config->item('do_not_preserve_sale_id_when_importing_sales') && $orig_sale_id) {
                    $sale_id_mapping[$orig_sale_id] = $sale_id;
                }
                if ($sale_id) {
                    $sale_item_data['sale_id'] = $sale_id;
                    $sale_item_data['description'] = $item_info->name;
	...
	/**
	 * Checks if the specified customer's balance is negative.
	 *
	 * @param int $customerId The ID of the customer to check.
	 * @return bool True if balance is below zero, otherwise false.
	 */
	public function check_customer_balance(int $customer_id)
	{
		$customerInfo = $this->Customer->get_info($customer_id);
		$customerBalance = $customerInfo->balance ?? 0;

		if ($customerBalance < 0) {
			return lang('sales_customer_account_balance_is_negative');
		}
	...
	/**
	 * Updates the customer balance negative status in session.
	 *
	 * Uses boolean validation to ensure proper type.
	 *
	 * @return void
	 */
	public function update_customer_balance_negative_status(): void
	{
		$status = filter_var($this->input->post('status'), FILTER_VALIDATE_BOOLEAN);
		$this->session->set_userdata('customer_balance_negative', $status);
	}
