...
$this->load->model('Auto_tier_range');
}
	...
	function index()
	{
		...
		$tiers=$tiers_result->result_array();
                ...
                $default_tier_dropdown=array(""=>lang('common_do_nothing'));

                foreach($tiers as $tier){
                        $default_tier_dropdown[$tier['id']]=$tier['name'];
                }
        ...
        $data['default_tier_dropdown']=$default_tier_dropdown;
		...
		$data['currency_denoms'] = $this->Register->get_register_currency_denominations();
               ...
               if ($this->db->table_exists('auto_tier_ranges'))
               {
                       $data['auto_tier_ranges'] = $this->Auto_tier_range->get_all();
               }
               else
               {
                       $data['auto_tier_ranges'] = $this->db->query('SELECT NULL AS id, NULL AS tier_id, NULL AS min_sales, NULL AS max_sales WHERE 1=2');
               }
		...
		foreach($data['shipping_zones']->result_array() as $shipping_zone)
		{
			$data['zones'][] = array('text' => $shipping_zone['name'], 'val' => $shipping_zone['id']);
               ...
               $data['module_menu_overrides'] = $this->config->item('module_menu_overrides') ? unserialize($this->config->item('module_menu_overrides')) : array();
	...
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
      ...
      curl_close($ch);		
		...
		'qr_code_price_include_tax'=>$this->input->post('qr_code_price_include_tax') ? 1 : 0,
		...
		'do_not_suspended_receipt_created_by_someone_else' => $this->input->post('do_not_suspended_receipt_created_by_someone_else') ? 1 : 0,
		'customers_store_accounts' => $this->input->post('customers_store_accounts') ? 1 : 0,
		'do_not_allow_charge_to_store_account_greater_than_amount_due' => $this->input->post('do_not_allow_charge_to_store_account_greater_than_amount_due') ? 1 : 0,
		'customer_with_negative_store_account_balances_must_keep_balance_at_or_below_0' => $this->input->post('customer_with_negative_store_account_balances_must_keep_balance_at_or_below_0') ? 1 : 0,
		...
		'enable_giftcard_security' => $this->input->post('enable_giftcard_security') ? 1 : 0,
		...
		'hide_price_on_qr_codes' => $this->input->post('hide_price_on_qr_codes') ? 1 : 0,
		...
		'scale_custom_format' => serialize($this->input->post('scale_custom_format')?$this->input->post('scale_custom_format'):array()),
		...
		'enable_ebt_payments' => $this->input->post('enable_ebt_payments') ? 1 : 0,
                ...
                'default_tier_id'=>$this->input->post('default_tier_id') ? $this->input->post('default_tier_id') : '',
		...
		'limit_manual_price_adj' => $this->input->post('limit_manual_price_adj') ? 1 : 0,
               'always_minimize_menu' => $this->input->post('always_minimize_menu')  ? 1 : 0,
                'hover_to_expand_sub_modules' => $this->input->post('hover_to_expand_sub_modules') ? 1 : 0,
                'module_menu_overrides' => $this->input->post('module_menu_overrides') ? serialize($this->input->post('module_menu_overrides')) : serialize(array()),
		...
		'sku_sync_field' => $this->input->post('sku_sync_field'),
                ...
                'do_not_preserve_sale_id_when_importing_sales' => $this->input->post('do_not_preserve_sale_id_when_importing_sales') ? 1 : 0,
		...
		'hide_name_on_qr_codes' => $this->input->post('hide_name_on_qr_codes') ? 1 : 0,
		...
		'override_fulfillment_sheet_title' => $this->input->post('override_fulfillment_sheet_title'),
		...
		'disable_verification_for_qr_codes' => $this->input->post('disable_verification_for_qr_codes') ? 1 : 0,
                ...
                'always_show_expire_date_in_recv' => $this->input->post('always_show_expire_date_in_recv') ? 1 : 0,
		...
		'work_order_all_warranty_repairs_are_free'	=> $this->input->post('work_order_all_warranty_repairs_are_free') ? 1 : 0,
		'take_inventory_out_of_system_when_creating_updating_work_order'	=> $this->input->post('take_inventory_out_of_system_when_creating_updating_work_order') ? 1 : 0,
		'create_work_order_is_checked_by_default_for_sale' => $this->input->post('create_work_order_is_checked_by_default_for_sale') ? 1 : 0,
		'allow_customers_to_pay_suspended_sales_online' => $this->input->post('allow_customers_to_pay_suspended_sales_online') ? 1 : 0,
		...
		'advanced_sales_interface' => $this->input->post('advanced_sales_interface') ? 1 : 0,
		'advanced_receiving_interface' => $this->input->post('advanced_receiving_interface') ? 1 : 0,
		...
		'show_exchanged_totals_on_receipt' => $this->input->post('show_exchanged_totals_on_receipt') ? 1 : 0,
                ...
                'show_item_description_on_work_order_sheet' => $this->input->post('show_item_description_on_work_order_sheet') ? 1 : 0,
                'disable_appointment_emails' => $this->input->post('disable_appointment_emails') ? 1 : 0,
		'allow_zero_price_location_price_overrides' => $this->input->post('allow_zero_price_location_price_overrides') ? 1 : 0,
		'hide_price_rule_on_receipt' => $this->input->post('hide_price_rule_on_receipt') ? 1 : 0,
		'work_order_terms' => $this->input->post('work_order_terms'),
		'show_email_of_company_on_receipt' => $this->input->post('show_email_of_company_on_receipt') ? 1 : 0,
		'update_base_unit_price_from_units' => $this->input->post('update_base_unit_price_from_units') ? 1 : 0,
		'sms_mobile_tel_username' => $this->input->post('sms_mobile_tel_username'),
		'sms_mobile_tel_password' => $this->input->post('sms_mobile_tel_password'),
		'sms_mobile_tel_alias' => $this->input->post('sms_mobile_tel_alias'),
		'show_weight_and_dim_on_receipt' => $this->input->post('show_weight_and_dim_on_receipt') ? 1 : 0,
		'always_enable_location_selling_price_during_receiving' => $this->input->post('always_enable_location_selling_price_during_receiving') ? 1 : 0,
		'inventory_post_date_is_always_sale_date' => $this->input->post('inventory_post_date_is_always_sale_date') ? 1 : 0,
		'show_line_item_sales_person_on_receipt' => $this->input->post('show_line_item_sales_person_on_receipt') ? 1 : 0,
		'hide_parts_and_labor_items_on_receipt' => $this->input->post('hide_parts_and_labor_items_on_receipt') ? 1 : 0,
		'show_debit_payment_option_when_using_integrated_credit_card_processing' => $this->input->post('show_debit_payment_option_when_using_integrated_credit_card_processing') ? 1 : 0,
		'shopify_step_size' => $this->input->post('shopify_step_size') ?? '',
		'do_not_format_phone_number' => $this->input->post('do_not_format_phone_number') ? 1 : 0,
		'do_not_allow_store_payment_more_than_balance' => $this->input->post('do_not_allow_store_payment_more_than_balance') ? 1 : 0,
		'require_payment_against_store_account_charge'	=> $this->input->post('require_payment_against_store_account_charge') ? 1 : 0,
		'hide_zendesk_chat' => $this->input->post('hide_zendesk_chat') ? 1 : 0,

	);

	$this->session->set_userdata('test_mode', $this->input->post('test_mode') ? 1 : 0);
		...
		if (is_on_demo_host())
		{
			...
			$batch_save_data['only_allow_sso_logins'] = 0;
               ...
               if($this->Appconfig->batch_save($batch_save_data)
                       ...
                       && $this->save_auto_tier_ranges($this->input->post('auto_tier_ranges'), $this->input->post('auto_tier_ranges_to_delete'))
	...
	function save_sale_types($sales_types_to_edit, $sales_types_to_delete)
	{
       ...
       function save_auto_tier_ranges($ranges_to_edit, $ranges_to_delete)
       {
               $this->load->model('Auto_tier_range');
               ...
               if ($ranges_to_edit)
               {
                       foreach($ranges_to_edit as $range_id => $data)
                       {
                               ...
                               if ($tier_id)
                               {
                                       ...
                                       $this->Auto_tier_range->save($range_data, $range_id < 0 ? false : $range_id);
                               }
                       }
               }
               ...
               if ($ranges_to_delete)
               {
                       foreach($ranges_to_delete as $range_id)
                       {
                               $this->Auto_tier_range->delete($range_id);
                       }
               }
	...
	function do_mysqldump_backup()
	{
		...
		if (is_callable('passthru') && false === stripos(ini_get('disable_functions'), 'passthru'))
		{
			...
			foreach($mysqldump_paths as $mysqldump)
			{
				...
				if (is_executable($mysqldump))
				{
					$backup_command = "\"$mysqldump\" --max-allowed-packet=1073741824 --host=$db_hostname --user=$db_username --password=$db_password --tz-utc=false --single-transaction --routines --triggers $database";
	...
	function ecom_documentation(){
       ...
       function calculate_customer_tiers()
       {
               session_write_close();
               if (!$this->db->table_exists('auto_tier_ranges'))
               {
                       echo json_encode(array('success'=>true,'message'=>lang('config_calculate_tier_ranges_successfully')));
                       return;
               }

               $ranges = $this->Auto_tier_range->get_all()->result_array();
               if ($ranges)
               {
                       $case_statement = "CASE ";
                       foreach($ranges as $range)
                       {
                               $case_statement .= "WHEN totals.total_spent >= " . $this->db->escape($range['min_sales']) .
                               " AND totals.total_spent <= " . $this->db->escape($range['max_sales']) .
                               " THEN " . $this->db->escape($range['tier_id']) . " ";
                       }
                       ...
                       $sql = "UPDATE $customers c " .
                               "LEFT JOIN (SELECT customer_id, SUM(total) AS total_spent FROM $sales WHERE deleted=0 AND suspended=0 GROUP BY customer_id) totals " .
                               "ON totals.customer_id = c.person_id " .
                               "SET c.tier_id = $case_statement " .
                               "WHERE c.deleted = 0";
