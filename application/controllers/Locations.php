...
class Locations extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function view($location_id=-1,$redirect=false)
	{
		...
		$exchange_rates_result = $this->Appconfig->get_exchange_rates();
		if ($exchange_rates_result && $exchange_rates_result->num_rows() > 0) {
			$data['exchange_currency_rates'] = ['' => lang('common_default')];
			foreach ($exchange_rates_result->result() as $rate)
			{
				$data['exchange_currency_rates'][$rate->id] = $rate->currency_code_to;
			}
		}
	...
	function save($location_id=-1)
	{
		...
		'tax_id' => $this->input->post('tax_id'),
               ...
               'do_not_allow_below_cost' => $this->input->post('do_not_allow_below_cost') !== '' ? (int)$this->input->post('do_not_allow_below_cost') : NULL,
               'do_not_allow_out_of_stock_items_to_be_sold' => $this->input->post('do_not_allow_out_of_stock_items_to_be_sold') !== '' ? (int)$this->input->post('do_not_allow_out_of_stock_items_to_be_sold') : NULL,
               'do_not_allow_items_to_go_out_of_stock_when_transfering' => $this->input->post('do_not_allow_items_to_go_out_of_stock_when_transfering') !== '' ? (int)$this->input->post('do_not_allow_items_to_go_out_of_stock_when_transfering') : NULL,
		...
		'valor_appid' => $this->input->post('valor_appid'),
		'valor_channel_override' => $this->input->post('valor_channel_override'),
		'currency_rate_id'	 => $this->input->post('currency_rate_id') ? $this->input->post('currency_rate_id') : NULL,
	);
		...
		{			
			...
			unset($location_data['valor_appid']);
	...
	{		
		if ($registers_to_edit)
		{
			foreach($registers_to_edit as $register_id => $register)
			{
				if ($register['name'])
				{
					$register_data = array('valor_appkey' => $register['valor_appkey'], 'enable_tips' => isset($register['enable_tips']) && $register['enable_tips'] ? 1 : 0,'emv_pinpad_ip' => $register['emv_pinpad_ip'],'emv_pinpad_port' => $register['emv_pinpad_port'],'card_connect_hsn' => $register['card_connect_hsn'], 'name' => $register['name'], 'iptran_device_id' => $register['iptran_device_id'], 'emv_terminal_id' => $register['emv_terminal_id'], 'secure_device_emv' => $register['secure_device_emv'], 'secure_device_non_emv' => $register['secure_device_non_emv'],'location_id' => $location_id);
	...
	function square_terminal_get_id($location_id=1,$register_id=1)
	{
		...
		$code_id = $api_response->getResult()->getDeviceCode()->getId();
		
		echo json_encode(['id' => $code_id, 'code' => $device_code]);
	}
	
	function square_get_and_set_device_id($device_code_id,$location_id=1,$register_id=1)
	{
	    ...
	    do 
		{
	        ...
	        $api_response = $client->getDevicesApi()->getDeviceCode($device_code_id);
			
	        if ($api_response->isSuccess()) 
			{
			    $result = $api_response->getResult();
			    $status = $result->getDeviceCode()->getStatus();
			    $device_id = $result->getDeviceCode()->getDeviceId();
		...
		while($status != 'PAIRED' && $attempt < $max_attempts);

	    // If paired, update the database
	    if ($status === 'PAIRED') 
		{
