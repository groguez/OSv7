...
class Customers extends Person_controller
{
	...
	{
		$params = $this->session->userdata('customers_search_data') ? $this->session->userdata('customers_search_data') : array('offset' => 0, 'search_type' => '', 'order_col' => 'last_name', 'order_dir' => 'asc', 'search' => FALSE, 'deleted' => 0, 'location_id' => $this->config->item('only_allow_current_location_customers') ? $this->Employee->get_logged_in_employee_current_location_id() : '');
		...
		if ($data['search'])
		{
			$config['total_rows'] = $this->Customer->search_count_all($data['search'],$params['location_id'],$params['deleted'],10000,$params['search_type']);
			$table_data = $this->Customer->search($data['search'],$params['location_id'],$params['deleted'],$data['per_page'],$params['offset'],$params['order_col'],$params['order_dir'], $params['search_type']);
		}
		else
		{
		...
		$data['search_type'] = array(
			'' => lang('common_all'),
			'full_name' => lang('common_name'), 
			'phone_number' => lang('common_phone'),
			'email' => lang('common_email'), 
			'address_1' => lang('common_address'), 
			'city' => lang('common_city'), 
			'state' => lang('common_state')
		);
		$data['selected_search_type'] = $params['search_type'];
	...
	function sorting()
	{
		$params = $this->session->userdata('customers_search_data') ? $this->session->userdata('customers_search_data') : array('order_col' => 'last_name', 'search_type' => '','order_dir' => 'asc','deleted' => 0,'location_id' => $this->config->item('only_allow_current_location_customers') ? $this->Employee->get_logged_in_employee_current_location_id() : '');
		...
		if ($search)
		{
			...
			$table_data = $this->Customer->search($search,$params['location_id'],$params['deleted'],$per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $order_col ,$order_dir, $params['search_type']);
		}
		else
		{
			$config['total_rows'] = $this->Customer->count_all($params['location_id'],$params['deleted']);
	...
	*/
	function search()
	{
		...
		$search_type=$this->input->post('search_type_id');
		$offset = $this->input->post('offset') ? $this->input->post('offset') : 0;
		...
		$customers_search_data = array('offset' => $offset, 'order_col' => $order_col, 'order_dir' => $order_dir, 'search' => $search,'deleted' => $deleted,'location_id' => $location_id, 'search_type' => $search_type);
		$this->session->set_userdata("customers_search_data",$customers_search_data);
		$per_page=$this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20;
		$search_data=$this->Customer->search($search,$location_id,$deleted,$per_page,$offset, $order_col ,$order_dir, $search_type);
		$config['base_url'] = site_url('customers/search');
		$config['total_rows'] = $this->Customer->search_count_all($search,$location_id,$deleted,10000,$search_type);
		$config['per_page'] = $per_page ;
		
		$this->load->library('pagination');$this->pagination->initialize($config);				
		$data['pagination'] = $this->pagination->create_links();
		$data['total_rows'] = $this->Customer->search_count_all($search,$location_id,$deleted,10000,$search_type);
	...
	*/
	function view($customer_id=-1,$redirect_code=0)
	{
		...
		$data['controller_name']=strtolower(get_class());
        $data['additional_phone_numbers'] = $this->Customer->get_additional_phone_numbers($customer_id);
	...
	{
		$params = $this->session->userdata('customers_search_data');
		$this->session->set_userdata('customers_search_data', array('offset' => 0,'search_type' => '', 'order_col' => 'last_name', 'order_dir' => 'asc', 'search' => FALSE, 'deleted' => $params['deleted'],'location_id' => $this->config->item('only_allow_current_location_customers') ? $this->Employee->get_logged_in_employee_current_location_id() : ''));
		redirect('customers');
	}
	/*
	...
	*/
	function save($customer_id=-1)
	{
		...
		if($this->Customer->save_customer($person_data,$customer_data,$customer_id))
		{
	        ...
	        $additional_phone_numbers = $this->input->post('additional_phone_numbers');
	        $this->Customer->save_additional_phone_numbers($customer_id !=-1 ? $customer_id : $customer_data['person_id'], $additional_phone_numbers);
	...
	*/
	...
	{		
		$return = array(lang('common_first_name'),lang('common_last_name'),lang('common_email'),lang('common_phone_number'),lang('common_phone_number_2'),lang('common_phone_number_3'),lang('common_phone_number_4'),lang('common_phone_number_5'),lang('common_phone_number_6'),lang('common_address_1'),lang('common_address_2'),lang('common_city'),	lang('common_state'),lang('common_zip'),lang('common_country'),lang('common_comments'),lang('customers_account_number'),lang('common_taxable'),lang('customers_tax_certificate'), lang('customers_company_name'),lang('common_tier_name'));
		
		$return[] = lang('common_internal_notes');
		$return[] = lang('common_customer_info_popup');
	...
	function excel_export() {
		...
		if ($search)
		{
			$data = $this->Customer->search($search,$params['location_id'],$params['deleted'],$this->Customer->search_count_all($search,$params['location_id'],$params['deleted']),0,$params['order_col'],$params['order_dir'],$params['search_type'])->result_object();
		}
		else
		{
		...
		foreach ($data as $r) {
			...
			$additional_phone_numbers = $this->Customer->get_additional_phone_numbers($r->person_id);
			...
			$row = array(
				...
				$r->email,
				$r->phone_number,
				isset($additional_phone_numbers[0]) ? format_phone_number($additional_phone_numbers[0]) : '',
				isset($additional_phone_numbers[1]) ? format_phone_number($additional_phone_numbers[1]) : '',
				isset($additional_phone_numbers[2]) ? format_phone_number($additional_phone_numbers[2]) : '',
				isset($additional_phone_numbers[3]) ? format_phone_number($additional_phone_numbers[3]) : '',
				isset($additional_phone_numbers[4]) ? format_phone_number($additional_phone_numbers[4]) : '',
				$r->address_1,
				$r->address_2,
				$r->city,
				$r->state,
	...
	{		
		...
		$fields[] = array('Name' => lang('common_phone_number_2'), 'key' => 'phone_number_2');
		$fields[] = array('Name' => lang('common_phone_number_3'), 'key' => 'phone_number_3');
		$fields[] = array('Name' => lang('common_phone_number_4'), 'key' => 'phone_number_4');
		$fields[] = array('Name' => lang('common_phone_number_5'), 'key' => 'phone_number_5');
		$fields[] = array('Name' => lang('common_phone_number_6'), 'key' => 'phone_number_6');
		...
		$fields[] = array('Name' => lang('common_customer_info_popup'), 'key' => 'customer_info_popup');	
		$this->lang->load('locations');
	...
	function dedup_excel_import_data()
	{
		...
		if (isset($fieldId_to_colIndex[16]))
		{
			$account_num_index = $fieldId_to_colIndex[16];
	...
	function complete_excel_import()
	{
		...
		for ($i = 0; $i < $numRows -1; $i++)
		{
			...
			foreach($fields as $field)
			{
				...
				if($field['key'] !== "")
				{
					//These are mapped differtnly
					if ($field['key'] == 'phone_number_2' || $field['key'] == 'phone_number_3' || $field['key'] == 'phone_number_4' || $field['key'] == 'phone_number_5' || $field['key'] == 'phone_number_6' )
					{
						$additional_phone_numbers[] = $this->_clean($field['key'], $columns_with_data[$key]['data'][$i], $i+2);
						continue;
					}
		...
		if ($can_commit)
		{				
			if (count($additional_phone_numbers) > 0)
			{
				$save_customer_id = $person_id ? $person_id : $customer_data['person_id'];
				$current_additional_phone_numbers = $this->Customer->get_additional_phone_numbers($save_customer_id);
				
				//we only allow 6 phone numbers in spreadsheet but if customer record has more then we need to keep those
				if (count($current_additional_phone_numbers) > 5)
				{
				   $excess_numbers = array_slice($current_additional_phone_numbers, 5);
				   $additional_phone_numbers = array_merge($additional_phone_numbers, $excess_numbers);
				}
				
				$this->Customer->save_additional_phone_numbers($save_customer_id, $additional_phone_numbers);
			}
