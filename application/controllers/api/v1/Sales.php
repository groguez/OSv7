...
class Sales extends REST_Controller {
	...
	function index_get($sale_id = NULL) 
	{
	        ...
	        else 
			{
	            ...
	            $location_id_param = urldecode($this->input->get('location_id'));
				...
				{					
	            	$sale_ids = $this->Sale->get_sale_ids($start_date, $end_date, $customer_id, $suspended_type_ids, $start_payment_date, $end_payment_date, $include_created_sales_in_range, $start_date_created, $end_date_created, $start_date_updated, $end_date_updated, $location_id);
				}
				else
				{
					$sale_ids = $this->Sale->get_sale_ids_for_range($start_date, $end_date, $customer_id, $location_id);
				}
				
	            $total_records = count($sale_ids);
				
	            $response = array();
            
				$limit = $this->input->get('limit') ?? 500; // Default limit is 500
				$offset = $this->input->get('offset') ?? 0; // Default offset is 0
			
				// Ensure the limit does not exceed 1000
				$limit = min($limit, 1000);
				
				$sale_ids = array_slice($sale_ids, $offset, $limit);
