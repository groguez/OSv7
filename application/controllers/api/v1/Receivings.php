...
class Receivings extends REST_Controller {
    ...
    function index_get($receiving_id = NULL) 
	{
        if ($receiving_id !== NULL) {
			...
			if ($start_date_created || $end_date_created  || $start_date_updated || $end_date_updated)
			{
            	$receiving_ids = $this->Receiving->get_receiving_ids($start_date, $end_date, $start_date_created, $end_date_created, $start_date_updated, $end_date_updated);				
			}
			else
			{
				$receiving_ids = $this->Receiving->get_receiving_ids_for_range($start_date,$end_date);
			}
            $total_records = count($receiving_ids);
			...
			$receiving_ids = array_slice($receiving_ids, $offset, $limit);
