    ...
    function __construct()
    {
        ...
        parent::__construct();
			...
			$this->load->helper('date');
               ...
               private function _resolve_appointments_type_id($type_input)
               {
                       ...
                       $info = $this->Appointment->get_info_category($type_input);
                       if($info->id)
                       {
                               return $info->id;
                       }

                       $type_id = $this->Appointment->get_category_id_by_name($type_input);
                       return $type_id ? $type_id : 1;
               }
    ...
    private function _create_appointments($appointments_request)
    {
			...
			$appointments_data=array(
				...
				'employee_id' => isset($appointments_request['employee_id']) && $appointments_request['employee_id'] ? $appointments_request['employee_id'] : 1,
                                'appointments_type_id' => $this->_resolve_appointments_type_id(isset($appointments_request['appointments_type_id']) ? $appointments_request['appointments_type_id'] : NULL),
  		...
  		$appointments = $this->Appointment->get_info($appointments_id);
			...
			if (isset($appointments_request['start_time']))
			{
				$appointments_request['start_time'] = date('Y-m-d H:i:s',strtotime($appointments_request['start_time']));
                        ...
                        if (isset($appointments_request['appointments_type_id']))
                        {
                                $appointments_request['appointments_type_id'] = $this->_resolve_appointments_type_id($appointments_request['appointments_type_id']);
                        }
