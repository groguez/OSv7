...
class Expenses extends Secure_area implements Idata_controller {
    ...
    function save($id = -1) 
    {
        ...
        if($id > 0){
            $former_expense_data = $this->Expense->get_info($id);
            $former_cash_register_id = $former_expense_data->register_id;
            if($former_cash_register_id){
                $this->Register->clear_cash_register_expenses($former_expense_data);
            }
        }
        ...
        if ($this->Expense->save($expense_data, $id))
        {
            ...
            {
                $expense_data['register_id'] = $this->input->post('cash_register_id');
                $this->Expense->save($expense_data, $id > 0 ? $id : $expense_data['id']);
                ...
                $register_audit_log_data = array(
                    ...
                    'expense_id' => $id > 0 ? $id : $expense_data['id']
                );
        
                $this->Register->insert_audit_log($register_audit_log_data);
			} else {
                $expense_data['register_id'] = NULL;
                $this->Expense->save($expense_data, $id > 0 ? $id : $expense_data['id']);
            }
