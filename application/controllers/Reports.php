...
{	
	...
	function generate($report)
	{
		...
		$authenticated_locations = $this->Employee->get_authenticated_location_ids($this->Employee->get_logged_in_employee_info()->person_id,TRUE);
		if (count($authenticated_locations) > 1){
			$selected_report_locations = array();
			$selected_report_location_ids = Report::get_selected_location_ids();
			foreach($selected_report_location_ids as $s_locationid){
				array_push($selected_report_locations,  $this->Location->get_info($s_locationid)->name);
			}
			$data['selected_report_locations'] = $selected_report_locations;
		}
		...
		if ($this->input->get('generate_report')) { // Generate Custom Raport
			...
			if (!$this->input->get('export_excel'))
			{
				...
				if (count($authenticated_locations) > 1){
					...
					$reportdata['selected_report_locations'] = $selected_report_locations;
				}
	...
	function category_search()
	{
		...
		//allow parallel searchs to improve performance.
		session_write_close();
		$suggestions = $this->Category->get_search_suggestions($this->input->get('term'));
	...
	public function add_saved_report()
	{
		...
		$base_url = base_url();
		$base_url = str_replace($prev_folder.'/','',$base_url);
		
		$url = $this->input->post('url');
		$url = str_replace($prev_folder.'/','',$url);
	 	$url = str_replace($base_url.'index.php','',$url);
