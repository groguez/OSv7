...
class Giftcards extends Secure_area implements Idata_controller
{
	...
	function _excel_get_header_row()
	{
		if($this->config->item('enable_giftcard_security')){
			return array(lang('giftcards_giftcard_number'),lang('common_description'),lang('giftcards_card_value'), lang('common_inactive'), lang('common_customer'), 'PIN');
		}
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				for($k = 2;$k<=$num_rows; $k++)
				{
					...
					if($this->config->item('enable_giftcard_security')){
						$pin = $sheet->getCellByColumnAndRow(5, $k);
						//If we don't have a gift pin skip the import
						if (!$pin) // required pin
						{
							continue;
						}
	...
	*/
		...
		if($this->config->item('enable_giftcard_security')){
			if($giftcard_id == -1){
				echo json_encode(array(
					'success' => false,
					'message' => lang('giftcards_disabled_new_through_page'),
					'giftcard_id'=> -1
				));
				exit();
			};
	...
	function giftcard_number_exists()
	{
		$this->check_action_permission('add_update');
		$giftcard_number = $this->input->post('giftcard_number');
		if($this->Giftcard->exist_giftcard_number($giftcard_number))
		{
			echo 'false'; exit();
		}
		echo 'true'; exit();
	...
	function check_giftcard_exist()
	{
		...
		if(!$giftcard_number || !$giftcard_pin || !($this->Giftcard->get_giftcard_id_from_number_pin($giftcard_number, $giftcard_pin)>0)){
			echo json_encode(
				array(
					'success' => false,
					'message' => lang('giftcards_does_not_exist'),
					'item_id'=>-1
				)
			);
			exit();
		...
		echo json_encode(
			array(
				'success' => true,
				'message' => lang('common_success'),
				'item_id'=>$this->Giftcard->get_giftcard_id_from_number_pin($giftcard_number, $giftcard_pin)
			)
		);
		exit();
	}
		...
		if($this->config->item('enable_giftcard_security')){
			...
			if(!$giftcard_number || !$giftcard_pin || !($this->Giftcard->get_giftcard_id_from_number_pin($giftcard_number, $giftcard_pin)>0)){
				echo json_encode(
					array(
						'success' => false,
						'message' => lang('common_error_adding_updating').' '.H($item_name),
						'item_id'=>-1
					)
				);
				exit();
