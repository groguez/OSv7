	...
	*/
	function save($employee_id=-1)
	{
		...
		if($this->input->post('password')!='')
		{
			...
			if ($employee_id!=-1)
			{
				$this->Employee_appconfig->save('changed_password', 1,$employee_id);
			
				$cur_timezone = date_default_timezone_get();
				//We are doing this to make sure same timezone is used for expiration date
				date_default_timezone_set('America/New_York');
				$this->Employee_appconfig->save('changed_password_expire', strtotime('+16 hours'),$employee_id);
				date_default_timezone_set($cur_timezone);
			
				$this->load->helper('array');
				$allowed_sessions = json_decode($this->Employee_appconfig->get('allowed_sessions',$employee_id), TRUE);
				$allowed_sessions = delete_all_elements_in_array_not_matching_value(session_id(),$allowed_sessions);
				$this->Employee_appconfig->save('allowed_sessions',json_encode($allowed_sessions),$employee_id);
			}
		}
	...
	*/
	...
	function do_edit_profile()
	{
		...
		if($this->input->post('password')!='')
		{
			...
			$this->Employee_appconfig->save('changed_password', 1);
			
			$cur_timezone = date_default_timezone_get();
			//We are doing this to make sure same timezone is used for expiration date
			date_default_timezone_set('America/New_York');
			$this->Employee_appconfig->save('changed_password_expire', strtotime('+16 hours'));
			date_default_timezone_set($cur_timezone);
			
			$this->load->helper('array');
			$allowed_sessions = json_decode($this->Employee_appconfig->get('allowed_sessions'), TRUE);
			$allowed_sessions = delete_all_elements_in_array_not_matching_value(session_id(),$allowed_sessions);
			$this->Employee_appconfig->save('allowed_sessions',json_encode($allowed_sessions));
