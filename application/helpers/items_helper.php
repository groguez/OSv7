...
function get_item_variations_qr_code_data($item_variation_ids)
{
	$CI =& get_instance();	
	$CI->load->model('Item');
	$CI->load->model('Item_variations');
...
function get_items_qr_code_data($item_ids, $predefined_qr_code=false, $custom_fields = false, $estimated_repair_date = false)
{
	$CI =& get_instance();	
	
	$hide_prices = $CI->config->item('hide_price_on_qr_codes');
	...
	foreach ($item_ids as $item_id)
	{
		$qr_code_number = number_pad($item_id,$CI->config->item('zerofill_barcode') ? $CI->config->item('zerofill_barcode') : 10);	
		...
		$barcode_name = $CI->config->item('hide_name_on_qr_codes') ? '' : ($item_info->barcode_name ? $item_info->barcode_name : $item_info->name).($item_info->size ? ' ('.$item_info->size.')' : '');
		...
		if ($id_to_show_on_barcode = $CI->config->item('id_to_show_on_barcode'))
		{
			...
			elseif($id_to_show_on_barcode == 'number')
			{
				$qr_code_number = $item_info->item_number ? $item_info->item_number : number_pad($item_id,$CI->config->item('zerofill_barcode') ? $CI->config->item('zerofill_barcode') : 10);
			}
			elseif($id_to_show_on_barcode == 'product_id')
			{
				$qr_code_number = $item_info->product_id ? $item_info->product_id : number_pad($item_id,$CI->config->item('zerofill_barcode') ? $CI->config->item('zerofill_barcode') : 10);
			}
		}
		...
		$qr_code_number = $predefined_qr_code ? $predefined_qr_code : $qr_code_number;
		
		if($CI->config->item('qr_code_price_include_tax'))
		{
			if($item_info->tax_included)
			{
				$result[] = array('description' => $item_info->description, 'name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency($regular_item_price).'</span> ' : ' ').'<span class="item-price-barcode" style="font-weight:bold;">'.to_currency($item_price).'</span> '.$barcode_name) : $barcode_name , 'id'=> $qr_code_number);
			}
			else
			{				
				$result[] = array('description' => $item_info->description, 'name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency(get_price_for_item_including_taxes($item_id,$regular_item_price)).'</span> ' : ' ').'<span class="item-price-barcode" style="font-weight:bold;">'.to_currency(get_price_for_item_including_taxes($item_id,$item_price)).'</span> '.$barcode_name) : $barcode_name, 'id'=> $qr_code_number);
	  	 	}
	  }
	  else
	  {
		if ($item_info->tax_included)
		{
		    $result[] = array('description' => $item_info->description, 'name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency(get_price_for_item_excluding_taxes($item_id, $regular_item_price)).'</span> ' : ' ').'<span class="item-price-barcode" style="font-weight:bold;">'.to_currency(get_price_for_item_excluding_taxes($item_id, $item_price)).'</span> '.$barcode_name) : $barcode_name, 'id'=> $qr_code_number);
		}
		else
		{
			$result[] = array('description' => $item_info->description, 'name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency($regular_item_price).'</span> ' : ' ').'<span class="item-price-barcode" style="font-weight:bold;">'.to_currency($item_price).'</span> '.$barcode_name) : $barcode_name, 'id'=> $qr_code_number);
...
function get_commission_for_item($cart, $item_id, $price, $cost, $quantity, $discount, $sold_employee_id_2 = 0)
{
	if ($price == 0)
	{
		return 0;
	...
	if($sold_employee_id_2 > 0){
		$employee_id = $sold_employee_id_2;
		$sales_person_info = $CI->Employee->get_info($employee_id);
	}
...
function parse_item_scan_data($scan)
{
	...
	if (strpos($scan,'|FORCE_ITEM_ID|') !== FALSE)
	{
		...
		//Lookup item based on just store config; ignore all other fields
               $return['item_id'] = $CI->Item->lookup_item_id($scan,array('item_number','product_id','additional_item_numbers','item_variation_item_number','serial_numbers','ecommerce_product_id'));
	}
	else
	{
...
function get_scale_format_attribute($scan, $attribute = "item_number"){
	...
	if($scale_format == 'scale_0')
	{
		$scale_custom_format = $CI->config->item('scale_custom_format') ? unserialize($CI->config->item('scale_custom_format')) : array();
		foreach($scale_custom_format as $k=>$s_b_i)
		{
			if($s_b_i == $attribute)
			{
				$attribute_value .= substr($scan, $k , 1);
			}
		}
		...
		if ($attribute_value)
		{
			if($attribute == 'weight')
			{
				$attribute_value = $attribute_value/$divide_by;
			} 
	...
	{
		if($attribute == 'item_number')
		{
			$attribute_value = substr($scan,$number_start_index,($number_end_index+1) - $number_start_index);
		} 
		else if($attribute == 'weight')
		{
			$attribute_value = substr($scan,$quantity_start_index,($quantity_end_index+1) - $quantity_start_index)/$divide_by;
		} 
		else if($attribute == 'price')
		{
			$attribute_value = substr($scan,$price_start_index,($price_end_index+1) - $price_start_index)/$divide_by;
		}
	}
...
function parse_scale_data($scan)
{
	...
	$item_number = get_scale_format_attribute( $scan, 'item_number' );
	...
	if (($scale_format == 'scale_0' && get_scale_format_attribute($scan, "price")) || ($scale_format == 'scale_1' || $scale_format == 'scale_2' || $scale_format == 'scale_3' || $scale_format == 'scale_4'))
	{
		$total_price = get_scale_format_attribute($scan, 'price');
		$sell_quantity = (float)$item_price ? $total_price/$item_price : 1;
		$cost_quantity = (float)$item_cost_price ? $total_price/$item_cost_price : 1;
	...
	elseif (($scale_format == 'scale_0' && get_scale_format_attribute($scan, "weight")) || ($scale_format == 'scale_5' || $scale_format == 'scale_6' || $scale_format == 'scale_7' || $scale_format == 'scale_8'))
	{
		$total_quantity = get_scale_format_attribute($scan, "weight");
