...
function shopify_app_file_url($file_id)
{
    $CI =& get_instance();
  	$CI->load->model('Appfile');	
	$signature = $CI->Appfile->get_signature($file_id);
	$app_file_info = $CI->Appfile->get_file_info($file_id);
	return site_url('app_files/view_signed_shopify/'.$file_id.'/'.rawurlencode($signature.'_'.$CI->Appfile->get_file_timestamp($file_id).'_'.clean_filename($app_file_info->file_name)));
}
...
{
	// 1) Extract the path, ignoring query strings and fragments.
	$path = parse_url($url, PHP_URL_PATH);
	// $path now looks like: "skin1004-50ml-probio-cica-glow-sun-ampoule-41175245684982_1440x.png.webp"

	// 2) Get the final extension. (In this case, "webp".)
	$finalExtension = pathinfo($path, PATHINFO_EXTENSION);

	// 3) Get the "filename" portion (excluding the final extension).
	//    This might still contain an earlier extension, e.g. "skin1004-..._1440x.png"
	$filenamePart = pathinfo($path, PATHINFO_FILENAME);

	// 4) Strip any additional extension (e.g., from .png), leaving only the bare name.
	$bareName = pathinfo($filenamePart, PATHINFO_FILENAME);

	// 5) Combine the bare name with the final extension.
	$newFilename = $bareName . '.' . $finalExtension;
	
	return $newFilename;
}

//Only change the base_url function if on a phppos_host. We do this so we can have 2 versions of application running at once
if (is_on_phppos_host())
{
	function base_url($uri = '', $protocol = NULL)
	{
	    $CI =& get_instance();
		$current_base_url = $CI->config->item('base_url');
	
		if (defined('IS_PREVIOUS') && IS_PREVIOUS)
		{
			$prev_folder = isset($_SERVER['CI_PREV_FOLDER']) ?  $_SERVER['CI_PREV_FOLDER'] : 'PHP-Point-Of-Sale-Prev';
			$CI->config->set_item('base_url',$current_base_url.$prev_folder.'/');
		}
	
		if (defined('IS_NEW') && IS_NEW)
		{
			$prev_folder = isset($_SERVER['CI_PREV_FOLDER']) ?  $_SERVER['CI_PREV_FOLDER'] : 'PHP-Point-Of-Sale-Prev';
			$CI->config->set_item('base_url',str_replace($prev_folder.'/','',$current_base_url));
		}
	
		//Get the url then set back so next url works without appending it over and over
		$url = $CI->config->base_url($uri, $protocol);
		$CI->config->set_item('base_url',$current_base_url);
		return $url;
	}
}
