...
function get_item_data_row($item,$controller)
{
	...
	if(!$params['deleted'])
	{
		...
		$table_data_row.= '<li>'. anchor($controller_name."/qr_barcodes/$item->item_id?redirect=items", '<i class="ion-android-print"></i> ' .  lang('common_print') .' '. 'QR/' . lang('common_barcodes') , array('class'=>'','title'=>'QR'.'/'.lang('common_barcodes'))).'</li>';
...
function get_suspended_receivings_manage_table($items,$controller)
{
	...
	$headers[] = array('label' => lang('common_email_receipt'), 'sort_column' => '');
	if ($CI->Employee->has_module_action_permission($controller_name, 'delete_suspended_receiving', $CI->Employee->get_logged_in_employee_info()->person_id)) {
		$headers[] = array('label' => lang('common_delete'), 'sort_column' => '');
	}
