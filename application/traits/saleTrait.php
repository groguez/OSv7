...
trait saleTrait
{
	private function sale_id_to_array($sale_id)
	{
		...
		if (get_parent_class() == 'REST_Controller')
		{
        ...
        $response['created_at'] = $sale_info['created_at'] ? date_as_display_datetime($sale_info['created_at']) : NULL;
        $response['order_time'] = $response['created_at'];
        $response['updated_at'] = $sale_info['updated_at'] ? date_as_display_datetime($sale_info['updated_at']) : NULL;
		...
		foreach($receipt_cart->get_items() as $cart_item)
		{
			...
			$cart_item_row['quantity'] = to_quantity($cart_item->quantity);
                        $cart_item_row['unit_price'] = to_currency_no_money($cart_item->unit_price,10);
                        $cart_item_row['regular_price'] = to_currency_no_money($cart_item->regular_price,10);
			$cart_item_row['cost_price'] = to_currency_no_money($cart_item->cost_price,10);
			$cart_item_row['discount'] = to_quantity($cart_item->discount);
			$this->load->model('Category');
			$cart_item_row['category_name'] = $this->Category->get_info($cart_item->category_id, TRUE)->name;
			
			$cart_item_row['discount_percent'] = to_quantity($cart_item->discount);
			$cart_item_row['discount_flat_discount'] = to_quantity($cart_item->flat_discount_amount);
