...
function enable_add_to_sale(confirm_message, none_selected_message, item_type) {
    //Keep track of enable_add_to_sale has been called
    if (!enable_add_to_sale.enabled)
        enable_add_to_sale.enabled = true;

    $('#add_to_sale').click(function(event) {
        event.preventDefault();
        if ($("#sortable_table tbody :checkbox:checked").length > 0) {
            bootbox.confirm(confirm_message, function(result) {
                if (result) {
                    do_add_to_sale($("#add_to_sale").attr('href'), item_type);
                }
            });
        }
    });
}
enable_add_to_sale.enabled = false;

function do_add_to_sale(url, item_type) {
    //If add_to_sale is not enabled, don't do anything
    if (!enable_add_to_sale.enabled)
        return;

    var row_ids = get_selected_values();
    var selected_rows = get_selected_rows();
    $.post(url, { 'ids[]': row_ids, 'item_type': item_type }, function(response) {
        //add_to_sale was successful, remove checkbox rows
        if (response.success) {
            show_feedback('success', response.message, COMMON_SUCCESS);
            bootbox.confirm(response.confirm_go_to_sale_screen, function(result) {
                if (result) {
                    location.href = response.sale_url;
                }
            });
        } else {
            show_feedback('error', response.message, COMMON_ERROR);
...
function enable_add_to_receiving(confirm_message, none_selected_message, item_type) {
    //Keep track of enable_add_to_receiving has been called
    if (!enable_add_to_receiving.enabled)
        enable_add_to_receiving.enabled = true;

    $('#add_to_receiving').click(function(event) {
        event.preventDefault();
        if ($("#sortable_table tbody :checkbox:checked").length > 0) {
            bootbox.confirm(confirm_message, function(result) {
                if (result) {
                    do_add_to_receiving($("#add_to_receiving").attr('href'), item_type);
                }
            });
        }
    });
}
enable_add_to_receiving.enabled = false;

function do_add_to_receiving(url, item_type) {
    //If add_to_receiving is not enabled, don't do anything
    if (!enable_add_to_receiving.enabled)
        return;
    ...
    var selected_rows = get_selected_rows();
    $.post(url, { 'ids[]': row_ids, 'item_type': item_type }, function(response) {
        //add_to_receiving was successful, remove checkbox rows
        if (response.success) {
            show_feedback('success', response.message, COMMON_SUCCESS);
            bootbox.confirm(response.confirm_go_to_sale_screen, function(result) {
                if (result) {
